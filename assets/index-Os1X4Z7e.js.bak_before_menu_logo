...
const numberToWordsVN_v3 = (num) => {
  ...
  function readGroup(n) {
    ...
    if (ch === 0 && dv > 0 && tr > 0) s += "lẻ ";
    else if (ch === 1) s += teens[dv] + " ";
    else if (ch > 1) {
      s += units[ch] + " mươi ";
      if (dv === 1) s += "mốt ";
      else if (dv === 5) s += "lăm ";
  ...
  let tr = Math.floor((num % 1000000000) / 1000000);
  let ng = Math.floor((num % 1000000) / 1000);
  let d = Math.floor(num % 1000);
  if (b > 0) s += readGroup(b) + " tỷ ";
  if (tr > 0) s += readGroup(tr) + " triệu ";
  if (ng > 0) s += readGroup(ng) + " ngàn ";
  if (d > 0) s += readGroup(d) + " ";
  s = s.trim() + " đồng";
  return s.charAt(0).toUpperCase() + s.slice(1);
};

const getDefaultTraderProfileV3 = () => ({
  fullName: "Cơ Sở Thu Mua Tôm Càng Xanh",
  phone: "0918 123 456",
  idCard: "089090012345",
  address: "Khóm 1, TT. Năm Căn, H. Năm Căn, Tỉnh Cà Mau",
  shrimpTypes: [
    { id: "st1", name: "Tôm càng sen", price: 160000 },
    { id: "st2", name: "Tôm càng xào", price: 110000 },
    { id: "st3", name: "Tôm ngộp", price: 85000 },
    { id: "st4", name: "Tôm lột", price: 95000 },
    { id: "st5", name: "Tôm càng đại (loại 1)", price: 190000 },
    { id: "st6", name: "Tôm xô tuyển chọn tại ao", price: 135000 },
    { id: "st7", name: "Tôm dạt / gãy càng", price: 75000 }
  ],
  shrimpSizes: [
    { id: "st1", name: "Tôm càng sen", price: 160000 },
    { id: "st2", name: "Tôm càng xào", price: 110000 },
  ...
  catchingSpecs: [
    ...
    { id: "c4", text: "Quy cách trừ bì: 100 con trừ 3 kg (hoặc theo thỏa thuận tại ao)", defaultSelected: true },
...
const getTraderProfileV3 = () => {
  ...
  return getDefaultTraderProfileV3();
};
...
const Xm_OverviewTab = ({ setRoute, onViewDetail, role }) => {
  const [selectedDate, setSelectedDate] = w.useState(() => {
    const d = new Date();
    return d.toISOString().split("T")[0];
  });
  const [filterMode, setFilterMode] = w.useState("DATE"); // "DATE" | "ALL"
  const [sessions, setSessions] = w.useState([]);
  ...
  const filteredSessions = w.useMemo(() => {
  ...
  }, [sessions, selectedDate, filterMode]);

  const stats = w.useMemo(() => {
    let totalKg = 0;
    let totalMoney = 0;
    let totalDeposit = 0;
    let totalPaid = 0;
    ...
    filteredSessions.forEach(s => {
      ...
      const money = Number(s.totalMoney || 0);
      ...
      if (Array.isArray(s.batches) && s.batches.length > 0) {
        s.batches.forEach(b => {
          const tName = (b.typeName || b.riceType || "Tôm loại khác").trim();
          const bKg = Number(b.netWeight || 0);
          const bMoney = Number(b.totalMoney || (bKg * (Number(b.price) || 0)));
          if (!typeMap[tName]) typeMap[tName] = { name: tName, kg: 0, money: 0, count: 0 };
          typeMap[tName].kg += bKg;
          typeMap[tName].money += bMoney;
          typeMap[tName].count += 1;
        });
      } else {
        const tName = (s.riceType || "Tôm loại 1").trim();
        if (!typeMap[tName]) typeMap[tName] = { name: tName, kg: 0, money: 0, count: 0 };
        typeMap[tName].kg += net;
        typeMap[tName].money += money;
        typeMap[tName].count += 1;
      }
    });

    const typeList = Object.values(typeMap).sort((a, b) => b.kg - a.kg);
    return {
  ...
  return c.jsxs("div", {
    className: "flex flex-col h-full bg-[#f8f9fa] dark:bg-[#121212] font-roboto overflow-y-auto pb-24",
    children: [
      // Top Bar
      c.jsxs("div", {
        ...
        children: [
          c.jsxs("div", {
            ...
            children: [
              ...
              c.jsxs("div", {
                children: [
                  c.jsx("h1", { className: "text-lg font-black uppercase tracking-tight", children: "Tổng quan thu mua" }),
                  c.jsx("p", { className: "text-[10px] opacity-80 uppercase tracking-wider font-bold", children: "Thống kê sản lượng & tài chính theo ngày" })
                ]
              })
            ]
          }),
          c.jsx("button", {
            onClick: () => setRoute(re.WEIGHING),
            className: "bg-yellow-400 text-green-950 px-3 py-1.5 rounded-xl font-black text-xs uppercase flex items-center gap-1.5 shadow-md active:scale-95",
            children: [c.jsx($u, { size: 16 }), "Cân ngay"]
          })
        ]
      }),
      ...
      c.jsxs("div", {
        ...
        children: [
          // Date Filter Card
          c.jsxs("div", {
            className: "bg-white dark:bg-gray-900 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 space-y-3",
            children: [
              c.jsxs("div", {
                ...
                children: [
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsx($1, { size: 18, className: "text-green-600 dark:text-green-400" }),
                      c.jsx("span", { className: "text-xs font-black uppercase tracking-wider text-gray-500 dark:text-gray-400", children: "Thời gian xem báo cáo" })
                    ]
                  }),
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsx("button", {
                        ...
                        className: `px-2.5 py-1 rounded-lg text-xs font-black uppercase transition-all ${filterMode === "DATE" && selectedDate === new Date().toISOString().split("T")[0] ? "bg-green-600 text-white shadow-sm" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300"}`,
                        children: "Hôm nay"
                      }),
                      c.jsx("button", {
                        onClick: () => setQuickDate(1),
                        className: `px-2.5 py-1 rounded-lg text-xs font-black uppercase transition-all ${filterMode === "DATE" && selectedDate === new Date(Date.now() - 86400000).toISOString().split("T")[0] ? "bg-green-600 text-white shadow-sm" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300"}`,
                        children: "Hôm qua"
                      }),
                      c.jsx("button", {
                        onClick: () => setFilterMode("ALL"),
                        className: `px-2.5 py-1 rounded-lg text-xs font-black uppercase transition-all ${filterMode === "ALL" ? "bg-green-600 text-white shadow-sm" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300"}`,
                        children: "Tất cả"
                      })
                    ]
                  })
              ...
              filterMode === "DATE" && c.jsx("input", {
                type: "date",
                value: selectedDate,
                onChange: (e) => setSelectedDate(e.target.value),
                className: "w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl font-bold text-gray-800 dark:text-gray-100 outline-none text-center"
              })
            ]
          }),
          ...
          c.jsxs("div", {
            ...
            children: [
              // Total Weight (Kg)
              c.jsxs("div", {
                className: "bg-gradient-to-br from-[#2e7d32] to-[#1b5e20] text-white p-5 rounded-[28px] shadow-lg relative overflow-hidden",
                children: [
                  c.jsx("span", { className: "text-[10px] font-black uppercase tracking-wider opacity-80 block mb-1", children: "Tổng lượng tôm mua" }),
                  c.jsxs("span", { className: "text-3xl font-black italic tracking-tighter tabular-nums", children: [stats.totalKg.toLocaleString("vi-VN"), " ", c.jsx("span", { className: "text-sm", children: "KG" })] }),
                  c.jsx("div", { className: "mt-2 text-[11px] font-bold opacity-90", children: [stats.sessionCount, " lượt thu mua"] }),
                  c.jsx($u, { size: 64, className: "absolute -right-2 -bottom-2 opacity-10" })
                ]
              }),

              // Total Money
              c.jsxs("div", {
                className: "bg-gradient-to-br from-[#1565c0] to-[#0d47a1] text-white p-5 rounded-[28px] shadow-lg relative overflow-hidden",
                children: [
                  c.jsx("span", { className: "text-[10px] font-black uppercase tracking-wider opacity-80 block mb-1", children: "Tổng tiền mua tôm" }),
                  c.jsxs("span", { className: "text-2xl font-black italic tracking-tighter tabular-nums", children: [(stats.totalMoney / 1000000).toFixed(2), " ", c.jsx("span", { className: "text-xs", children: "Triệu" })] }),
                  c.jsx("div", { className: "mt-2 text-[11px] font-bold opacity-90 truncate", children: fmtVND(stats.totalMoney) }),
                  c.jsx(zm, { size: 64, className: "absolute -right-2 -bottom-2 opacity-10" })
                ]
              }),

              // Total Deposit deducted
              c.jsxs("div", {
                className: "bg-white dark:bg-gray-900 p-4 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm",
                children: [
                  c.jsx("span", { className: "text-[10px] font-black uppercase text-gray-400 block mb-1", children: "Tiền cọc cấn trừ" }),
                  c.jsx("span", { className: "text-lg font-black text-purple-600 dark:text-purple-400 tabular-nums", children: fmtVND(stats.totalDeposit) })
                ]
              }),
              ...
              c.jsxs("div", {
                className: "bg-white dark:bg-gray-900 p-4 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm",
                children: [
                  c.jsx("span", { className: "text-[10px] font-black uppercase text-gray-400 block mb-1", children: "Tiền thực chi trả" }),
                  c.jsx("span", { className: "text-lg font-black text-amber-600 dark:text-amber-400 tabular-nums", children: fmtVND(stats.totalPaid || Math.max(0, stats.totalMoney - stats.totalDeposit)) })
                ]
              })
            ]
          }),

          // SHRIMP TYPES BREAKDOWN (Loại tôm trong ngày)
          c.jsxs("div", {
            className: "bg-white dark:bg-gray-900 p-5 rounded-[30px] shadow-sm border border-gray-100 dark:border-gray-800 space-y-4",
            children: [
              c.jsxs("div", {
                className: "flex items-center justify-between",
                children: [
                  c.jsxs("div", {
                    className: "flex items-center gap-2",
                    children: [
                      c.jsx(pm, { size: 18, className: "text-amber-500" }),
                      c.jsx("h3", { className: "font-black uppercase text-sm text-gray-800 dark:text-gray-100 tracking-tight", children: "Chi tiết các loại tôm đã mua" })
                    ]
                  }),
                  c.jsxs("span", { className: "text-xs font-bold text-gray-400 uppercase", children: [stats.typeList.length, " loại"] })
                ]
              }),

              stats.typeList.length === 0 ? c.jsx("div", {
                className: "py-8 text-center text-gray-400 text-xs font-bold uppercase",
                children: "Chưa có dữ liệu mua tôm trong ngày được chọn"
              }) : c.jsx("div", {
                className: "space-y-3",
                children: stats.typeList.map((t, idx) => {
                  const pct = stats.totalKg > 0 ? Math.round((t.kg / stats.totalKg) * 100) : 0;
                  const avgPrice = t.kg > 0 ? Math.round(t.money / t.kg) : 0;
                  return c.jsxs("div", {
                    className: "p-3.5 bg-gray-50 dark:bg-gray-800/60 rounded-2xl border border-gray-100 dark:border-gray-700/50 space-y-2",
                    children: [
                      c.jsxs("div", {
                        className: "flex items-center justify-between",
                        children: [
                          c.jsxs("div", {
                            className: "flex items-center gap-2",
                            children: [
                              c.jsx("div", { className: "w-6 h-6 rounded-lg bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300 flex items-center justify-center font-black text-xs", children: idx + 1 }),
                              c.jsx("span", { className: "font-black text-sm text-gray-800 dark:text-gray-100", children: t.name })
                            ]
                          }),
                          c.jsxs("div", {
                            className: "text-right",
                            children: [
                              c.jsxs("span", { className: "font-black text-base text-green-700 dark:text-green-400 tabular-nums", children: [t.kg.toLocaleString("vi-VN"), " kg"] }),
                              c.jsxs("span", { className: "ml-2 text-[11px] font-bold text-gray-400", children: ["(", pct, "%)"] })
                            ]
                          })
                        ]
                      }),
                      ...
                      // Sub info: money & avg price
                      c.jsxs("div", {
                        className: "flex items-center justify-between text-[11px] font-bold text-gray-500 dark:text-gray-400 pt-1",
                        children: [
                          c.jsxs("span", { children: ["Thành tiền: ", c.jsx("strong", { className: "text-red-600 dark:text-red-400", children: fmtVND(t.money) })] }),
                          c.jsxs("span", { children: ["Giá TB: ", c.jsx("strong", { className: "text-blue-600 dark:text-blue-400", children: fmtVND(avgPrice) }), "/kg"] })
                        ]
                      })
                    ]
                  }, t.name || idx);
          ...
          // SESSIONS LIST IN SELECTED DATE
          c.jsxs("div", {
            className: "bg-white dark:bg-gray-900 p-5 rounded-[30px] shadow-sm border border-gray-100 dark:border-gray-800 space-y-3",
            children: [
              c.jsxs("div", {
                className: "flex items-center justify-between",
                children: [
                  c.jsxs("div", {
                    className: "flex items-center gap-2",
                    children: [
                      c.jsx(Fd, { size: 18, className: "text-blue-600 dark:text-blue-400" }),
                      c.jsx("h3", { className: "font-black uppercase text-sm text-gray-800 dark:text-gray-100 tracking-tight", children: "Danh sách phiếu cân trong ngày" })
                    ]
                  }),
                  c.jsx("button", {
                    onClick: () => setRoute(re.HISTORY),
                    className: "text-xs font-black text-green-600 dark:text-green-400 uppercase hover:underline",
                    children: "Xem tất cả lịch sử →"
                  })
                ]
              }),

              filteredSessions.length === 0 ? c.jsx("div", {
                className: "py-6 text-center text-gray-400 text-xs font-bold uppercase",
                children: "Không có phiếu cân nào trong khoảng thời gian này"
              }) : c.jsx("div", {
                className: "space-y-2",
                children: filteredSessions.map((s) => c.jsxs("div", {
                  onClick: () => onViewDetail ? onViewDetail(s.id) : setRoute(re.HISTORY),
                  className: "p-3.5 bg-gray-50 dark:bg-gray-800/60 rounded-2xl border border-gray-100 dark:border-gray-700/50 flex items-center justify-between cursor-pointer hover:bg-green-50/50 transition-colors",
                  children: [
                    c.jsxs("div", {
                      children: [
                        c.jsx("p", { className: "font-black text-sm text-gray-800 dark:text-gray-100 uppercase", children: s.farmerName || "Chủ ao" }),
                        c.jsxs("p", { className: "text-[11px] text-gray-400 font-bold", children: [s.riceType || "Tôm", " • ", new Date(s.date).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })] })
                      ]
                    }),
                    c.jsxs("div", {
                      className: "text-right",
                      children: [
                        c.jsxs("p", { className: "font-black text-sm text-green-700 dark:text-green-400 tabular-nums", children: [(s.totalNet || 0).toLocaleString("vi-VN"), " kg"] }),
                        c.jsx("p", { className: "font-bold text-xs text-red-600 dark:text-red-400 tabular-nums", children: fmtVND(s.totalMoney) })
                      ]
                    })
                  ]
                }, s.id))
...
const Mm = ({ weights: S, onWeightsChange: b, page: p, onPageChange: f }) => {
  const [O, D] = w.useState(null);
  const [U, Y] = w.useState("");
  const A = w.useRef(me.getSettings());
  const v = 5, _ = 5;
  const z = w.useRef(Array(v).fill(null).map(() => Array(_).fill(null)));
  const advanceTimerRef = w.useRef(null);
  const lastSpokenRef = w.useRef({ text: "", time: 0 });
  const justAdvancedRef = w.useRef(false);
  ...
  const speakVal = (num) => {
    ...
    if (lastSpokenRef.current.text === key && now - lastSpokenRef.current.time < 1200) {
      return;
    }
    lastSpokenRef.current = { text: key, time: now };
    try {
      const s = me.getSettings ? me.getSettings() : null;
      const isVoiceOn = !s || s.useVoice !== false;
      if (isVoiceOn && Am && Am.speakWeight) {
        Am.speakWeight(num);
      }
    } catch(e) {
  ...
  const L = [0, 1, 2, 3, 4].map(Z => S.reduce((W, de) => W + (de[Z] || 0), 0));
  const te = L.reduce((Z, W) => Z + W, 0);

  const Q = (Z, W, de) => {
    let le = 0;
    const clean = (de || "").toString().replace(/,/g, ".").replace(/[^0-9.]/g, "");
    if (clean === "" || clean === ".") {
      const Ue = S.map((st, $) => $ === Z ? st.map((Te, Pe) => Pe === W ? 0 : Te) : st);
      b(Ue);
      return 0;
    }
    le = parseFloat(clean) || 0;
    le = Math.round(le * 1000) / 1000;
    const _e = S.map((Ue, st) => st === Z ? Ue.map(($, Te) => Te === W ? le : $) : Ue);
    b(_e);
    return le;
  };
  ...
  const se = (Z, W) => {
    ...
    const target = z.current[de] && z.current[de][le];
    if (target) {
      target.focus();
      try {
  ...
  const commitAndAdvance = (r, c, valToCommit) => {
    clearAdvanceTimer();
    const G = Q(r, c, valToCommit);
    if (G > 0) {
  ...
  return c.jsxs("div", {
    className: "flex flex-col bg-[#f0f0f0] dark:bg-gray-800 p-3 rounded-[32px] shadow-lg mb-8 border border-gray-200 dark:border-gray-700 transition-colors",
    children: [
      c.jsxs("div", {
        className: "bg-[#43a047] dark:bg-[#1b4d1e] rounded-t-[20px] px-4 py-3 flex justify-between items-center text-white",
        children: [
          c.jsxs("span", {
            className: "font-black text-2xl italic uppercase tracking-tight",
            children: ["BẢNG ", p]
          }),
          c.jsx("span", {
            className: "font-black text-3xl tabular-nums",
            children: fmtTotal(te)
          })
        ]
      }),
      c.jsx("div", {
        className: "grid grid-cols-5 py-2",
        children: [1, 2, 3, 4, 5].map(Z => c.jsxs("div", {
          key: Z,
          className: "text-center text-[#2e7d32] dark:text-green-400 font-black text-xl italic uppercase",
          children: ["C", Z]
        }))
      }),
      c.jsx("div", {
        className: "grid grid-cols-5 gap-2 px-1",
        children: S.map((Z, W) => Z.map((de, le) => {
          const isFocused = (O == null ? void 0 : O.r) === W && (O == null ? void 0 : O.c) === le;
          const displayVal = isFocused ? U : fmtVal(de);
          return c.jsx("div", {
            key: `${W}-${le}`,
            className: "aspect-[4/3] bg-white dark:bg-gray-900 rounded-xl border border-gray-300 dark:border-gray-700 relative overflow-hidden transition-colors",
            children: c.jsx("input", {
              ref: Ue => {
                if (z.current[W]) z.current[W][le] = Ue;
              },
              type: "text",
              inputMode: "decimal",
              value: displayVal,
              onChange: Ue => Oe(W, le, Ue.target.value),
              onFocus: () => De(W, le, de),
              onBlur: () => J(W, le),
              onKeyDown: Ue => {
                if (Ue.key === "Enter") {
                  ...
                  clearAdvanceTimer();
                  if (U !== "") {
                    commitAndAdvance(W, le, U);
                  } else {
                    se(W, le);
                  }
                } else if (Ue.key === "Backspace" && (U === "" || U === "0")) {
                  clearAdvanceTimer();
                  Q(W, le, "");
                }
              },
              className: `w-full h-full text-center outline-none font-black text-3xl transition-all tabular-nums leading-none caret-transparent ${
                isFocused
                  ? "border-[3px] border-red-500 bg-yellow-50 dark:bg-red-900/20 text-red-600 dark:text-red-400"
                  : de > 0
                    ? "text-[#b71c1c] dark:text-red-500 font-black"
      ...
      c.jsx("div", {
        className: "grid grid-cols-5 gap-1 mt-3 rounded-b-xl overflow-hidden border-t-2 border-gray-200 dark:border-gray-700",
        children: L.map((Z, W) => c.jsx("div", {
          key: W,
          className: "bg-[#fdd835] dark:bg-yellow-700/80 py-3 text-center font-black text-2xl text-gray-900 dark:text-gray-100 tabular-nums border-r border-yellow-600/20 last:border-r-0",
          children: fmtTotal(Z)
        }))
      }),
      c.jsx("div", {
        className: "text-center py-4 bg-white dark:bg-gray-900 rounded-b-[20px] transition-colors",
        children: c.jsx("span", {
          className: "text-6xl font-black text-[#d32f2f] dark:text-red-500 italic tracking-tighter tabular-nums drop-shadow-sm",
          children: fmtTotal(te)
        })
      })
    ]
...
const Om = ({ setRoute: S, role: b }) => {
  const fmtKg = (n) => {
    if (!n || n <= 0) return "0";
    const r = Math.round(n * 100) / 100;
    return Number.isInteger(r) ? String(r) : String(r);
  };
  const p = w.useMemo(() => me.getSettings(), []);
  const f = w.useRef(null);
  const traderProfile = w.useMemo(() => getTraderProfileV3(), []);

  // Is viewing / editing parameters (true) or active weighing (false)
  const [O, D] = w.useState(true);
  const [U, Y] = w.useState(null); // Vehicle

  // Contracts list for quick auto-fill
  const [contractsList, setContractsList] = w.useState([]);
  const [showContractPicker, setShowContractPicker] = w.useState(false);

  // Settlement modal state
  const [showSettlementModal, setShowSettlementModal] = w.useState(false);
  // Dropdown states for Pond Owner & Shrimp Type
  const [showFarmerDropdown, setShowFarmerDropdown] = w.useState(false);
  const [farmerSearch, setFarmerSearch] = w.useState("");
  const [showShrimpDropdown, setShowShrimpDropdown] = w.useState(false);
  // Tare deduction setting: 100kg shrimp tare X kg (default 3.0 kg)
  const [tarePer100Kg, setTarePer100Kg] = w.useState(() => {
    try {
      const stored = localStorage.getItem("temp_weighing_info");
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed && parsed.tarePer100Kg) return Number(parsed.tarePer100Kg);
      }
      const s = me.getSettings();
      return Number(s.tarePer100Kg) || 3.0;
    } catch(e) {
      return 3.0;
    }
  });
  ...
  const [A, v] = w.useState(() => {
    try {
      const stored = localStorage.getItem("temp_weighing_info");
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed && parsed.farmerName === "Chủ lúa mới") parsed.farmerName = "Chủ ao tôm";
        return parsed;
      }
    } catch(e) {}
    const defaultType = (traderProfile.shrimpTypes && traderProfile.shrimpTypes[0]) || { name: "Tôm càng sen", price: 160000 };
    return {
      farmerName: "Chủ ao tôm",
      farmerPhone: "",
      riceType: defaultType.name || "Tôm càng sen",
      price: defaultType.price || 160000,
      deposit: 0,
      tareSpec: "100 con trừ 3 kg",
      contractId: ""
    };
  });

  // Multi-shrimp-type completed batches in current session
  const [completedBatches, setCompletedBatches] = w.useState([]);

  // Active weighing tables for current shrimp type
  const [_, z] = w.useState([Array(5).fill(0).map(() => Array(5).fill(0))]);
  const [L, te] = w.useState(0);  // impurityWeight / Tạp chất
  const [Q, se] = w.useState(false); // isPaidFull (Đã trả / Chưa trả)

  // Load contracts and active vehicle
  w.useEffect(() => {
    try {
      const rawC = localStorage.getItem("tom_electronic_contracts");
      if (rawC) {
        const arr = JSON.parse(rawC);
        if (Array.isArray(arr)) setContractsList(arr);
      }
    } catch(e) {}

    const vId = localStorage.getItem("active_vehicle_id");
    if (vId) {
      const foundV = me.getVehicles().find(et => et.id === vId);
      if (foundV) Y(foundV);
    }
  }, []);

  // Auto-fill when shrimp type changes from predefined types
  const standardShrimpList = w.useMemo(() => {
    if (traderProfile.shrimpTypes && traderProfile.shrimpTypes.length > 0) {
      return traderProfile.shrimpTypes;
    }
    return [
      { name: "Tôm càng sen", price: 160000, desc: "Tôm càng xanh loại 1, thịt chắc" },
      { name: "Tôm càng xào", price: 120000, desc: "Tôm càng size trung, tươi ngon" },
      { name: "Tôm ngộp", price: 95000, desc: "Tôm yếu / ngộp bảo quản đá" },
      { name: "Tôm lột", price: 80000, desc: "Tôm mềm vỏ lột tự nhiên" },
      { name: "Tôm càng đại", price: 220000, desc: "Tôm càng xanh size khủng" },
      { name: "Tôm thẻ chân trắng", price: 105000, desc: "Tôm thẻ công nghiệp" },
      { name: "Tôm sú oxy", price: 210000, desc: "Tôm sú sống oxy chạy hàng" }
    ];
  }, [traderProfile]);

  const allFarmersList = w.useMemo(() => {
    try {
  ...
  const handleUpdateTare = (val) => {
    const num = Number(val) || 3.0;
    setTarePer100Kg(num);
    v(prev => ({ ...prev, tareSpec: `100kg tôm trừ bì ${num} kg`, tarePer100Kg: num }));
    try {
      const s = me.getSettings();
      me.save(Ot.SETTINGS, { ...s, tarePer100Kg: num });
    } catch(e) {}
  };

  const handleSelectShrimpType = (typeName) => {
    const found = traderProfile.shrimpTypes && traderProfile.shrimpTypes.find(t => t.name.toLowerCase() === typeName.toLowerCase());
    if (found && found.price) {
      v(prev => ({ ...prev, riceType: found.name, price: found.price }));
    } else {
      v(prev => ({ ...prev, riceType: typeName }));
    }
  };

  // Select farmer from contract
  const handleSelectContract = (contract) => {
    if (!contract) return;
    v(prev => ({
      ...prev,
      farmerName: contract.farmerName || prev.farmerName,
      farmerPhone: contract.farmerPhone || prev.farmerPhone,
      riceType: contract.shrimpType || contract.shrimpSize || prev.riceType,
      price: Number(contract.agreedPrice) || prev.price,
      deposit: Number(contract.depositAmount) || prev.deposit,
      contractId: contract.id || ""
    }));
    setShowContractPicker(false);
  };

  // Calculations for current active tables
  const Oe = w.useMemo(() => _.flat(2).filter($ => $ > 0), [_]);
  const De = w.useMemo(() => Math.round(Oe.reduce(($, Te) => $ + Te, 0) * 100) / 100, [Oe]);
  const J = w.useMemo(() => Oe.length, [Oe]);
  const Z = w.useMemo(() => { const gross = De; const rate = Number(tarePer100Kg) || 3.0; return Math.round((gross * rate / 100) * 100) / 100; }, [De, tarePer100Kg]);
  const currentNet = w.useMemo(() => Math.max(0, De - Z - L), [De, Z, L]);
  const currentGrossTotal = De;
  const currentMoney = w.useMemo(() => Math.round(currentNet * Number(A.price || 0)), [currentNet, A.price]);

  // Aggregate sums across completed batches + current active batch
  const totalNetAllTypes = w.useMemo(() => {
    const prevNet = completedBatches.reduce((acc, b) => acc + (b.net || 0), 0);
    return Math.round((prevNet + currentNet) * 100) / 100;
  }, [completedBatches, currentNet]);

  const totalGrossAllTypes = w.useMemo(() => {
    const prevGross = completedBatches.reduce((acc, b) => acc + (b.gross || 0), 0);
    return Math.round((prevGross + currentGrossTotal) * 100) / 100;
  }, [completedBatches, currentGrossTotal]);

  const totalBalesAllTypes = w.useMemo(() => {
    const prevBales = completedBatches.reduce((acc, b) => acc + (b.bales || 0), 0);
    return prevBales + J;
  }, [completedBatches, J]);

  const totalMoneyAllTypesRaw = w.useMemo(() => {
    const prevMoney = completedBatches.reduce((acc, b) => acc + (b.money || 0), 0);
    return prevMoney + currentMoney;
  }, [completedBatches, currentMoney]);

  const totalMoneyFinal = w.useMemo(() => {
    if (p.currencyRounding === "1000") return Math.round(totalMoneyAllTypesRaw / 1000) * 1000;
    if (p.currencyRounding === "500") return Math.round(totalMoneyAllTypesRaw / 500) * 500;
    return totalMoneyAllTypesRaw;
  }, [totalMoneyAllTypesRaw, p.currencyRounding]);

  // Remaining payment after deposit
  const remainingPayment = w.useMemo(() => totalMoneyFinal - Number(A.deposit || 0), [totalMoneyFinal, A.deposit]);

  const _e = ($) => Number($ || 0).toLocaleString("vi-VN");

  // Multi-type: finish current shrimp type and start next
  const handleSwitchShrimpType = () => {
    if (currentNet > 0 || J > 0) {
      const newBatch = {
        typeName: A.riceType,
      ...
      // Reset tables for new type
      z([Array(5).fill(0).map(() => Array(5).fill(0))]);
    }
    // Switch to parameter view so user selects new type & price
    D(true);
  };

  // Button in top right: "Cân" (Switch to weigh mode) or "Lưu" (Open Settlement)
  const Ue = () => {
    if (O) {
      D(false);
      setTimeout(() => {
        if (f.current) f.current.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 100);
    } else {
  ...
  // Final Commit & Save Session to me.addSession & mark contract as completed
  const handleConfirmSaveSession = () => {
    // Collect all batches
    const allBatches = [...completedBatches];
    if (currentNet > 0 || J > 0) {
      allBatches.push({
        typeName: A.riceType,
        unitPrice: Number(A.price),
    ...
    const newSession = {
      ...
      riceType: allBatches.length > 1 ? allBatches.map(b => b.typeName).join(", ") : A.riceType,
      vehicleId: U == null ? void 0 : U.id,
      ...
      totalMoney: totalMoneyFinal,
      isCompleted: Q,
      batches: allBatches,
      tareSpec: `100kg tôm trừ bì ${tarePer100Kg} kg`, tareWeight: Z, tarePer100Kg: tarePer100Kg,
      contractId: A.contractId
    };

    me.addSession(newSession);

    // If connected to a contract, mark contract as "Ao đã cân"
    if (A.contractId) {
      try {
        const rawC = localStorage.getItem("tom_electronic_contracts");
        if (rawC) {
          const arr = JSON.parse(rawC);
          const updated = arr.map(c => c.id === A.contractId ? { ...c, status: "COMPLETED", completedAt: new Date().toISOString(), sessionId } : c);
          localStorage.setItem("tom_electronic_contracts", JSON.stringify(updated));
        }
      } catch(e) {}
    }

    localStorage.removeItem("active_vehicle_id");
    localStorage.removeItem("temp_weighing_info");
    setShowSettlementModal(false);
    S(re.DASHBOARD);
  };

  // Quick Zalo Share message
  const handleShareZalo = () => {
    ...
    let msg = `🦐 [PHIẾU CÂN THU MUA TÔM CÀNG XANH]\n`;
    msg += `Chủ ao: ${A.farmerName} ${A.farmerPhone ? "(" + A.farmerPhone + ")" : ""}\n`;
    msg += `Ngày cân: ${new Date().toLocaleDateString("vi-VN")}\n`;
    msg += `-----------------------------\n`;
    allBatches.forEach((b, idx) => {
      msg += `${idx + 1}. ${b.typeName}: ${fmtKg(b.net)} kg x ${Number(b.unitPrice).toLocaleString()} đ = ${Number(b.money).toLocaleString()} đ\n`;
    });
    msg += `-----------------------------\n`;
    msg += `Tổng sản lượng sạch: ${fmtKg(totalNetAllTypes)} kg (${totalBalesAllTypes} rổ)\n`;
    msg += `Tổng thành tiền: ${_e(totalMoneyFinal)} VNĐ\n`;
    if (Number(A.deposit) > 0) {
      msg += `Tiền cọc đã nhận: -${_e(A.deposit)} VNĐ\n`;
      msg += `SỐ TIỀN CÒN LẠI: ${_e(remainingPayment)} VNĐ\n`;
    } else {
      msg += `SỐ TIỀN THANH TOÁN: ${_e(remainingPayment)} VNĐ\n`;
    }
    msg += `(${numberToWordsVN_v3(remainingPayment)})\n`;
    msg += `Trạng thái: ${Q ? "ĐÃ THANH TOÁN ĐỦ 100%" : "CHƯA THANH TOÁN"}\n`;
    msg += `Kính chúc Quý khách vụ mùa bội thu!`;

    const zaloUrl = `https://zalo.me/share?text=${encodeURIComponent(msg)}`;
    window.open(zaloUrl, "_blank");
  };

  return c.jsxs("div", {
    className: "flex flex-col h-screen bg-[#f1f3f4] dark:bg-[#121212] font-roboto select-none overflow-hidden transition-colors",
    children: [
      // Top Header: EXACT PREVIOUS STYLE
      c.jsxs("div", {
        className: "bg-[#1a237e] dark:bg-[#0d134d] px-4 py-3 flex items-center justify-between shadow-xl z-50",
        children: [
          // Back button
          c.jsx("button", {
            onClick: () => {
            ...
            className: "p-2 text-white bg-white/10 rounded-2xl active:scale-90",
            children: c.jsx(On, { size: 24 })
          }),
          // Center Title
          c.jsxs("div", {
            className: "flex flex-col items-center flex-1 mx-3 text-center",
            children: [
              c.jsx("span", {
                className: "font-black text-white text-lg uppercase italic truncate w-full tracking-tighter",
                children: A.farmerName
              }),
              U && c.jsxs("div", {
                className: "text-[9px] font-black text-blue-200 uppercase italic bg-white/10 px-2 py-0.5 rounded-full mt-1",
                children: [c.jsx(tc, { size: 10 }), " ", U.name]
              })
            ]
          }),
          // Right Button: Cân / Lưu
          c.jsxs("button", {
            onClick: Ue,
            className: `h-12 px-5 rounded-2xl flex items-center gap-2 font-black shadow-lg transition-all active:scale-95 border-b-4 ${
              O
                ? "bg-yellow-400 text-blue-900 border-yellow-700"
                : "bg-green-600 text-white border-green-900"
            }`,
            children: [
              O ? c.jsx(mm, { size: 22, fill: "currentColor" }) : c.jsx(ec, { size: 22 }),
              c.jsx("span", {
                className: "uppercase text-xl italic tracking-tighter",
                children: O ? "Cân" : "Lưu"
              })
            ]
          })
      ...
      // Main Scroll Area
      c.jsxs("div", {
        className: "flex-1 overflow-y-auto bg-white dark:bg-[#181818] scroll-smooth pb-20 transition-colors",
        children: [
          // Parameters Panel: EXACT PREVIOUS STYLE + Upgraded Auto-fill
          c.jsxs("div", {
            className: `p-4 space-y-4 transition-all duration-500 ${!O && p.lockParamsDuringWeighing ? "opacity-30 pointer-events-none" : ""}`,
            children: [
              // 2 NÚT BẤM CÓ DANH MỤC THẢ XUỐNG: CHỦ AO & LOẠI TÔM
              c.jsxs("div", {
                className: "grid grid-cols-2 gap-4 relative z-20",
                children: [
                  // Nút 1: Chọn Chủ Ao Tôm (Có Dropdown thả xuống)
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsxs("div", {
                        ...
                        children: [
                          c.jsx("label", {
                            className: "text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase italic",
                            children: "Chủ Ao Tôm"
                          }),
                          c.jsx("button", {
                            type: "button",
                            onClick: () => {
                              const newName = prompt("Nhập tên chủ ao mới hoặc khách lẻ:", A.farmerName || "");
                              if (newName && newName.trim()) {
                                v({ ...A, farmerName: newName.trim() });
                              }
                            },
                            className: "text-[9px] font-black text-blue-600 dark:text-blue-400 uppercase italic underline",
                            children: "Đổi tên"
                          })
                        ]
                      }),
                      ...
                      c.jsxs("button", {
                        type: "button",
                        onClick: () => {
                          setShowFarmerDropdown(!showFarmerDropdown);
                          setShowShrimpDropdown(false);
                        },
                        className: `w-full p-3 rounded-2xl border text-left flex items-center justify-between transition-all active:scale-[0.98] ${
                          showFarmerDropdown
                            ? "bg-blue-50 border-blue-500 dark:bg-blue-950/40 dark:border-blue-400 shadow-md"
                        ...
                        children: [
                          c.jsxs("div", {
                            ...
                            children: [
                              c.jsx("div", {
                                className: "text-base font-black text-gray-800 dark:text-gray-100 uppercase truncate",
                                children: A.farmerName || "Chọn Chủ Ao..."
                              }),
                              A.farmerPhone && c.jsx("div", {
                                className: "text-[10px] font-bold text-gray-400 font-mono",
                                children: A.farmerPhone
                              })
                            ]
                          }),
                          c.jsx("span", {
                            className: `text-xs font-black text-gray-500 transition-transform duration-200 ${showFarmerDropdown ? "rotate-180" : ""}`,
                            children: "▼"
                          })
                        ]
                      }),
                      // Dropdown Menu List
                      showFarmerDropdown && c.jsxs("div", {
                        className: "absolute left-0 right-0 top-full mt-1.5 z-[90] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 max-h-80 overflow-y-auto p-2 space-y-1.5",
                        children: [
                          // Search Box in Dropdown
                          c.jsxs("div", {
                            className: "relative mb-1",
                            children: [
                              c.jsx(Id, { size: 14, className: "absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" }),
                              c.jsx("input", {
                                type: "text",
                                placeholder: "Tìm tên hoặc SĐT chủ ao...",
                                value: farmerSearch,
                                onChange: e => setFarmerSearch(e.target.value),
                                onClick: e => e.stopPropagation(),
                                className: "w-full pl-8 pr-3 py-1.5 bg-gray-50 dark:bg-gray-800 rounded-xl text-xs font-bold outline-none border border-gray-200 dark:border-gray-700"
                              })
                            ]
                          }),
                          // Option 1: Nhập khách lẻ
                          c.jsxs("button", {
                            type: "button",
                            onClick: () => {
                              const customName = prompt("Nhập tên khách lẻ / chủ ao tự do:", "Khách Lẻ");
                              if (customName) {
                                v(prev => ({ ...prev, farmerName: customName, farmerPhone: "", contractId: "" }));
                              }
                              setShowFarmerDropdown(false);
                            },
                            className: "w-full p-2.5 bg-yellow-50 dark:bg-yellow-950/30 hover:bg-yellow-100 rounded-xl text-left flex items-center gap-2 border border-yellow-200 dark:border-yellow-800/40 text-yellow-900 dark:text-yellow-200",
                            children: [
                              c.jsx(As, { size: 14, className: "text-yellow-700" }),
                              c.jsxs("div", {
                                children: [
                                  c.jsx("div", { className: "font-black text-xs uppercase", children: "+ Nhập Khách Lẻ / Tự Do" }),
                                  c.jsx("div", { className: "text-[9px] text-yellow-700 dark:text-yellow-400", children: "Cân nhanh không cần lưu danh bạ" })
                                ]
                              })
                            ]
                          }),
                          // Farmer list
                          allFarmersList.filter(f => {
                            ...
                            onClick: () => {
                              v(prev => ({
                                ...
                                farmerName: f.name,
                                farmerPhone: f.phone || "",
                                riceType: f.shrimpType || prev.riceType,
                                price: f.traderPrice || f.defaultPrice || prev.price,
                                deposit: f.depositMoney || f.defaultDeposit || 0,
                                contractId: f.contractId || "",
                                tareSpec: f.tareSpec || `100kg tôm trừ bì ${tarePer100Kg} kg`
                              }));
                              if (f.tareSpec && f.tareSpec.includes("trừ bì")) {
                                const match = f.tareSpec.match(/(\d+(\.\d+)?)\s*kg/);
                                if (match && match[1]) {
                                  handleUpdateTare(Number(match[1]));
                                }
                              }
                              setShowFarmerDropdown(false);
                            },
                            className: "w-full p-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl text-left border border-transparent hover:border-gray-200 dark:hover:border-gray-700 transition-colors",
                            children: [
                              c.jsxs("div", {
                                className: "flex justify-between items-start",
                                children: [
                                  c.jsx("div", { className: "font-black text-xs uppercase text-gray-800 dark:text-gray-100", children: f.name }),
                                  c.jsx("span", {
                                    className: `text-[9px] font-bold px-1.5 py-0.5 rounded ${f.status === "CONTRACTED" ? "bg-green-100 text-green-800 dark:bg-green-950/60 dark:text-green-300" : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300"}`,
                                    children: f.status === "CONTRACTED" ? "Hợp Đồng" : "Ao Hẹn"
                                  })
                                ]
                              }),
                              c.jsxs("div", {
                                className: "text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-2",
                                children: [
                                  f.phone && c.jsxs("span", { children: ["📞 ", f.phone] }),
                                  f.shrimpType && c.jsxs("span", { className: "text-blue-600 dark:text-blue-400 font-bold", children: ["🦐 ", f.shrimpType] })
                                ]
                              })
                            ]
                          }))
                  ...
                  // Nút 2: Chọn Loại / Size Tôm (Có Dropdown thả xuống)
                  c.jsxs("div", {
                    className: "space-y-1 relative",
                    children: [
                      c.jsxs("div", {
                        ...
                        children: [
                          c.jsx("label", {
                            className: "text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase italic",
                            children: "Loại / Size Tôm"
                          }),
                          c.jsx("button", {
                            type: "button",
                            onClick: () => {
                              const customType = prompt("Nhập tên loại tôm mới:", A.riceType || "");
                              if (customType && customType.trim()) {
                                handleSelectShrimpType(customType.trim());
                              }
                            },
                            className: "text-[9px] font-black text-blue-600 dark:text-blue-400 uppercase italic underline",
                            children: "Khác"
                      ...
                      c.jsxs("button", {
                        type: "button",
                        onClick: () => {
                          setShowShrimpDropdown(!showShrimpDropdown);
                          setShowFarmerDropdown(false);
                        },
                        className: `w-full p-3 rounded-2xl border text-left flex items-center justify-between transition-all active:scale-[0.98] ${
                          showShrimpDropdown
                        ...
                        children: [
                          c.jsxs("div", {
                            ...
                            children: [
                              c.jsxs("div", {
                                className: "text-base font-black text-blue-950 dark:text-blue-200 uppercase truncate flex items-center gap-1.5",
                                children: [
                                  c.jsx("span", { children: "🦐" }),
                                  c.jsx("span", { children: A.riceType || "Chọn Loại Tôm..." })
                                ]
                              }),
                              c.jsxs("div", {
                                className: "text-[10px] font-bold text-gray-400 font-mono",
                                children: [Number(A.price || 0).toLocaleString(), " đ/kg"]
                              })
                            ]
                          }),
                          c.jsx("span", {
                            className: `text-xs font-black text-gray-500 transition-transform duration-200 ${showShrimpDropdown ? "rotate-180" : ""}`,
                            children: "▼"
                          })
                        ]
                      }),
                      // Dropdown Menu List
                      showShrimpDropdown && c.jsxs("div", {
                        className: "absolute left-0 right-0 top-full mt-1.5 z-[90] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 max-h-80 overflow-y-auto p-2 space-y-1 animate-in fade-in",
                        children: [
                          // Shrimp items
                          standardShrimpList.map(item => c.jsxs("button", {
                            key: item.name,
                            type: "button",
                            onClick: () => {
                              v(prev => ({
                            ...
                            className: `w-full p-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl text-left border transition-colors flex items-center justify-between ${
                              (A.riceType || "").toLowerCase() === item.name.toLowerCase()
                            ...
                            children: [
                              c.jsxs("div", {
                                children: [
                                  c.jsx("div", { className: "font-black text-xs uppercase text-gray-800 dark:text-gray-100", children: item.name }),
                                  item.desc && c.jsx("div", { className: "text-[9px] text-gray-400 italic", children: item.desc })
                                ]
                              }),
                              item.price && c.jsxs("div", {
                                className: "text-right shrink-0",
                                children: [
                                  c.jsxs("span", { className: "font-mono font-bold text-xs text-green-700 dark:text-green-400", children: [Number(item.price).toLocaleString(), "đ"] }),
                                  c.jsx("span", { className: "text-[9px] text-gray-400 block", children: "/kg" })
                                ]
                              })
                            ]
                          })),
                          ...
                          c.jsxs("button", {
                            type: "button",
                            onClick: () => {
                              const customName = prompt("Nhập tên loại tôm khác:", "");
                              if (customName && customName.trim()) {
                                const customPrice = prompt("Nhập đơn giá (đ/kg):", "120000");
                                v(prev => ({
                                  ...prev,
                                  riceType: customName.trim(),
                                  price: Number(customPrice) || prev.price
                                }));
                              }
                              setShowShrimpDropdown(false);
                            },
                            className: "w-full p-2 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl text-center text-xs font-bold text-blue-600 dark:text-blue-400 uppercase mt-1",
                            children: "+ Nhập Loại Tôm Khác"
                          })
                        ]
                      })
                    ]
              ...
              // Big Bright Yellow Net Weight Box: EXACT PREVIOUS STYLE
              c.jsxs("div", {
                className: "bg-[#fff176] dark:bg-yellow-700/90 p-6 rounded-[30px] shadow-xl border-b-[6px] border-yellow-600 dark:border-yellow-900 flex justify-between items-center active:translate-y-1 transition-all",
                children: [
                  c.jsxs("div", {
                    children: [
                      c.jsx("span", {
                        className: "text-[11px] font-black text-yellow-900 dark:text-yellow-100 uppercase italic block leading-none mb-1",
                        children: completedBatches.length > 0 ? "Tổng Tôm Sạch (Tất Cả Loại)" : "Tôm Sạch (Net)"
                      }),
                      c.jsxs("span", {
                        className: "text-5xl font-black text-black dark:text-white italic tracking-tighter tabular-nums leading-none",
                        children: [
                          fmtKg(totalNetAllTypes),
                          " ",
                          c.jsx("span", { className: "text-xl", children: "KG" })
                        ]
                      }),
                      // Multi-type breakdown preview if more than one batch
                      completedBatches.length > 0 && c.jsxs("div", {
                        className: "mt-2 flex flex-wrap gap-1.5",
                        children: [
                          completedBatches.map((b, i) => c.jsxs("span", {
                            key: i,
                            className: "text-[10px] font-black bg-black/10 dark:bg-white/10 px-2 py-0.5 rounded-full text-black dark:text-white",
                            children: [b.typeName, ": ", fmtKg(b.net), " kg"]
                          })),
                          currentNet > 0 && c.jsxs("span", {
                            className: "text-[10px] font-black bg-green-800 text-white px-2 py-0.5 rounded-full",
                            children: [A.riceType, " (đang cân): ", fmtKg(currentNet), " kg"]
                          })
                        ]
                      })
                    ]
                  }),
                  c.jsx(Ds, { size: 70, className: "opacity-10 text-yellow-900 dark:text-white rotate-12" })
                ]
              }),

              // Two Cards: Giá (kg) & Cọc (VNĐ): EXACT PREVIOUS STYLE
              c.jsxs("div", {
                className: "space-y-3",
                children: [
                  c.jsxs("div", {
                      ...
                      c.jsxs("div", {
                        ...
                        children: [
                          ...
                          c.jsx("input", {
                            type: "number",
                            value: A.price,
                            onChange: $ => v({ ...A, price: $.target.value }),
                            className: "w-full text-right text-2xl font-black text-[#1a237e] dark:text-blue-300 outline-none bg-transparent"
                          })
                        ]
                      }),
                      // Cọc (VNĐ)
                      c.jsxs("div", {
                        ...
                        children: [
                          ...
                          c.jsx("input", {
                            type: "number",
                            value: A.deposit,
                            onChange: $ => v({ ...A, deposit: $.target.value }),
                            className: "w-full text-right text-2xl font-black text-purple-700 dark:text-purple-300 outline-none bg-transparent"
                          })
                        ]
                      })
                    ]
                  ...
                  // Black Payment Card: EXACT PREVIOUS STYLE
                  c.jsxs("div", {
                    className: "bg-[#1e1e1e] dark:bg-black text-white p-6 rounded-[35px] shadow-2xl flex flex-col gap-1 border-b-8 border-black active:translate-y-1 transition-all",
                    children: [
                      c.jsxs("div", {
                        className: "flex justify-between items-center",
                        children: [
                          c.jsx("span", {
                            className: "text-[11px] font-black text-yellow-400 uppercase italic tracking-widest",
                            children: "Thanh Toán"
                          }),
                          c.jsx("div", {
                            className: `px-4 py-1.5 rounded-xl font-black text-[11px] uppercase cursor-pointer ${
                              Q ? "bg-green-600" : "bg-gray-700 text-gray-400"
                            }`,
                            onClick: () => se(!Q),
                            children: Q ? "Đã Trả" : "Chưa Trả"
                          })
                        ]
                      }),
                      c.jsxs("span", {
                        className: "text-4xl font-black text-red-500 dark:text-red-400 tabular-nums leading-none tracking-tighter",
                        children: [
                          _e(remainingPayment),
                          " ",
                          c.jsx("span", { className: "text-sm font-bold opacity-30 italic", children: "VNĐ" })
                        ]
                      }),
                      c.jsx("p", {
                        className: "text-[11px] text-gray-400 italic font-semibold mt-1",
                        children: numberToWordsVN_v3(remainingPayment)
                      })
                    ]
                  })
                ]
          ...
          // Weighing Tables Area: EXACT PREVIOUS STYLE
          c.jsxs("div", {
            ref: f,
            className: `px-3 transition-all duration-700 ${O ? "opacity-20 pointer-events-none translate-y-10" : "translate-y-0"}`,
            children: [
              // Badge indicating active shrimp type being weighed
              c.jsxs("div", {
                ...
                children: [
                  c.jsxs("div", {
                    ...
                    children: [
                      ...
                      c.jsxs("span", {
                        className: "text-xs font-black uppercase text-gray-700 dark:text-gray-300 italic",
                        children: ["Đang cân: ", A.riceType, " (", Number(A.price).toLocaleString(), " đ/kg)"]
                      })
                    ]
                  }),
                  // Multi-type action: Chuyển loại tôm
                  c.jsxs("button", {
                    onClick: handleSwitchShrimpType,
                    className: "px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black uppercase italic shadow flex items-center gap-1.5 active:scale-95 transition-all",
                    children: [
                      c.jsx(As, { size: 14 }),
                      "Cân Loại Khác"
                    ]
              ...
              _.map(($, Te) => c.jsx(Mm, {
                ...
                onPageChange: () => {
                  if (Te === _.length - 1) {
                    z([..._, Array(5).fill(0).map(() => Array(5).fill(0))]);
                  }
                }
              }, Te)),

              // Big Dashed "Sửa Thông Số" Button: EXACT PREVIOUS STYLE
              !O && c.jsxs("div", {
                className: "pb-40 pt-10 flex flex-col items-center gap-6",
                children: [
                  c.jsxs("button", {
                    onClick: () => D(true),
                    className: "w-full py-10 text-gray-400 dark:text-gray-500 font-black uppercase text-sm italic border-4 border-dashed border-gray-300 dark:border-gray-700 rounded-[40px] flex flex-col items-center gap-3 active:bg-gray-50 dark:active:bg-gray-800 transition-colors",
                    children: [
                      c.jsx(tm, { size: 40 }),
                      "Sửa Thông Số"
                    ]
                  ...
                  c.jsxs("button", {
                    onClick: handleSwitchShrimpType,
                    className: "w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-black uppercase text-base italic rounded-[28px] shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all",
                    children: [
                      c.jsx(As, { size: 24 }),
                      "Cân Tiếp Loại Tôm Tiếp Theo"
                    ]
                  })
                ]
              })
      ...
      // Contract Picker Modal (if open)
      showContractPicker && c.jsx("div", {
        className: "fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm",
        children: c.jsxs("div", {
          className: "bg-white dark:bg-gray-900 w-full max-w-md rounded-[32px] p-6 shadow-2xl space-y-4 border border-gray-200 dark:border-gray-800 max-h-[85vh] flex flex-col",
          children: [
            c.jsxs("div", {
              className: "flex justify-between items-center border-b pb-3",
              children: [
                c.jsx("h3", {
                  className: "text-lg font-black uppercase text-gray-800 dark:text-gray-100 italic tracking-tight",
                  children: "Chọn Ao Hẹn / Hợp Đồng"
                }),
                c.jsx("button", {
            ...
            c.jsx("div", {
              ...
              children: contractsList.length === 0 ? c.jsx("p", {
                className: "text-center text-gray-400 py-8 italic",
                children: "Chưa có danh sách hợp đồng ao tôm."
              }) : contractsList.map(contract => c.jsxs("div", {
                key: contract.id,
                onClick: () => handleSelectContract(contract),
                className: "p-4 rounded-2xl border border-gray-200 dark:border-gray-700 hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-950/20 cursor-pointer transition-all space-y-1.5",
                children: [
                  c.jsxs("div", {
                    className: "flex justify-between items-center",
                    children: [
                      c.jsx("span", {
                        className: "font-black text-gray-900 dark:text-gray-100 uppercase",
                        children: contract.farmerName
                      }),
                      c.jsx("span", {
                        className: "text-xs font-bold text-blue-600 dark:text-blue-400",
                        children: contract.weighingDate || "Chưa định ngày"
                      })
                    ]
                  }),
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsxs("span", { children: ["Loại: ", contract.shrimpType || contract.shrimpSize || "---"] }),
                      c.jsxs("span", { children: ["Giá: ", Number(contract.agreedPrice || 0).toLocaleString(), " đ/kg"] }),
                      c.jsxs("span", { children: ["Cọc: ", Number(contract.depositAmount || 0).toLocaleString(), " đ"] })
                    ]
                  })
                ]
              }))
      ...
      // Settlement Modal (on click "Lưu" / Hoàn thành)
      showSettlementModal && c.jsx("div", {
        className: "fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm",
        children: c.jsxs("div", {
          className: "bg-white dark:bg-gray-900 w-full max-w-lg rounded-[35px] p-6 shadow-2xl space-y-5 border border-gray-200 dark:border-gray-800 max-h-[90vh] flex flex-col",
          children: [
            // Modal Header
            c.jsxs("div", {
              className: "flex justify-between items-center border-b pb-3",
                ...
                c.jsxs("div", {
                  children: [
                    c.jsx("h3", {
                      ...
                      children: "Quyết Toán Phiếu Cân Tôm"
                    }),
                    c.jsx("p", {
                      className: "text-xs text-gray-500 font-bold",
                      children: `${A.farmerName} - ${new Date().toLocaleDateString("vi-VN")}`
                    })
                  ]
                }),
                c.jsx("button", {
              ...
              children: [
                ...
                c.jsx("div", {
                  ...
                  children: c.jsxs("table", {
                    ...
                    children: [
                      c.jsx("thead", {
                        className: "bg-gray-100 dark:bg-gray-800 text-xs font-black text-gray-500 uppercase",
                        children: c.jsxs("tr", {
                          children: [
                            c.jsx("th", { className: "p-2.5", children: "Loại tôm" }),
                            c.jsx("th", { className: "p-2.5 text-right", children: "Sạch (kg)" }),
                            c.jsx("th", { className: "p-2.5 text-right", children: "Giá" }),
                            c.jsx("th", { className: "p-2.5 text-right", children: "Thành tiền" })
                          ]
                        })
                      }),
                      c.jsxs("tbody", {
                        className: "divide-y divide-gray-100 dark:divide-gray-800 font-bold",
                        children: [
                          completedBatches.map((b, i) => c.jsxs("tr", {
                            key: i,
                            children: [
                              c.jsxs("td", { className: "p-2.5", children: [b.typeName, " (", b.bales, " rổ)"] }),
                              c.jsx("td", { className: "p-2.5 text-right text-green-700 dark:text-green-400 font-black", children: fmtKg(b.net) }),
                              c.jsx("td", { className: "p-2.5 text-right", children: Number(b.unitPrice).toLocaleString() }),
                              c.jsxs("td", { className: "p-2.5 text-right text-red-600 font-black", children: [_e(b.money), " đ"] })
                            ]
                          })),
                          (currentNet > 0 || completedBatches.length === 0) && c.jsxs("tr", {
                            children: [
                              c.jsxs("td", { className: "p-2.5", children: [A.riceType, " (", J, " rổ)"] }),
                              c.jsx("td", { className: "p-2.5 text-right text-green-700 dark:text-green-400 font-black", children: fmtKg(currentNet) }),
                              c.jsx("td", { className: "p-2.5 text-right", children: Number(A.price).toLocaleString() }),
                              c.jsxs("td", { className: "p-2.5 text-right text-red-600 font-black", children: [_e(currentMoney), " đ"] })
                            ]
                          })
                        ]
                      })
                ...
                // Summary Numbers
                c.jsxs("div", {
                  className: "p-4 bg-gray-50 dark:bg-gray-800/60 rounded-2xl space-y-2 border border-gray-100 dark:border-gray-800",
                  children: [
                    c.jsxs("div", {
                      className: "flex justify-between",
                      children: [
                        c.jsx("span", { className: "text-gray-500 font-bold", children: "Tổng sản lượng tôm sạch:" }),
                        c.jsxs("span", { className: "font-black text-gray-900 dark:text-gray-100", children: [fmtKg(totalNetAllTypes), " kg (", totalBalesAllTypes, " rổ)"] })
                      ]
                    }),
                    c.jsxs("div", {
                      className: "flex justify-between",
                      children: [
                        c.jsx("span", { className: "text-gray-500 font-bold", children: "Tổng tiền hàng:" }),
                        c.jsxs("span", { className: "font-black text-gray-900 dark:text-gray-100", children: [_e(totalMoneyFinal), " VNĐ"] })
                      ]
                    }),
                    Number(A.deposit) > 0 && c.jsxs("div", {
                      className: "flex justify-between text-purple-600 dark:text-purple-400",
                      children: [
                        c.jsx("span", { className: "font-bold", children: "Trừ tiền cọc đã đưa:" }),
                        c.jsxs("span", { className: "font-black", children: ["-", _e(A.deposit), " VNĐ"] })
                      ]
                    }),
                    c.jsxs("div", {
                      className: "flex justify-between items-baseline pt-2 border-t border-gray-200 dark:border-gray-700",
                      children: [
                        c.jsx("span", { className: "font-black uppercase text-sm text-gray-800 dark:text-gray-200", children: "Còn phải thanh toán:" }),
                        c.jsxs("span", { className: "text-2xl font-black text-red-600", children: [_e(remainingPayment), " VNĐ"] })
                      ]
                    }),
                    c.jsx("p", {
                      className: "text-xs italic text-gray-500 text-right font-bold pt-1",
                      children: numberToWordsVN_v3(remainingPayment)
                    })
                  ]
                }),
                ...
                c.jsxs("div", {
                  onClick: () => se(!Q),
                  className: `p-3 rounded-2xl flex items-center justify-between cursor-pointer border ${
                    Q
                      ? "bg-green-50 border-green-300 text-green-800 dark:bg-green-950/30 dark:border-green-800 dark:text-green-300"
                    ...
                    c.jsxs("div", {
                      ...
                      children: [
                        ...
                        c.jsx("span", {
                          ...
                          children: Q ? "Đã thanh toán đủ 100% cho chủ ao" : "Chưa thanh toán (ghi nợ)"
                        })
                      ]
                    }),
                    c.jsx("span", {
            ...
            c.jsxs("div", {
              ...
              children: [
                c.jsxs("div", {
                  ...
                  children: [
                    c.jsxs("button", {
                      ...
                      className: "py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-2xl flex items-center justify-center gap-2 text-xs uppercase italic active:scale-95 transition-all",
                      children: [
                        c.jsx(xm, { size: 18 }),
                        "Gửi Zalo"
                      ]
                    ...
                    c.jsxs("button", {
                      ...
                      className: "py-3 px-4 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-black rounded-2xl flex items-center justify-center gap-2 text-xs uppercase italic active:scale-95 transition-all",
                      children: [
                        c.jsx(Ms, { size: 18 }),
                        "In Phiếu"
                      ]
                ...
                c.jsxs("button", {
                  ...
                  className: "w-full py-4 bg-green-600 hover:bg-green-700 text-white font-black text-base uppercase italic tracking-tight rounded-2xl shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all",
                  children: [
                    c.jsx(ec, { size: 22 }),
                    "Xác Nhận & Hoàn Tất Buổi Cân"
                  ]
                })
              ]
            })
...
// ============================================================================
// LỊCH SỬ THU MUA & CHI TIẾT PHIẾU CÂN (HOÀN THIỆN & SỬA LỖI TOÀN DIỆN)
// ============================================================================
const _m = ({ setRoute: S, onViewDetail: b }) => {
  const [p, f] = w.useState([]);
  const [O, D] = w.useState("");

  w.useEffect(() => {
    try {
      const list = me.getSessions();
      f(Array.isArray(list) ? list : []);
    } catch(e) {
      f([]);
    }
  }, []);
  ...
  const U = (sessionId, e) => {
    e?.stopPropagation?.();
    if (confirm("Bạn có chắc chắn muốn xóa phiếu cân này khỏi lịch sử?")) {
  ...
  const Y = (p || []).filter(A => {
    const q = (O || "").toLowerCase().trim();
    if (!q) return true;
    const name = (A?.farmerName || "").toLowerCase();
    const type = (A?.riceType || "").toLowerCase();
    const phone = (A?.farmerPhone || "").toLowerCase();
    return name.includes(q) || type.includes(q) || phone.includes(q);
  });

  return c.jsxs("div", {
    className: "flex flex-col h-full bg-[#f8f9fa] dark:bg-[#121212] font-roboto",
    children: [
      // Header
      c.jsxs("div", {
        className: "p-4 flex items-center justify-between bg-white dark:bg-gray-900 shadow-sm border-b dark:border-gray-800 sticky top-0 z-30",
        children: [
          c.jsxs("div", {
            className: "flex items-center gap-3",
            children: [
              c.jsx("button", {
                onClick: () => S?.(re.DASHBOARD),
                className: "p-2 bg-gray-100 dark:bg-gray-800 rounded-xl text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 active:scale-95 transition-all",
                children: c.jsx(On, { size: 24 })
              }),
              c.jsxs("div", {
                children: [
                  c.jsx("h2", { className: "text-lg font-black uppercase italic text-gray-800 dark:text-gray-100 tracking-tight", children: "Lịch Sử Thu Mua Tôm" }),
                  c.jsx("p", { className: "text-[10px] text-gray-400 font-bold uppercase", children: `Tổng cộng ${p.length} phiếu cân đã lưu` })
                ]
              })
            ]
          }),
          c.jsxs("button", {
            onClick: () => S?.(re.WEIGHING),
            className: "bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-xl font-black text-xs uppercase flex items-center gap-1.5 shadow-md active:scale-95",
            children: [c.jsx(As, { size: 16 }), "Cân Mới"]
          })
        ]
      }),
      // Content
      c.jsxs("div", {
        className: "p-4 space-y-4 pb-28 overflow-auto max-w-4xl mx-auto w-full",
        children: [
          // Search box
          c.jsxs("div", {
            className: "relative",
            children: [
              c.jsx(Id, { className: "absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400", size: 18 }),
              c.jsx("input", {
                type: "text",
                placeholder: "Tìm kiếm theo tên chủ ao, số điện thoại hoặc loại tôm...",
                className: "w-full pl-11 pr-4 py-3 bg-white dark:bg-gray-900 dark:text-white rounded-2xl shadow-sm outline-none font-bold text-xs border border-gray-100 dark:border-gray-800 focus:border-green-500",
                value: O,
          ...
          c.jsx("div", {
            className: "space-y-3",
            children: Y.length > 0 ? Y.map(A => {
              const netVal = Number(A?.totalNet || 0);
              const priceVal = Number(A?.unitPrice || A?.price || 0);
              const moneyVal = Number(A?.totalMoney || 0);
              const dateStr = A?.date ? new Date(A.date).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "Gần đây";
              return c.jsxs("div", {
                onClick: () => b?.(A.id),
                className: "bg-white dark:bg-gray-900 rounded-[24px] shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden active:scale-[0.99] transition-all cursor-pointer hover:border-green-300 dark:hover:border-green-800",
                children: [
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsxs("div", {
                        ...
                        children: [
                          c.jsx("div", {
                            className: "w-12 h-12 bg-green-50 dark:bg-green-950/40 rounded-2xl flex items-center justify-center text-green-600 dark:text-green-400 shrink-0 text-xl font-black",
                            children: "🦐"
                          }),
                          c.jsxs("div", {
                            children: [
                              c.jsx("h4", {
                                className: "font-black text-gray-800 dark:text-gray-100 text-base uppercase italic leading-tight",
                                children: A?.farmerName || "Khách Lẻ"
                              }),
                              c.jsxs("div", {
                                className: "flex items-center gap-2 mt-1",
                                children: [
                                  c.jsx("span", {
                                    className: "text-[10px] font-black uppercase text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded-md",
                                    children: A?.riceType || "Tôm Càng Sen"
                                  }),
                                  c.jsxs("span", {
                                    className: "text-[10px] text-gray-400 font-bold flex items-center gap-1",
                                    children: [c.jsx(P1, { size: 10 }), dateStr]
                                  })
                                ]
                              })
                            ]
                      ...
                      c.jsxs("div", {
                        ...
                        children: [
                          c.jsx("span", {
                            className: `text-[10px] font-black uppercase px-2 py-1 rounded-lg ${A?.isPaidFull ? "bg-green-100 text-green-800 dark:bg-green-950/60 dark:text-green-300" : "bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300"}`,
                            children: A?.isPaidFull ? "Đã Trả Đủ" : "Còn Lại"
                          }),
                          c.jsx("button", {
                            onClick: (e) => U(A.id, e),
                            className: "p-2 text-gray-300 hover:text-red-600 dark:hover:text-red-400 rounded-lg transition-colors",
                            title: "Xóa phiếu cân",
                  ...
                  // Metrics Bar
                  c.jsxs("div", {
                    className: "bg-gray-50 dark:bg-gray-800/50 px-4 py-3 grid grid-cols-3 divide-x divide-gray-200 dark:divide-gray-700 border-t dark:border-gray-800",
                    children: [
                      c.jsxs("div", {
                        className: "text-center pr-2",
                        children: [
                          c.jsx("div", { className: "text-[9px] font-bold text-gray-400 uppercase", children: "Sản Lượng (Net)" }),
                          c.jsxs("div", { className: "font-black text-green-700 dark:text-green-400 text-sm", children: [netVal.toLocaleString("vi-VN", { maximumFractionDigits: 1 }), " kg"] })
                        ]
                      }),
                      c.jsxs("div", {
                        className: "text-center px-2",
                        children: [
                          c.jsx("div", { className: "text-[9px] font-bold text-gray-400 uppercase", children: "Giá Cân" }),
                          c.jsxs("div", { className: "font-black text-gray-800 dark:text-gray-100 text-sm", children: [priceVal.toLocaleString("vi-VN"), " đ"] })
                        ]
                      }),
                      c.jsxs("div", {
                        className: "text-center pl-2",
                        children: [
                          c.jsx("div", { className: "text-[9px] font-bold text-gray-400 uppercase", children: "Thành Tiền" }),
                          c.jsxs("div", { className: "font-black text-red-600 dark:text-red-400 text-sm", children: [moneyVal.toLocaleString("vi-VN"), " đ"] })
                        ]
                      })
                    ]
                  })
              ...
              className: "py-20 text-center opacity-40 flex flex-col items-center dark:text-gray-500",
              children: [
                c.jsx("span", { className: "text-5xl mb-2", children: "📋" }),
                c.jsx("p", { className: "font-black italic uppercase text-sm", children: "Chưa có phiếu cân nào trong lịch sử" })
              ]
            })
          })
        ]
...
const Dm = ({ setRoute: S, sessionId: b, role: p }) => {
  const O = w.useMemo(() => {
    try {
      const list = me.getSessions();
  ...
  const handleShareZalo = () => {
    if (!O) return;
    let msg = `🦐 [PHIẾU CÂN THU MUA TÔM CÀNG XANH]\n`;
    msg += `Chủ ao: ${O.farmerName || "Khách lẻ"} ${O.farmerPhone ? "(" + O.farmerPhone + ")" : ""}\n`;
    msg += `Ngày cân: ${O.date ? new Date(O.date).toLocaleDateString("vi-VN") : ""}\n`;
    msg += `--------------------------------\n`;
    if (O.batches && O.batches.length > 0) {
      O.batches.forEach((bt, idx) => {
        msg += `${idx + 1}. ${bt.typeName || "Tôm"}: ${bt.net} kg x ${(bt.unitPrice || 0).toLocaleString()}đ = ${(bt.money || 0).toLocaleString()}đ\n`;
      });
    } else {
      msg += `Loại tôm: ${O.riceType || "Tôm càng sen"}\n`;
      msg += `Đơn giá: ${(O.unitPrice || O.price || 0).toLocaleString()} đ/kg\n`;
    }
    msg += `--------------------------------\n`;
    msg += `Tổng cân gộp (Gross): ${(O.totalGross || 0).toLocaleString()} kg\n`;
    if (O.tareSpec) {
      msg += `Quy cách trừ bì: ${O.tareSpec}\n`;
    }
    msg += `TỔNG TÔM SẠCH (NET): ${(O.totalNet || 0).toLocaleString()} kg\n`;
    msg += `TỔNG THÀNH TIỀN: ${(O.totalMoney || 0).toLocaleString()} VNĐ\n`;
    if (O.deposit && Number(O.deposit) > 0) {
      msg += `Tiền cọc đã nhận: -${Number(O.deposit).toLocaleString()} VNĐ\n`;
      msg += `TIỀN CÒN LẠI: ${Math.max(0, (O.totalMoney || 0) - Number(O.deposit)).toLocaleString()} VNĐ\n`;
    }
    if (typeof numberToWordsVN_v3 === "function") {
      msg += `(Bằng chữ: ${numberToWordsVN_v3(O.totalMoney || 0)})\n`;
    }
    msg += `Trạng thái: ${O.isPaidFull ? "Đã thanh toán đủ" : "Chưa thanh toán đủ"}\n`;

    try {
      navigator.clipboard.writeText(msg);
      alert("Đã sao chép nội dung phiếu cân! Đang mở ứng dụng Zalo...");
      window.open("https://zalo.me", "_blank");
    } catch(e) {
      alert("Nội dung phiếu cân:\n\n" + msg);
    }
  };

  if (!O) {
    return c.jsxs("div", {
      className: "flex flex-col h-full bg-[#f8f9fa] dark:bg-[#121212] font-roboto p-6 items-center justify-center text-center",
      children: [
        c.jsx("span", { className: "text-5xl mb-3", children: "🔍" }),
        c.jsx("h3", { className: "text-lg font-black uppercase text-gray-700 dark:text-gray-200", children: "Không tìm thấy phiếu cân" }),
        c.jsx("p", { className: "text-xs text-gray-400 mt-1 mb-4", children: "Phiếu cân có thể đã bị xóa hoặc không tồn tại." }),
        c.jsx("button", {
          onClick: () => S?.(re.HISTORY),
          className: "px-4 py-2 bg-green-600 text-white rounded-xl font-bold text-xs uppercase shadow-md",
          children: "Quay lại lịch sử"
        })
      ]
    });
  }

  const netKg = Number(O.totalNet || 0);
  const grossKg = Number(O.totalGross || 0);
  const moneyVal = Number(O.totalMoney || 0);
  const depositVal = Number(O.deposit || 0);
  const remainVal = Math.max(0, moneyVal - depositVal);
  const dateFormatted = O.date ? new Date(O.date).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "Gần đây";
  ...
  return c.jsxs("div", {
    ...
    children: [
      // Top Navigation Bar
      c.jsxs("div", {
        className: "p-4 bg-[#1a237e] text-white shadow-md flex items-center justify-between sticky top-0 z-40",
        children: [
          c.jsxs("div", {
            className: "flex items-center gap-3",
            children: [
              ...
              c.jsxs("div", {
                children: [
                  c.jsx("h2", { className: "text-base font-black uppercase italic tracking-tight", children: "Chi Tiết Phiếu Cân" }),
                  c.jsx("p", { className: "text-[10px] text-blue-200 font-mono font-bold", children: `MÃ: #${(O.id || "").slice(-8)}` })
                ]
              })
            ]
          }),
          c.jsxs("div", {
            ...
            children: [
              c.jsx("button", {
                onClick: handleShareZalo,
                className: "px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black uppercase flex items-center gap-1 shadow-md active:scale-95",
                children: "Zalo"
              }),
              c.jsx("button", {
                onClick: handlePrint,
                className: "p-2 bg-green-600 hover:bg-green-700 text-white rounded-xl shadow-md active:scale-95",
                title: "In phiếu cân",
                children: c.jsx(Ms, { size: 18 })
              })
            ]
          })
        ]
      ...
      // Main Body
      c.jsxs("div", {
        className: "p-4 max-w-3xl mx-auto w-full space-y-4",
        children: [
          // Farmer Card
          c.jsxs("div", {
            ...
            children: [
              c.jsxs("div", {
                ...
                children: [
                  c.jsxs("div", {
                    children: [
                      c.jsx("span", { className: "text-[10px] font-black text-gray-400 uppercase italic", children: "Chủ ao tôm" }),
                      c.jsx("h1", { className: "text-2xl font-black text-gray-800 dark:text-gray-100 uppercase italic", children: O.farmerName || "Khách Lẻ" }),
                      O.farmerPhone && c.jsx("p", { className: "text-xs font-bold text-gray-500 dark:text-gray-400 mt-0.5", children: `SĐT: ${O.farmerPhone}` })
                    ]
                  }),
                  c.jsx("span", {
                    className: `text-xs font-black uppercase px-3 py-1.5 rounded-xl ${O.isPaidFull ? "bg-green-100 text-green-800 dark:bg-green-950/60 dark:text-green-300" : "bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300"}`,
                    children: O.isPaidFull ? "Đã Trả Đủ" : "Còn Nợ"
                  })
                ]
              }),
              c.jsxs("div", {
                className: "flex items-center gap-4 text-xs font-bold text-gray-400 border-t dark:border-gray-800 pt-3",
                children: [
                  c.jsxs("span", { children: ["📅 Ngày cân: ", dateFormatted] }),
                  O.tareSpec && c.jsxs("span", { children: ["⚖️ Trừ bì: ", O.tareSpec] })
                ]
              })
            ]
          }),
          ...
          c.jsxs("div", {
            className: "bg-[#1e1e1e] text-white rounded-[28px] p-6 shadow-xl border border-gray-800 space-y-4",
            children: [
              c.jsxs("div", {
                className: "grid grid-cols-2 gap-4 border-b border-gray-800 pb-4",
                children: [
                  c.jsxs("div", {
                    children: [
                      c.jsx("span", { className: "text-[10px] font-bold text-gray-400 uppercase tracking-wider block", children: "Cân Gộp (Gross)" }),
                      c.jsxs("span", { className: "text-2xl font-black text-yellow-400", children: [grossKg.toLocaleString("vi-VN", { maximumFractionDigits: 1 }), " kg"] })
                    ]
                  }),
                  c.jsxs("div", {
                    children: [
                      c.jsx("span", { className: "text-[10px] font-bold text-gray-400 uppercase tracking-wider block", children: "Tôm Sạch (Net)" }),
                      c.jsxs("span", { className: "text-3xl font-black text-green-400", children: [netKg.toLocaleString("vi-VN", { maximumFractionDigits: 1 }), " kg"] })
                    ]
                  })
                ]
              }),
              c.jsxs("div", {
                ...
                children: [
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsx("span", { children: "Tổng thành tiền tôm:" }),
                      c.jsxs("span", { className: "font-mono font-bold text-base text-white", children: [moneyVal.toLocaleString("vi-VN"), " đ"] })
                    ]
                  }),
                  depositVal > 0 && c.jsxs("div", {
                    className: "flex justify-between items-center text-amber-400",
                    children: [
                      c.jsx("span", { children: "Tiền cọc đã nhận:" }),
                      c.jsxs("span", { className: "font-mono font-bold", children: ["-", depositVal.toLocaleString("vi-VN"), " đ"] })
                    ]
                  }),
                  c.jsxs("div", {
                    className: "flex justify-between items-center text-red-400 border-t border-gray-800 pt-2",
                    children: [
                      c.jsx("span", { className: "font-black uppercase text-xs", children: "Số tiền thực trả:" }),
                      c.jsxs("span", { className: "font-mono font-black text-2xl text-red-500", children: [remainVal.toLocaleString("vi-VN"), " đ"] })
                    ]
                  })
                ]
              }),
              typeof numberToWordsVN_v3 === "function" && c.jsxs("div", {
                className: "bg-black/40 p-3 rounded-xl text-[11px] text-gray-300 italic border border-gray-800",
                children: [
                  c.jsx("span", { className: "font-bold text-yellow-400", children: "Bằng chữ: " }),
                  numberToWordsVN_v3(moneyVal)
                ]
              })
            ]
          }),
...
if (typeof window !== "undefined") {
  window.encodeContractToB64 = encodeContractToB64;
  window.decodeContractFromB64 = decodeContractFromB64;
}
...
const encodeContractForShare = (contract) => {
  ...
  try {
    ...
    if (b64) {
      return `${base}?contract=${encodeURIComponent(cleanId)}&d=${encodeURIComponent(b64)}#c=${encodeURIComponent(b64)}`;
    }
  } catch (e) {}
  return `${base}?contract=${encodeURIComponent(cleanId)}`;
};
if (typeof window !== "undefined") {
...
const Xm_StandaloneContractPortal = ({ contractId }) => {
  ...
  w.useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!found && contractId && contractId !== "hash_portal" && contractId !== "portal" && typeof loadContractFromCloud === "function") {
        try {
          found = await loadContractFromCloud(contractId);
        } catch (e) {}
      }

      // 5. Fallback: try fetching contracts from API Server or LocalStorage
      if (!found) {
        try {
          ...
          if (res.ok) {
            ...
            if (Array.isArray(list) && list.length > 0) {
              if (contractId && contractId !== "portal" && contractId !== "latest" && contractId !== "hash_portal") {
                found = list.find(x => x && (x.id === contractId || x.id === contractId.replace(/Đ/g, "D")));
              }
      ...
      if (!found && typeof getContracts === "function") {
        const all = getContracts();
        if (all && all.length > 0) {
          if (contractId && contractId !== "portal" && contractId !== "latest" && contractId !== "hash_portal") {
            found = all.find(x => x && (x.id === contractId || x.id === contractId.replace(/Đ/g, "D")));
          }
          if (!found) found = all[all.length - 1];
        }
      }
  ...
  // Đăng nhập bằng tài khoản Google / Gmail THẬT (Firebase Auth Popup + Graceful Google Verification)
  ...
  // Hỗ trợ xem trực tiếp mượt mà trên iPhone & Android (Không chặn màn hình)
  if (!customerUser && false) {
    return c.jsxs("div", {
      className: "min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden",
      children: [
        c.jsx("div", { className: "absolute -top-40 -left-40 w-96 h-96 bg-emerald-600/20 rounded-full blur-3xl pointer-events-none" }),
        c.jsx("div", { className: "absolute -bottom-40 -right-40 w-96 h-96 bg-blue-600/20 rounded-full blur-3xl pointer-events-none" }),
        c.jsxs("div", {
          className: "w-full max-w-md bg-slate-800/90 backdrop-blur-xl border border-slate-700/80 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 relative z-10",
          children: [
            c.jsxs("div", {
              className: "text-center space-y-2",
              children: [
                c.jsxs("div", {
                  className: "inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/10 border border-emerald-500/30 rounded-full text-emerald-400 font-bold text-xs uppercase tracking-wider",
                  children: [c.jsx("span", { children: "🦐" }), "Vua Tôm Càng Xanh 4.0"]
                }),
                c.jsx("h2", {
                  className: "text-xl sm:text-2xl font-black text-white tracking-tight",
                  children: "Cổng Hợp Đồng Mua Bán Điện Tử"
                }),
                c.jsxs("p", {
            ...
            c.jsxs("div", {
              ...
              children: [
                c.jsxs("div", {
                  ...
                  children: [
                    c.jsx("span", { className: "text-slate-500", children: "Bên thu mua:" }),
                    c.jsx("span", { className: "font-bold text-white", children: contract?.trader?.fullName || contract?.traderName || "Cơ Sở Thu Mua Tôm" })
                  ]
                }),
                c.jsxs("div", {
                  className: "flex justify-between items-center text-slate-300",
            ...
            // Nút 1 chạm đăng nhập Google đề xuất
            c.jsxs("button", {
              type: "button",
              onClick: () => handleQuickGoogleAuth("levanmung98@gmail.com"),
              className: "w-full py-3 px-4 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/50 text-emerald-300 rounded-2xl font-bold text-xs flex items-center justify-between shadow-sm active:scale-[0.98] transition-all cursor-pointer",
              children: [
                c.jsxs("div", {
                  ...
                  children: [
                    ...
                    c.jsxs("div", {
                      className: "text-left",
                      children: [
                        c.jsx("span", { className: "font-black text-white block", children: "Đăng nhập với levanmung98@gmail.com" }),
                        c.jsx("span", { className: "text-[10px] text-emerald-400 block", children: "Tài khoản Google chính chủ (Bấm 1-chạm)" })
                      ]
                    })
                  ]
                }),
                c.jsx("span", { className: "text-[11px] bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1 rounded-xl font-black shadow-sm", children: "Vào Ký ➔" })
              ]
            }),
          ...
          children: c.jsxs("div", {
            ...
            children: [
                ...
                children: [
                  c.jsxs("div", {
                    ...
                    children: [
                      ...
                      c.jsx("h3", { className: "font-black text-sm", children: "Xác thực Tài khoản Google" })
                    ]
                  }),
                  c.jsx("button", {
                    type: "button",
              ...
              c.jsx("p", {
                ...
                children: "Chọn hoặc nhập tài khoản Google (Gmail) của Quý khách để vào ký hợp đồng:"
              }),
              c.jsxs("button", {
                type: "button",
                onClick: () => handleQuickGoogleAuth("levanmung98@gmail.com"),
                className: "w-full py-3 px-3 bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/50 rounded-2xl flex items-center justify-between text-xs text-emerald-300 font-bold transition-all cursor-pointer",
                children: [
                  c.jsxs("div", {
                    className: "flex items-center gap-2",
                    children: [
                      ...
                      c.jsxs("div", {
                        className: "text-left",
                        children: [
                          c.jsx("span", { className: "text-white font-bold block", children: "levanmung98@gmail.com" }),
  ...
  return c.jsxs("div", {
    ...
    children: [
      // Verified Gmail status bar
      c.jsxs("div", {
        className: "max-w-4xl mx-auto my-2 px-3 py-2 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-800 rounded-2xl flex flex-wrap items-center justify-between gap-2 text-xs shadow-sm",
        children: [
          c.jsxs("div", {
            className: "flex items-center gap-2",
            children: [
              ...
              c.jsxs("div", {
                children: [
                  c.jsxs("span", { className: "font-black text-emerald-900 dark:text-emerald-300", children: ["Đã xác thực Gmail: ", customerUser?.email || "Chính chủ"] }),
                  c.jsxs("span", { className: "text-slate-600 dark:text-slate-400 block text-[11px]", children: ["Chủ ao: ", customerUser?.name || contract?.farmer?.fullName || "Bên B"] })
                ]
              })
            ]
          }),
          c.jsx("button", {
            type: "button",
            onClick: () => {
              if (confirm("Bạn có muốn đăng xuất tài khoản Gmail này?")) {
                try { localStorage.removeItem("tom_customer_gmail_auth"); } catch (e) {}
                setCustomerUser(null);
              }
            },
            className: "px-2.5 py-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-100 rounded-lg text-slate-700 dark:text-slate-300 font-bold text-[11px] cursor-pointer",
        ...
        children: [
          c.jsxs("div", {
            ...
            children: [
              ...
              // TÓM TẮT ĐIỀU KHOẢN NHANH VÀ NÚT CHẤP NHẬN ĐIỀU KHOẢN CHO KHÁCH HÀNG
              c.jsxs("div", {
                className: "p-4 sm:p-5 bg-gradient-to-br from-emerald-50 via-teal-50/50 to-emerald-100/60 dark:from-emerald-950/40 dark:via-slate-800 dark:to-emerald-900/30 rounded-2xl border-2 border-emerald-500/50 dark:border-emerald-600/50 space-y-3 shadow-sm",
                children: [
                  c.jsxs("div", {
                    className: "flex flex-wrap items-center justify-between gap-3 border-b border-emerald-200 dark:border-emerald-800 pb-3",
                    children: [
                      c.jsxs("div", {
                        className: "flex items-center gap-2.5",
                        children: [
                          ...
                          c.jsxs("div", {
                            children: [
                              c.jsx("h4", { className: "font-black text-xs sm:text-sm uppercase text-emerald-900 dark:text-emerald-300 tracking-wide", children: "TÓM TẮT ĐIỀU KHOẢN MUA BÁN CHÍNH" }),
                              c.jsx("p", { className: "text-[11px] text-emerald-700 dark:text-emerald-400 font-medium", children: "Quý khách kiểm tra thông tin thu mua và bấm Chấp nhận điều khoản bên dưới:" })
                            ]
                          })
                        ]
                      }),
                      isSigned ? c.jsxs("div", {
                        className: "px-3.5 py-1.5 bg-emerald-600 text-white rounded-xl font-black text-xs flex items-center gap-1.5 shadow-sm",
                        children: [c.jsx(Fi, { size: 16 }), "ĐÃ CHẤP NHẬN & KÝ HỢP ĐỒNG"]
                      }) : c.jsxs("button", {
                        type: "button",
                        onClick: () => setIsSigning(true),
                        className: "px-4 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl font-black text-xs uppercase tracking-wider flex items-center gap-2 shadow-md active:scale-95 transition-all cursor-pointer",
                        children: [c.jsx("span", { children: "✍️" }), "Chấp nhận điều khoản & Ký ngay"]
                      })
                    ]
                  }),
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsxs("div", {
                        className: "bg-white/90 dark:bg-slate-900/80 p-2.5 rounded-xl border border-emerald-200/80 dark:border-emerald-800/50 shadow-xs",
                        children: [
                          c.jsx("span", { className: "text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold block", children: "Loại tôm thu mua" }),
                      ...
                      c.jsxs("div", {
                        className: "bg-white/90 dark:bg-slate-900/80 p-2.5 rounded-xl border border-emerald-200/80 dark:border-emerald-800/50 shadow-xs",
                        children: [
                          c.jsx("span", { className: "text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold block", children: "Đơn giá thu mua" }),
                          c.jsx("span", { className: "font-black text-red-600 dark:text-red-400", children: priceVal ? `${priceVal.toLocaleString()} đ/kg` : "Theo bảng size" })
                        ]
                      }),
                      c.jsxs("div", {
                        className: "bg-white/90 dark:bg-slate-900/80 p-2.5 rounded-xl border border-emerald-200/80 dark:border-emerald-800/50 shadow-xs",
                        children: [
                          c.jsx("span", { className: "text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold block", children: "Tiền đặt cọc" }),
                          c.jsx("span", { className: "font-black text-blue-700 dark:text-blue-400", children: depositAmt ? `${depositAmt.toLocaleString()} đ` : "0 đ" })
                        ]
                      }),
                      c.jsxs("div", {
                        className: "bg-white/90 dark:bg-slate-900/80 p-2.5 rounded-xl border border-amber-200/80 dark:border-amber-800/50 sm:col-span-2 shadow-xs",
                        children: [
                          c.jsx("span", { className: "text-[10px] text-amber-800 dark:text-amber-400 uppercase font-bold block", children: "⚖️ Quy cách dạt tôm" }),
                          c.jsx("span", { className: "font-bold text-amber-950 dark:text-amber-200", children: contract.rejectionSpec || "Dạt tôm mềm, ốp, gãy càng chuyển tính giá xào" })
                        ]
                      }),
                      c.jsxs("div", {
                        className: "bg-white/90 dark:bg-slate-900/80 p-2.5 rounded-xl border border-emerald-200/80 dark:border-emerald-800/50 shadow-xs",
                        children: [
                          c.jsx("span", { className: "text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold block", children: "Ngày kéo cân" }),
                ...
                children: [
                  ...
                  c.jsxs("div", {
                    ...
                    children: [
                      ...
                      c.jsxs("p", { children: ["1.1. Bên B nhận bán và Bên A đồng ý thu mua loại: ", c.jsx("strong", { className: "text-emerald-800 dark:text-emerald-300 uppercase font-black", children: contract.shrimpType || "Tôm Càng Xanh Loại 1" }), contract.estimatedYield ? ` (Sản lượng ước tính: ${Number(contract.estimatedYield).toLocaleString()} kg)` : "", " nuôi thả thực tế tại ao nuôi của Bên B theo địa chỉ và định vị đã xác nhận."] }),
                      c.jsxs("p", { children: ["1.2. Thời gian tiến hành kéo lưới cân tôm: ", c.jsx("strong", { className: "text-emerald-700 dark:text-emerald-400 font-black", children: contract.weighingDate || "Theo thỏa thuận hai bên" }), ". Khung giờ bắt dự kiến: ", c.jsx("strong", { children: contract.weighingTime || "06:00" }), " sáng để đảm bảo chất lượng tôm sống."] })
                    ]
                  }),
                  ...
                  c.jsxs("div", {
                    ...
                    children: [
                      ...
                      c.jsxs("div", {
                        className: "p-3.5 bg-amber-50/70 dark:bg-amber-950/20 rounded-xl border border-amber-200 dark:border-amber-900/40 space-y-1.5 my-2",
                        children: [
                          c.jsxs("p", { className: "font-black text-amber-900 dark:text-amber-300 text-xs uppercase flex items-center gap-1", children: [c.jsx("span", { children: "⚖️" }), "Quy cách dạt tôm & Trừ hao:"] }),
                          ...
                          c.jsxs("p", { children: [c.jsx("strong", { children: "• Quy cách trừ hao ráo nước & trừ bì: " }), c.jsx("span", { className: "text-slate-800 dark:text-slate-200", children: contract.tareSpec || "Trừ hao ráo nước chuẩn 1kg/thùng cân" })] }),
                          c.jsxs("p", { children: [c.jsx("strong", { children: "• Tiêu chuẩn chất lượng: " }), c.jsx("span", { className: "text-slate-800 dark:text-slate-200", children: contract.qualityStandard || "Tôm khỏe mạnh, sục oxy ≥ 95% lúc cân tại bờ ao, vỏ sạch không bọng nước" })] })
                        ]
                      }),
...
const Xm_ShareZaloModal = ({ contract, onClose }) => {
  ...
  const officialMessage = `Kính gửi anh/chị ${farmerName},\nCơ sở ${traderName} gửi bản Hợp Đồng Mua Bán Tôm Càng Xanh điện tử:\n• Loại tôm: ${contract.shrimpType || "Tôm Càng Xanh"}\n• Đơn giá: ${priceText}\n• Ngày cân: ${weighingDate}\nQuý khách bấm vào liên kết bên dưới, đăng nhập Gmail để xem chi tiết và ký hợp đồng trực tuyến:\n🔗 ${shareUrl}\nKính chúc hai bên hợp tác vui vẻ, thuận buồm xuôi gió!`;
          ...
          c.jsx("div", {
            ...
            children: [
              ...
              c.jsxs("div", {
                className: "space-y-4",
                children: [
                  c.jsx("h4", { className: "font-black uppercase text-center text-sm tracking-wider text-emerald-800 dark:text-emerald-400 border-y border-slate-200 dark:border-slate-700 py-2", children: "CÁC ĐIỀU KHOẢN KINH TẾ CHI TIẾT" }),
                  c.jsxs("div", {
                    className: "space-y-1 bg-emerald-50/50 dark:bg-emerald-950/20 p-3 rounded-xl border border-emerald-200/60 dark:border-emerald-800/40",
                    children: [
                      c.jsxs("p", { children: [c.jsx("strong", { children: "Điều 1 (Đối tượng & Ngày cân): " }), "Bên B bán và Bên A thu mua loại: ", c.jsx("strong", { className: "text-emerald-800 dark:text-emerald-300 uppercase", children: cData.shrimpType || "Tôm Càng Xanh Loại 1" }), ". Ngày kéo cân: ", c.jsx("strong", { className: "text-emerald-700 font-bold", children: cData.weighingDate || "Theo thỏa thuận" }), " (Giờ bắt: ", c.jsx("span", { className: "font-bold", children: cData.weighingTime || "06:00" }), ")."] })
                    ]
                  }),
                  c.jsxs("div", {
                    className: "space-y-1 bg-amber-50/50 dark:bg-amber-950/20 p-3 rounded-xl border border-amber-200/60 dark:border-amber-800/40",
                    children: [
                      c.jsxs("p", { children: [c.jsx("strong", { children: "Điều 2 (Đơn giá & Quy cách dạt tôm): " }), "Đơn giá chốt thu mua: ", priceVal ? c.jsxs("strong", { className: "text-red-600 font-bold text-sm", children: [priceVal.toLocaleString(), " VNĐ/kg"] }) : "Theo phân loại kích cỡ tôm sống oxy ≥ 95%."] }),
                      c.jsxs("p", { children: [c.jsx("strong", { children: "• Quy cách dạt tôm: " }), c.jsx("span", { className: "text-amber-900 dark:text-amber-300 font-bold", children: cData.rejectionSpec || "Dạt tôm mềm, ốp, gãy càng chuyển tính giá xào" })] }),
                      c.jsxs("p", { children: [c.jsx("strong", { children: "• Quy cách trừ bì & ráo nước: " }), c.jsx("span", { className: "text-slate-700 dark:text-slate-300", children: cData.tareSpec || "Trừ hao ráo nước chuẩn 1kg/thùng cân" })] })
                    ]
                  }),
                  c.jsxs("p", { children: [c.jsx("strong", { children: "Điều 3 (Phương thức cân): " }), "Cân đo bằng Cân Điện Tử chính xác tại bờ ao có chứng kiến của hai bên."] }),
                  c.jsxs("p", { children: [c.jsx("strong", { children: "Điều 4 (Tiền đặt cọc): " }), "Bên A đã đặt cọc trước: ", c.jsx("strong", { className: "text-blue-700 font-bold text-sm", children: depositAmt ? depositAmt.toLocaleString() + " VNĐ" : "0 VNĐ" }), " (Bằng chữ: ", c.jsx("em", { className: "font-bold", children: numberToWordsVN(depositAmt) }), "). Thanh toán dứt điểm 100% sau khi hoàn tất cân tôm."] }),
                  c.jsxs("div", {
...
// ============================================================================
// 2. THIẾT LẬP THÔNG TIN VỰA & DANH MỤC LOẠI TÔM
// ============================================================================
const Xm_TraderConfig = ({ setRoute }) => {
  ...
  // New shrimp type inputs
  const [newTypeName, setNewTypeName] = w.useState("");
  const [newTypePrice, setNewTypePrice] = w.useState("");

  // New catching spec input
  const [newSpecText, setNewSpecText] = w.useState("");

  const handleAddShrimpType = () => {
    if (!newTypeName.trim()) {
      alert("Vui lòng nhập tên loại tôm (vd: Tôm càng sen, Tôm càng xào, Tôm ngộp...)");
      return;
    }
    const p = parseFloat(newTypePrice.replace(/[^0-9]/g, "")) || 0;
    const newType = {
      id: "st_" + Date.now(),
...
const handleDeleteShrimpType = (id) => {
  ...
  return c.jsxs("div", {
    className: "flex flex-col h-full bg-[#f8f9fa] dark:bg-[#121212] font-roboto overflow-y-auto pb-28",
    children: [
      // Top Bar
      c.jsxs("div", {
        className: "bg-[#2e7d32] dark:bg-[#1b4d1e] p-4 text-white flex items-center justify-between shadow-lg sticky top-0 z-40",
        children: [
          c.jsxs("div", {
            ...
            children: [
              ...
              c.jsxs("div", {
                children: [
                  c.jsx("h1", { className: "text-lg font-black uppercase tracking-tight", children: "Thiết Lập Thông Tin Vựa & Loại Tôm" }),
                  c.jsx("p", { className: "text-[10px] opacity-80 uppercase tracking-wider font-bold", children: "Thông tin pháp lý, danh mục loại tôm & điều khoản hợp đồng" })
                ]
              })
            ]
          }),
          c.jsx("button", {
            onClick: handleSave,
            className: "bg-yellow-400 text-green-950 px-4 py-2 rounded-xl font-black text-xs uppercase flex items-center gap-1.5 shadow-md active:scale-95",
            children: [c.jsx(ec, { size: 16 }), "Lưu Cài Đặt"]
          })
        ]
      }),
      ...
      savedToast && c.jsxs("div", {
        className: "mx-4 mt-4 p-3 bg-emerald-600 text-white rounded-2xl flex items-center gap-2 shadow-lg font-bold text-xs animate-in slide-in-from-top-2",
        children: [c.jsx(P1, { size: 18 }), " Đã lưu thiết lập thông tin vựa & danh mục loại tôm thành công!"]
      }),

      c.jsxs("div", {
        className: "p-4 space-y-6 max-w-4xl mx-auto w-full",
        children: [
          // PHẦN 1: THÔNG TIN VỰA THU MUA
          c.jsxs("div", {
            className: "bg-white dark:bg-gray-900 p-5 rounded-[28px] shadow-sm border border-gray-100 dark:border-gray-800 space-y-4",
            children: [
              c.jsxs("div", {
                className: "flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800",
                children: [
                  c.jsx(Fu, { size: 20, className: "text-green-600 dark:text-green-400" }),
                  c.jsx("h3", { className: "font-black uppercase text-sm text-gray-800 dark:text-gray-100", children: "1. Thông tin Vựa / Cơ sở Thu mua (Bên A)" })
                ]
              }),
              c.jsxs("div", {
                ...
                children: [
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Tên thương lái / Cơ sở thu mua (*)" }),
                      c.jsx("input", {
                        type: "text",
                        value: profile.fullName || "",
                        onChange: (e) => setProfile({ ...profile, fullName: e.target.value }),
                        className: "w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-sm outline-none"
                      })
                    ]
                  }),
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Số điện thoại liên hệ (*)" }),
                      c.jsx("input", {
                        type: "text",
                        value: profile.phone || "",
                  ...
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Số CCCD / CMND (*)" }),
                      c.jsx("input", {
                        type: "text",
                  ...
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Địa chỉ vựa / cơ sở (*)" }),
                      c.jsx("input", {
                        type: "text",
          ...
          // PHẦN 2: THIẾT LẬP DANH MỤC LOẠI TÔM (Thay vì size tôm)
          c.jsxs("div", {
            className: "bg-white dark:bg-gray-900 p-5 rounded-[28px] shadow-sm border border-gray-100 dark:border-gray-800 space-y-4",
            children: [
              c.jsxs("div", {
                ...
                children: [
                  c.jsxs("div", {
                    className: "flex items-center gap-2",
                    children: [
                      c.jsx(pm, { size: 20, className: "text-amber-500" }),
                      c.jsx("h3", { className: "font-black uppercase text-sm text-gray-800 dark:text-gray-100", children: "2. Thiết Lập Danh Mục Loại Tôm & Giá Gợi Ý (VNĐ/kg)" })
                    ]
                  }),
                  c.jsxs("span", {
                    className: "text-xs font-bold text-gray-400 uppercase",
                    children: [(profile.shrimpTypes || []).length, " loại"]
                  })
                ]
              }),
              c.jsx("p", {
                className: "text-xs text-gray-500 dark:text-gray-400 font-semibold",
                children: "Cài đặt danh mục các loại tôm (vd: Tôm càng sen, Tôm càng xào, Tôm ngộp, Tôm lột, Tôm càng đại...). Khi bắt đầu cân, hệ thống sẽ cho phép chọn nhanh các loại tôm này."
              }),

              // Shrimp Types List
              c.jsx("div", {
                className: "space-y-2.5",
                children: (profile.shrimpTypes || []).map((t) => c.jsxs("div", {
                  className: "p-3.5 bg-gray-50 dark:bg-gray-800/60 rounded-2xl border border-gray-100 dark:border-gray-700/50 flex items-center justify-between gap-3",
                  children: [
                    c.jsxs("div", {
                      className: "flex items-center gap-3",
                      children: [
                        c.jsx("div", { className: "w-8 h-8 rounded-xl bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 flex items-center justify-center font-black text-xs", children: "🦐" }),
                        c.jsxs("div", {
                          children: [
                            c.jsx("p", { className: "font-black text-sm text-gray-800 dark:text-gray-100 uppercase", children: t.name }),
                            c.jsxs("p", { className: "text-xs font-bold text-gray-400", children: ["Giá thu mua gợi ý: ", c.jsx("span", { className: "text-green-700 dark:text-green-400 font-black", children: (t.price || 0).toLocaleString("vi-VN") }), " đ/kg"] })
                          ]
                        })
                      ]
                    }),
              ...
              // Add New Shrimp Type Form
              c.jsxs("div", {
                className: "p-4 bg-amber-50/50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl space-y-3",
                children: [
                  c.jsx("span", { className: "text-xs font-black uppercase text-amber-900 dark:text-amber-200 block", children: "+ Thêm loại tôm mới vào danh mục:" }),
                  c.jsxs("div", {
                    className: "grid grid-cols-1 sm:grid-cols-3 gap-2",
                    children: [
                      c.jsx("input", {
                        type: "text",
                        placeholder: "Tên loại tôm (vd: Tôm càng sen, Tôm càng xào...)",
                        value: newTypeName,
                        onChange: (e) => setNewTypeName(e.target.value),
                        className: "sm:col-span-2 p-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs outline-none uppercase"
                      }),
                      c.jsx("input", {
                        type: "number",
                        placeholder: "Giá gợi ý (VNĐ/kg)",
                        value: newTypePrice,
                        onChange: (e) => setNewTypePrice(e.target.value),
                        className: "p-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs outline-none text-right"
                      })
                    ]
                  }),
                  c.jsx("button", {
                    ...
                    className: "w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-black text-xs uppercase flex items-center justify-center gap-1.5 shadow-md active:scale-95 transition-all",
                    children: [c.jsx(As, { size: 16 }), "Thêm Loại Tôm Này"]
                  })
                ]
              })
            ]
          ...
          // PHẦN 3: QUY CÁCH BẮT & ĐIỀU KHOẢN HỢP ĐỒNG
          c.jsxs("div", {
            className: "bg-white dark:bg-gray-900 p-5 rounded-[28px] shadow-sm border border-gray-100 dark:border-gray-800 space-y-4",
            children: [
              c.jsxs("div", {
                className: "flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800",
                children: [
                  c.jsx(im, { size: 20, className: "text-blue-600 dark:text-blue-400" }),
                  c.jsx("h3", { className: "font-black uppercase text-sm text-gray-800 dark:text-gray-100", children: "3. Quy Cách Bắt & Điều Khoản Hợp Đồng Mặc Định" })
                ]
              }),
              ...
              c.jsx("div", {
                ...
                children: (profile.catchingSpecs || []).map((sp, idx) => c.jsxs("div", {
                  className: "p-3 bg-gray-50 dark:bg-gray-800/60 rounded-xl border border-gray-100 dark:border-gray-700/50 flex items-center justify-between gap-3 text-xs font-semibold text-gray-800 dark:text-gray-200",
                  children: [
                    c.jsxs("div", {
                      className: "flex items-center gap-2",
                      children: [
                        c.jsx(P1, { size: 14, className: "text-green-600 flex-shrink-0" }),
              ...
              c.jsxs("div", {
                ...
                children: [
                  c.jsx("input", {
                    type: "text",
                    placeholder: "Nhập quy cách bắt mới (vd: Quy cách trừ bì 100 con trừ 3 kg...)",
                    value: newSpecText,
                    onChange: (e) => setNewSpecText(e.target.value),
                    className: "flex-1 p-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs outline-none"
                  }),
                  c.jsx("button", {
                    type: "button",
                    onClick: handleAddSpec,
                    className: "px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl font-black text-xs uppercase shadow-sm",
                    children: "+ Thêm"
                  })
                ]
              }),
              ...
              c.jsxs("div", {
                className: "pt-2",
                children: [
                  c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Điều khoản bồi thường / phạt cọc mặc định:" }),
                  c.jsx("textarea", {
                    rows: 3,
                    value: profile.defaultTerms || "",
                    onChange: (e) => setProfile({ ...profile, defaultTerms: e.target.value }),
                    className: "w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-semibold text-xs text-gray-800 dark:text-gray-200 outline-none"
                  })
                ]
              })
            ]
          ...
          c.jsx("button", {
            type: "button",
            onClick: handleSave,
            className: "w-full py-4 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-black text-sm uppercase shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-transform",
            children: [c.jsx(ec, { size: 20 }), "Lưu Toàn Bộ Thiết Lập Thông Tin"]
          })
        ]
      })
    ]
...
// ============================================================================
// 3. DANH SÁCH CHỦ AO (AO CẦN CÂN SẮP THEO NGÀY BẮT GẦN NHẤT & AO ĐÃ CÂN)
// ============================================================================

// ============================================================================
// FIX CHỦ AO: KHAI BÁO CÁC MODAL & ICON BỊ THIẾU TRƯỚC CM
// ============================================================================
const Km = PinIcon;
const Xm_ElectronicContractModal = Xm_ContractModal;
const Xm_LocationPickerModal = ({ farmer, onClose, onSaveLocation }) => {
  const [lat, setLat] = w.useState(farmer?.lat ? String(farmer.lat) : "");
  const [lng, setLng] = w.useState(farmer?.lng ? String(farmer.lng) : "");
  const [loading, setLoading] = w.useState(false);
  const handleGetGPS = () => {
    if (typeof navigator !== "undefined" && navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          alert("Không thể lấy vị trí GPS: " + (err?.message || "Lỗi định vị"));
          setLoading(false);
        },
  ...
  return c.jsxs("div", {
    ...
    children: [
      c.jsx("div", { className: "fixed inset-0", onClick: onClose }),
      c.jsxs("div", {
        className: "relative bg-white dark:bg-gray-900 rounded-[28px] max-w-md w-full p-6 shadow-2xl border border-gray-100 dark:border-gray-800 z-10 font-roboto space-y-4",
        children: [
          c.jsxs("div", {
            className: "flex items-center justify-between border-b pb-3 dark:border-gray-800",
            children: [
              c.jsxs("div", {
                className: "flex items-center gap-2 text-green-700 dark:text-green-400",
                children: [
                  c.jsx(PinIcon, { size: 22 }),
                  c.jsx("h3", { className: "font-black uppercase text-base", children: "Định Vị Tọa Độ Ao Tôm" })
                ]
              }),
              c.jsx("button", { onClick: onClose, className: "p-1.5 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-500 font-bold", children: "✕" })
            ]
          }),
          c.jsxs("div", {
            className: "space-y-3",
            children: [
              c.jsxs("div", {
                className: "bg-green-50 dark:bg-green-950/30 p-3 rounded-xl text-xs",
                children: [
                  c.jsx("p", { className: "font-black text-green-900 dark:text-green-200 uppercase", children: farmer?.name || "Chủ ao" }),
                  c.jsx("p", { className: "text-gray-500 dark:text-gray-400 text-[11px]", children: farmer?.address || "Địa chỉ ao tôm" })
                ]
              }),
              c.jsxs("div", {
                className: "grid grid-cols-2 gap-2",
                children: [
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-[10px] font-bold text-gray-400 block mb-1 uppercase", children: "Vĩ độ (Latitude)" }),
                      c.jsx("input", {
                        type: "number",
                        step: "any",
                        value: lat,
                        onChange: e => setLat(e.target.value),
                        placeholder: "Vd: 9.176823",
                        className: "w-full p-2.5 bg-gray-50 dark:bg-gray-800 border rounded-xl text-xs font-mono font-bold outline-none"
                      })
                    ]
                  }),
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-[10px] font-bold text-gray-400 block mb-1 uppercase", children: "Kinh độ (Longitude)" }),
                      c.jsx("input", {
                        type: "number",
                        step: "any",
                        value: lng,
                        onChange: e => setLng(e.target.value),
                        placeholder: "Vd: 105.152341",
                        className: "w-full p-2.5 bg-gray-50 dark:bg-gray-800 border rounded-xl text-xs font-mono font-bold outline-none"
                      })
                    ]
                  })
              ...
              c.jsxs("button", {
                ...
                disabled: loading,
                className: "w-full py-2.5 bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100 rounded-xl font-black text-xs uppercase flex items-center justify-center gap-2 active:scale-95 transition-all",
                children: [c.jsx(PinIcon, { size: 16 }), loading ? "Đang định vị vệ tinh GPS..." : "📍 Lấy Tọa Độ GPS Hiện Tại"]
              })
            ]
          }),
          c.jsxs("div", {
            ...
            children: [
              c.jsx("button", {
                type: "button",
                onClick: onClose,
                className: "flex-1 py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-xl font-black text-xs uppercase active:scale-95",
                children: "Đóng"
              }),
              c.jsx("button", {
                type: "button",
                onClick: () => {
                  if (lat && lng) {
                    onSaveLocation(Number(lat), Number(lng));
                  } else {
                    alert("Vui lòng nhập hoặc lấy tọa độ GPS hợp lệ!");
                  }
                },
                className: "flex-1 py-2.5 bg-green-600 text-white rounded-xl font-black text-xs uppercase shadow-md active:scale-95",
                children: "Lưu Tọa Độ"
              })
            ]
          })
...
const Cm = ({ setRoute: S }) => {
  const [b, p] = w.useState(() => me.getFarmers());
  const [f, O] = w.useState("");
  const [D, U] = w.useState("UNWEIGHED"); // "UNWEIGHED" | "WEIGHED" | "ADD"
  const [Y, A] = w.useState(null);

  // Modals
  const [selectedPondForContract, setSelectedPondForContract] = w.useState(null);
  ...
  const [v, _] = w.useState({
    ...
    weighingTime: "06:00",
    traderPrice: "",
    catchingMethod: "Kéo lưới vét rạng sáng",
    oxygenRatio: "95% trở lên lúc cân tại bờ ao",
    qualityStandard: "Tôm khỏe mạnh, đều màu, nguyên vẹn càng",
    tareSpec: "Trừ hao ráo nước chuẩn 1kg/thùng cân",
    compensationTerms: "Bồi hoàn 100% tiền cọc và phạt gấp đôi (02 lần) nếu tự ý bán cho người khác",
    lat: null,
    lng: null
  });

  // Calculate days difference to today
  ...
  // Sort unweighed ponds by nearest harvest date
  const unweighedPonds = w.useMemo(() => {
    const list = b.filter(x => !x.weighed && x.status !== "WEIGHED");
    return list.sort((a, bItem) => {
      const diffA = getDaysDiff(a.weighingDate);
      const diffB = getDaysDiff(bItem.weighingDate);
      // Ponds with nearest positive diff (today=0, tomorrow=1) first, overdue next, no date last
      return diffA - diffB;
    });
  }, [b]);

  // Weighed ponds
  const weighedPonds = w.useMemo(() => {
    return b.filter(x => x.weighed === true || x.status === "WEIGHED");
  }, [b]);

  // Filtered by search query
  const displayedPonds = w.useMemo(() => {
    const source = D === "WEIGHED" ? weighedPonds : unweighedPonds;
    if (!f.trim()) return source;
    const q = f.toLowerCase();
    return source.filter(x => (x.name || "").toLowerCase().includes(q) || (x.phone || "").includes(q) || (x.address || "").toLowerCase().includes(q));
  }, [D, unweighedPonds, weighedPonds, f]);

  // Action: Start Weighing This Pond Immediately
  const handleStartWeighingThisPond = (pond) => {
    const weighingData = {
      farmerName: pond.name,
      riceType: pond.shrimpType || "Tôm càng sen",
      price: pond.traderPrice || pond.defaultPrice || 160000,
      deposit: pond.depositMoney || pond.defaultDeposit || 0,
      pondId: pond.id,
      contractId: pond.contractId || null,
      tareSpec: pond.tareSpec || "100 con trừ 3 kg"
    };
    localStorage.setItem("temp_weighing_info", JSON.stringify(weighingData));
    S(re.WEIGHING);
  };

  // Action: Mark Pond as Weighed / Unweighed
  const handleToggleWeighedStatus = (pondId, currentStatus) => {
    const updated = b.map(x => {
      if (x.id === pondId) {
        const next = !currentStatus;
        return { ...x, weighed: next, status: next ? "WEIGHED" : "CONTRACTED" };
      }
      return x;
    });
    p(updated);
    localStorage.setItem("annong_farmers", JSON.stringify(updated));
  };
  ...
  // Save new pond & create contract
  const handleSavePond = (shouldCreateContract = true) => {
    if (!v.name.trim()) {
      alert("Vui lòng nhập họ tên chủ ao tôm!");
      return;
    }
    const traderProf = getTraderProfileV3();
    const newId = Date.now().toString();
    const contractId = "HDTCX-" + (1e9 + Math.floor(Math.random() * 9e8));
    ...
    const newFarmer = {
      ...
      phone: v.phone,
      area: v.area,
      address: v.address || `${v.commune ? v.commune + ", " : ""}${v.district ? v.district + ", " : ""}${v.province}`,
      lat: v.lat,
      lng: v.lng,
      ...
      weighingTime: v.weighingTime,
      contractId: contractId,
      weighed: false,
      status: "CONTRACTED"
    };

    me.addFarmer(newFarmer);
    ...
    if (shouldCreateContract) {
      const newContract = {
        ...
        farmerName: v.name,
        farmerPhone: v.phone,
        farmerAddress: newFarmer.address,
        farmerIdCard: "",
        ...
        agreedPrice: Number(v.traderPrice) || 160000,
        unitPrice: Number(v.traderPrice) || 160000,
        weighingDate: v.weighingDate,
        weighingTime: v.weighingTime,
        catchingMethod: v.catchingMethod,
        oxygenRatio: v.oxygenRatio,
        qualityStandard: v.qualityStandard || "Tôm tươi sống oxy ≥ 95% lúc cân tại bờ ao, vỏ sạch không bọng nước",
        compensationTerms: v.compensationTerms || "Bên nào tự ý hủy cọc hoặc vi phạm bồi thường gấp 02 lần tiền cọc",
        createdAt: new Date().toISOString(),
        signedAt: null,
        farmerSignature: null,
        traderSignature: null,
        ...
        farmer: {
          ...
          idCard: "",
          coordinates: (v.lat && v.lng) ? { lat: Number(v.lat), lng: Number(v.lng) } : null,
          agreed: false
        },
        trader: {
          fullName: traderProf.fullName || "Cơ Sở Thu Mua Tôm Càng Xanh",
          phone: traderProf.phone || "",
          address: traderProf.address || "",
          idCard: traderProf.idCard || "",
          agreed: true
        },
        pondLocation: v.lat && v.lng ? { lat: v.lat, lng: v.lng } : null
      };

      try {
        if (window.saveContract) window.saveContract(newContract);
        if (window.syncContractToCloud) window.syncContractToCloud(newContract);
      } catch (e) {}

      // Open share modal
      setSelectedContractForShare(newContract);
    }
  ...
  return c.jsxs("div", {
    ...
    children: [
        ...
        children: [
          c.jsxs("div", {
            ...
            children: [
              ...
              c.jsx("div", {
                children: c.jsx("h1", { className: "text-lg font-black uppercase tracking-tight", children: "Danh Sách Ao Tôm" })
              })
            ]
          }),
          c.jsx("button", {
      ...
      // Navigation Tabs
      c.jsx("div", {
        className: "p-4 pb-0 max-w-4xl mx-auto w-full",
        children: c.jsxs("div", {
          className: "bg-white dark:bg-gray-900 p-1.5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex gap-1",
          children: [
            c.jsxs("button", {
              onClick: () => U("UNWEIGHED"),
              className: `flex-1 py-2.5 rounded-xl font-black text-xs uppercase flex items-center justify-center gap-2 transition-all ${D === "UNWEIGHED" ? "bg-green-600 text-white shadow-md" : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"}`,
              children: [
                c.jsx($u, { size: 16 }),
                "Ao Cần Cân",
                c.jsx("span", { className: `px-2 py-0.5 rounded-full text-[10px] ${D === "UNWEIGHED" ? "bg-white/20 text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300"}`, children: unweighedPonds.length })
              ]
            }),
            c.jsxs("button", {
              onClick: () => U("WEIGHED"),
              className: `flex-1 py-2.5 rounded-xl font-black text-xs uppercase flex items-center justify-center gap-2 transition-all ${D === "WEIGHED" ? "bg-blue-600 text-white shadow-md" : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"}`,
              children: [
                c.jsx(P1, { size: 16 }),
                "Ao Đã Cân",
                c.jsx("span", { className: `px-2 py-0.5 rounded-full text-[10px] ${D === "WEIGHED" ? "bg-white/20 text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300"}`, children: weighedPonds.length })
              ]
            })
          ]
        })
      ...
      // Search Bar (When in lists)
      D !== "ADD" && c.jsx("div", {
        className: "px-4 pt-3 max-w-4xl mx-auto w-full",
        children: c.jsxs("div", {
          className: "relative",
          children: [
            c.jsx(Id, { size: 18, className: "absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" }),
            c.jsx("input", {
              type: "text",
              placeholder: "Tìm chủ ao theo tên, số điện thoại hoặc địa chỉ...",
              value: f,
              onChange: (e) => O(e.target.value),
              className: "w-full pl-11 pr-4 py-3 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 text-xs font-bold outline-none shadow-sm"
            })
          ]
        })
      }),
      ...
      c.jsx("div", {
        ...
        children: D === "ADD" ? (
          // FORM THÊM CHỦ AO MỚI
          c.jsxs("div", {
            className: "bg-white dark:bg-gray-900 p-6 rounded-[32px] shadow-sm border border-gray-100 dark:border-gray-800 space-y-4 animate-in fade-in duration-200",
            children: [
              c.jsx("h2", { className: "text-base font-black uppercase text-gray-800 dark:text-gray-100 border-b pb-3 border-gray-100 dark:border-gray-800", children: "Thêm Chủ Ao Thu Mua Tôm Mới" }),
              c.jsxs("div", {
                className: "grid grid-cols-1 sm:grid-cols-2 gap-3",
                children: [
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Họ và tên chủ ao (*)" }),
                      c.jsx("input", {
                        type: "text",
                        value: v.name,
                        onChange: (e) => _({ ...v, name: e.target.value }),
                        placeholder: "Vd: Nguyễn Văn Út",
                        className: "w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs outline-none uppercase"
                      })
                    ]
                  }),
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Số điện thoại (*)" }),
                      c.jsx("input", {
                        type: "text",
                        value: v.phone,
                        onChange: (e) => _({ ...v, phone: e.target.value }),
                  ...
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Ngày dự kiến bắt tôm (*)" }),
                      c.jsx("input", {
                        type: "date",
                        value: v.weighingDate,
                        onChange: (e) => _({ ...v, weighingDate: e.target.value }),
                        className: "w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs outline-none"
                      })
                    ]
                  }),
                  c.jsxs("div", {
                    children: [
                      c.jsx("label", { className: "text-xs font-bold text-gray-500 block mb-1", children: "Giờ bắt tôm" }),
                      c.jsx("input", {
                        type: "time",
                        value: v.weighingTime,
                        onChange: (e) => _({ ...v, weighingTime: e.target.value }),
                        className: "w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs outline-none"
                      })
                    ]
                  }),
                  ...
                  // Section: Quy cách trừ hao ráo nước & trừ bì
                  c.jsxs("div", {
                  ...
                  c.jsxs("div", {
                    ...
                    children: [
                      ...
                      c.jsx("input", {
                        type: "text",
                        value: v.address,
                        onChange: (e) => _({ ...v, address: e.target.value }),
                        placeholder: "Số nhà, ấp/khóm, xã, huyện...",
                        className: "w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs outline-none"
                      })
                    ]
                  })
              ...
              c.jsxs("div", {
                ...
                children: [
                  c.jsxs("button", {
                    ...
                    className: "flex-1 p-4 bg-yellow-400 hover:bg-yellow-500 text-gray-900 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2 shadow-lg border-b-4 border-yellow-700 active:scale-95",
                    children: [c.jsx(im, { size: 18 }), "Lưu & Tạo Hợp Đồng Điện Tử"]
                  }),
                  c.jsxs("button", {
                    type: "button",
                    onClick: () => handleSavePond(false),
                    className: "p-4 bg-green-700 hover:bg-green-800 text-white rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2 shadow-lg active:scale-95",
                    children: [c.jsx(ec, { size: 18 }), "Chỉ Lưu Chủ Ao"]
                  })
                ]
              })
            ]
          })
        ) : displayedPonds.length === 0 ? (
          // EMPTY STATE
          c.jsxs("div", {
            className: "p-12 text-center bg-white dark:bg-gray-900 rounded-[32px] border border-gray-100 dark:border-gray-800 space-y-3",
            children: [
              c.jsx($u, { size: 48, className: "mx-auto text-gray-300 dark:text-gray-600" }),
              c.jsx("p", { className: "font-black text-sm uppercase text-gray-500", children: D === "WEIGHED" ? "Chưa có ao nào đã cân hoàn tất" : "Không có ao tôm nào cần cân trong danh sách" }),
              c.jsx("button", {
                onClick: () => U("ADD"),
                className: "px-4 py-2 bg-green-600 text-white font-bold text-xs rounded-xl uppercase",
                children: "+ Thêm chủ ao mới"
              })
            ]
          })
        ) : (
          ...
          c.jsx("div", {
            className: "space-y-3",
            children: displayedPonds.map((pond) => {
              const daysDiff = getDaysDiff(pond.weighingDate);
              let badgeColor = "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300";
              let badgeText = pond.weighingDate ? `Ngày bắt: ${pond.weighingDate}` : "Chưa đặt ngày bắt";

              if (D === "UNWEIGHED") {
                if (daysDiff === 0) {
                  badgeColor = "bg-red-600 text-white animate-pulse shadow-sm";
                  badgeText = "🚨 HÔM NAY BẮT TÔM";
                } else if (daysDiff === 1) {
                  badgeColor = "bg-amber-500 text-white shadow-sm";
                  badgeText = "📅 NGÀY MAI BẮT TÔM";
                } else if (daysDiff > 1 && daysDiff < 999) {
                  badgeColor = "bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 border border-green-200 dark:border-green-800";
                  badgeText = `⏳ Còn ${daysDiff} ngày (${pond.weighingDate})`;
                } else if (daysDiff < 0) {
                  badgeColor = "bg-orange-100 text-orange-800 dark:bg-orange-900/40 dark:text-orange-300 border border-orange-200";
                  badgeText = `⚠️ Quá hẹn ${Math.abs(daysDiff)} ngày (${pond.weighingDate})`;
                }
              } else {
                badgeColor = "bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300";
                badgeText = "✓ ĐÃ CÂN XONG";
              }

              return c.jsxs("div", {
                className: "p-4 bg-white dark:bg-gray-900 rounded-[28px] border border-gray-100 dark:border-gray-800 shadow-sm space-y-3 hover:shadow-md transition-all",
                children: [
                  // Pond Card Header
                  c.jsxs("div", {
                          ...
                          c.jsxs("div", {
                            ...
                            children: [
                              ...
                              pond.phone && c.jsxs("a", {
                                ...
                                title: "Gọi điện",
                                children: c.jsx(hm, { size: 14 })
                              })
                            ]
                          }),
                          c.jsx("p", { className: "text-xs text-gray-400 font-semibold truncate", children: pond.address || "Địa chỉ chưa cập nhật" })
                        ]
                      }),
                      c.jsx("span", {
                        className: `px-3 py-1 rounded-xl text-[11px] font-black uppercase whitespace-nowrap ${badgeColor}`,
                        children: badgeText
                      })
                    ]
                  }),

                  // Pond Card Key Details
                  c.jsxs("div", {
                    className: "grid grid-cols-2 sm:grid-cols-4 gap-2 bg-gray-50 dark:bg-gray-800/60 p-3 rounded-2xl text-xs font-semibold",
                    children: [
                      c.jsxs("div", {
                        children: [
                          c.jsx("span", { className: "text-[10px] text-gray-400 uppercase block", children: "Loại tôm" }),
                          c.jsx("span", { className: "font-black text-gray-800 dark:text-gray-200", children: pond.shrimpType || "Tôm càng sen" })
                        ]
                      }),
                      c.jsxs("div", {
                        children: [
                          c.jsx("span", { className: "text-[10px] text-gray-400 uppercase block", children: "Giá mua" }),
                          c.jsxs("span", { className: "font-black text-green-700 dark:text-green-400", children: [(pond.defaultPrice || 160000).toLocaleString(), " đ/kg"] })
                        ]
                      }),
                      c.jsxs("div", {
                        children: [
                          c.jsx("span", { className: "text-[10px] text-gray-400 uppercase block", children: "Tiền cọc" }),
                          c.jsxs("span", { className: "font-black text-purple-700 dark:text-purple-300", children: [(pond.defaultDeposit || 0).toLocaleString(), " đ"] })
                        ]
                      }),
                      c.jsxs("div", {
                        children: [
                          c.jsx("span", { className: "text-[10px] text-gray-400 uppercase block", children: "Trừ bì" }),
                          c.jsx("span", { className: "font-bold text-gray-600 dark:text-gray-300 truncate block", children: pond.tareSpec || "100 con trừ 3 kg" })
                        ]
                      })
                    ]
                  }),
                  ...
                  // Action Buttons
                  c.jsxs("div", {
                    className: "flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-gray-100 dark:border-gray-800",
                    children: [
                      // Left Buttons: Hợp đồng, Toạ độ ao
                      c.jsxs("div", {
                        className: "flex items-center gap-2",
                        children: [
                          ...
                          c.jsxs("button", {
                            type: "button",
                            onClick: () => {
                              const contracts = (typeof window !== "undefined" && window.getContracts) ? window.getContracts() : (typeof getContracts === "function" ? getContracts() : []);
                              let found = contracts.find(c => c && (c.farmerId === pond.id || c.id === pond.contractId || c.pondId === pond.id));
                              if (!found) {
                                const traderProf = typeof getTraderProfileV3 === "function" ? getTraderProfileV3() : {};
                                const genId = pond.contractId || ("HDTCX-" + (pond.id || Date.now()));
                                found = {
                                  id: genId,
                                  pondId: pond.id,
                                  farmerId: pond.id,
                                  farmer: {
                                    fullName: pond.name || "Chủ ao",
                                    phone: pond.phone || "",
                                    idCard: pond.idCard || "",
                                    address: pond.address || "",
                                    coordinates: (pond.lat && pond.lng) ? { lat: Number(pond.lat), lng: Number(pond.lng) } : null,
                                    agreed: !!pond.weighed
                                  },
                                  trader: {
                                    fullName: traderProf.fullName || "Cơ Sở Thu Mua Tôm Càng Xanh",
                                  ...
                                  farmerName: pond.name,
                                  farmerPhone: pond.phone,
                                  farmerAddress: pond.address,
                                  traderName: traderProf.fullName,
                                  traderPhone: traderProf.phone,
                                  traderAddress: traderProf.address,
                                  ...
                                  unitPrice: Number(pond.defaultPrice) || 160000,
                                  weighingDate: pond.weighingDate || new Date().toISOString().split("T")[0],
                                  weighingTime: pond.weighingTime || "06:00",
                                  qualityStandard: "Tôm khỏe mạnh, sục oxy ≥ 95% lúc cân tại bờ ao, không bọng nước",
                                  compensationTerms: "Bên nào tự ý hủy cọc hoặc vi phạm bồi thường gấp 02 lần tiền cọc",
                                  status: pond.weighed ? "COMPLETED" : "PENDING_FARMER",
                                  createdAt: new Date().toISOString()
                                };
                                if (typeof saveContract === "function") saveContract(found);
                                else if (typeof window !== "undefined" && window.saveContract) window.saveContract(found);
                              }
                          ...
                          c.jsxs("button", {
                            type: "button",
                            onClick: () => setLocationPickerPond(pond),
                            className: `px-3 py-1.5 rounded-xl font-black text-xs uppercase flex items-center gap-1.5 transition-colors ${pond.lat && pond.lng ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300" : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300"}`,
                            children: [c.jsx(Km, { size: 14 }), pond.lat && pond.lng ? "Vị Trí Ao" : "Định Vị"]
                          })
                        ]
                      }),

                      // Right Buttons: CÂN NGAY & Trạng thái đã cân
                      c.jsxs("div", {
                        className: "flex items-center gap-2",
                        children: [
                          // CÂN NGAY (Primary Button)
                          D === "UNWEIGHED" && c.jsxs("button", {
                            type: "button",
                            onClick: () => handleStartWeighingThisPond(pond),
                            className: "px-4 py-2 bg-gradient-to-r from-[#2e7d32] to-[#1b5e20] hover:from-green-700 hover:to-green-900 text-white rounded-xl font-black text-xs uppercase flex items-center gap-1.5 shadow-md active:scale-95 transition-all border-b-2 border-green-900",
                            children: [c.jsx($u, { size: 16 }), "Cân Ngay"]
                          }),

                          // Toggle Weighed Button
                          c.jsxs("button", {
                            type: "button",
                            onClick: () => handleToggleWeighedStatus(pond.id, pond.weighed),
                            className: `px-3 py-1.5 rounded-xl font-black text-xs uppercase flex items-center gap-1 transition-colors ${pond.weighed ? "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300" : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300"}`,
                            title: pond.weighed ? "Chuyển lại sang chưa cân" : "Đánh dấu đã cân xong",
                            children: [c.jsx(P1, { size: 14 }), pond.weighed ? "Chưa Cân" : "Đã Cân Xong"]
                          }),
      ...
      // Contract Viewer Modal if selected
      selectedContractForView && c.jsx(Xm_ElectronicContractModal, {
        contract: selectedContractForView,
        onClose: () => setSelectedContractForView(null),
        onShare: () => {
          setSelectedContractForShare(selectedContractForView);
        }
      }),

      // Share Zalo Modal if selected
      selectedContractForShare && c.jsx(Xm_ShareZaloModal, {
        contract: selectedContractForShare,
        onClose: () => setSelectedContractForShare(null)
      }),

      // Location Picker Modal if selected
      locationPickerPond && c.jsx(Xm_LocationPickerModal, {
        farmer: locationPickerPond,
        onClose: () => setLocationPickerPond(null),
        onSaveLocation: (lat, lng) => {
          const updated = b.map(x => x.id === locationPickerPond.id ? { ...x, lat, lng } : x);
          p(updated);
          localStorage.setItem("annong_farmers", JSON.stringify(updated));
          setLocationPickerPond(null);
        }
      })
    ]
  });
...
// ==================== SALES FEATURE EXTENSION (3 TARGET GROUPS & NO TARE) ====================
Ot.SALES = "annong_sales_v2";
Ot.BUYERS = "annong_buyers_v2";

// Extend me storage
me.getSales = function() {
  try {
    return JSON.parse(localStorage.getItem(Ot.SALES) || "[]");
  } catch(e) {
    return [];
  }
};

me.saveSales = function(list) {
  try {
    localStorage.setItem(Ot.SALES, JSON.stringify(list));
  } catch(e) {
    console.warn("Save sales error", e);
  }
};

me.updateSale = function(id, updates) {
...
me.deleteSale = function(id) {
  const list = me.getSales().filter(s => s.id !== id);
  me.saveSales(list);
};

me.getBuyers = function() {
  try {
    ...
    const defaults = [
      {
        id: "b_xn1",
        targetGroup: "FACTORY",
        name: "Công ty CP Thủy sản Camimex",
        representative: "Nguyễn Văn Cảnh (GĐ Thu mua)",
        phone: "0290.3838383",
        address: "KCN Phường 8, TP. Cà Mau, Tỉnh Cà Mau",
        latitude: 9.1768,
        longitude: 105.1524,
        shrimpPrices: [
          { id: "sp_1", typeName: "Tôm càng sen size lớn (10-15 c/kg)", price: 220000 },
          { id: "sp_2", typeName: "Tôm càng xô công nghiệp", price: 175000 },
          { id: "sp_3", typeName: "Tôm lột cấp đông xuất khẩu", price: 140000 }
        ],
        note: "Yêu cầu tôm sống hoặc bảo quản nước đá 0-2 độ C",
        debtAmount: 0
      },
      {
        id: "b_xn2",
        ...
        representative: "Trần Thị Mai (Phòng Kế hoạch)",
        phone: "0290.3839393",
        address: "Khu B, KCN Sông Đốc, Trần Văn Thời, Cà Mau",
        latitude: 9.0667,
        longitude: 104.9167,
        shrimpPrices: [
          { id: "sp_4", typeName: "Tôm càng sen cồ loại 1", price: 240000 },
          { id: "sp_5", typeName: "Tôm càng sen size trung", price: 190000 },
          { id: "sp_6", typeName: "Tôm thẻ chân trắng size 30", price: 165000 }
        ],
        note: "Tiêu chuẩn xuất khẩu Châu Âu",
        debtAmount: 0
      },
      {
        id: "b_bl1",
        ...
        representative: "Huỳnh Thanh Phong (Chủ nhà hàng)",
        phone: "0913.555.666",
        address: "Số 45 Trần Phú, Phường 7, TP. Bạc Liêu",
        latitude: 9.2882,
        longitude: 105.7244,
        shrimpPrices: [],
        note: "Nhập thủ công theo thực tế thực đơn ngày",
        debtAmount: 0
      },
      {
        id: "b_bl2",
        targetGroup: "RETAIL",
        name: "Vựa hải sản Tươi Sống Anh Ba",
        representative: "Nguyễn Văn Ba",
        phone: "0918.777.888",
        address: "Ấp 3, Xã Tân Thành, TP. Cà Mau",
        latitude: 9.1855,
        longitude: 105.1822,
        shrimpPrices: [],
        note: "Khách quen lấy tôm sống mỗi sáng",
        debtAmount: 0
      },
      {
        id: "b_cho1",
        targetGroup: "MARKET",
        name: "Vựa tôm Cô Tư (Chợ Đầu Mối Bình Điền)",
        representative: "Võ Thị Bé Tư (Chủ vựa)",
        phone: "0918.111.222",
        address: "Khu B2, Chợ Đầu Mối Bình Điền, Phường 7, Quận 8, TP.HCM",
        latitude: 10.7061,
        longitude: 106.6272,
        defaultVehiclePlate: "69C-088.99",
        defaultDriverPhone: "0919.444.555",
        defaultDriverName: "Tài xế Ba Chở Hàng",
        marketDeliveryTime: "Chuyến xe đêm 2h sáng",
        shrimpPrices: [],
        note: "Xe tải đông lạnh chạy chợ đêm Bình Điền",
        debtAmount: 0
      },
      {
        id: "b_cho2",
        targetGroup: "MARKET",
        name: "Sạp Hải Sản Tươi Chợ Cần Thơ",
        representative: "Lê Hoàng Quân",
        phone: "0903.333.444",
        address: "Cổng số 2, Chợ Tân An, Ninh Kiều, Cần Thơ",
        latitude: 10.0333,
        longitude: 105.7833,
        defaultVehiclePlate: "65C-123.45",
        defaultDriverPhone: "0909.888.999",
        defaultDriverName: "Bác Năm Giao Chợ",
        marketDeliveryTime: "Chuyến sớm 4h sáng",
        shrimpPrices: [],
        note: "Tôm oxy sục khí giao chợ nổi & chợ dân sinh",
        debtAmount: 0
      }
    ];
    localStorage.setItem(Ot.BUYERS, JSON.stringify(defaults));
    return defaults;
  } catch(e) {
    return [];
  }
...
me.saveBuyers = function(list) {
  try {
    localStorage.setItem(Ot.BUYERS, JSON.stringify(list));
  } catch(e) {
    console.warn("Save buyers error", e);
  }
};

me.addBuyer = function(buyer) {
  const list = me.getBuyers();
  ...
  if (idx < 0) {
    idx = list.findIndex(b => (phone && b.phone === phone) || (name && b.name.toLowerCase() === name.toLowerCase()));
  }
  
  if (idx >= 0) {
    list[idx] = { ...list[idx], ...buyer, updatedAt: new Date().toISOString() };
  } else {
    list.unshift({
      id: "b_" + Date.now().toString(),
      targetGroup: buyer.targetGroup || "FACTORY",
      name,
      ...
      shrimpPrices: buyer.shrimpPrices || [],
      defaultVehiclePlate: buyer.defaultVehiclePlate || "",
      defaultDriverPhone: buyer.defaultDriverPhone || "",
      defaultDriverName: buyer.defaultDriverName || "",
      marketDeliveryTime: buyer.marketDeliveryTime || "",
      note: buyer.note || "",
      debtAmount: buyer.debtAmount || 0,
      createdAt: new Date().toISOString()
    });
  }
  me.saveBuyers(list);
...
me.deleteBuyer = function(id) {
  const list = me.getBuyers().filter(b => b.id !== id);
  me.saveBuyers(list);
};
...
if (typeof re !== "undefined") {
  re.SALES = "SALES";
  re.SALES_DETAIL = "SALES_DETAIL";
  re.CUSTOMERS = "CUSTOMERS";
}

// 3 Target Groups Configuration
const SALE_GROUPS = {
  FACTORY: {
    id: "FACTORY",
    num: "1",
    label: "Xí nghiệp tôm",
    shortLabel: "Xí nghiệp",
    badgeBg: "bg-blue-100 text-blue-800 dark:bg-blue-950/80 dark:text-blue-300 border-blue-200 dark:border-blue-800",
    pillActive: "bg-blue-600 text-white shadow-md shadow-blue-500/30",
    pillInactive: "bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-100",
    icon: "🏢",
    sub: "Nhà máy chế biến, công ty thủy sản xuất khẩu",
    defaultShrimpTypes: [
      { id: "df_1", typeName: "Tôm càng sen size lớn (10-15 c/kg)", price: 220000 },
      { id: "df_2", typeName: "Tôm càng xô công nghiệp", price: 175000 },
      { id: "df_3", typeName: "Tôm lột cấp đông xuất khẩu", price: 140000 },
      { id: "df_4", typeName: "Tôm thẻ chân trắng size 30", price: 165000 }
    ]
  },
  RETAIL: {
    id: "RETAIL",
    ...
    pillActive: "bg-emerald-600 text-white shadow-md shadow-emerald-500/30",
    pillInactive: "bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-100",
    icon: "🛒",
    sub: "Nhà hàng, quán ăn, khách mua trực tiếp tại vựa",
    defaultShrimpTypes: [
      { id: "df_5", typeName: "Tôm càng xanh sống oxy", price: 240000 },
      { id: "df_6", typeName: "Tôm sen tươi sống loại 1", price: 260000 },
      { id: "df_7", typeName: "Tôm càng xô tươi", price: 180000 }
    ]
  },
  MARKET: {
    id: "MARKET",
    num: "3",
    label: "Chợ thủy hải sản",
    shortLabel: "Chợ hải sản",
    badgeBg: "bg-amber-100 text-amber-800 dark:bg-amber-950/80 dark:text-amber-300 border-amber-200 dark:border-amber-800",
    pillActive: "bg-amber-600 text-white shadow-md shadow-amber-500/30",
    pillInactive: "bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-100",
    icon: "🏪",
    sub: "Vựa tôm chợ đầu mối, sạp cá chợ dân sinh & xe giao chợ",
    defaultShrimpTypes: [
      { id: "df_8", typeName: "Tôm chở chợ đầu mối Bình Điền", price: 195000 },
      { id: "df_9", typeName: "Tôm dạt chợ / tôm nhỏ", price: 110000 },
      { id: "df_10", typeName: "Tôm sú oxy chạy bạn hàng", price: 235000 }
    ]
  }
};


// ==================== SCREEN: Xm_CustomersScreen ====================
const Xm_CustomersScreen = ({ setRoute, onSelectForSale, role }) => {
  const [activeGroup, setActiveGroup] = w.useState("FACTORY");
  const [buyersList, setBuyersList] = w.useState([]);
  const [searchTerm, setSearchTerm] = w.useState("");
  const [showModal, setShowModal] = w.useState(false);
  const [editingBuyer, setEditingBuyer] = w.useState(null);
  const [gpsLoading, setGpsLoading] = w.useState(false);
  const [gpsSuccess, setGpsSuccess] = w.useState(false);

  const [formData, setFormData] = w.useState({
    targetGroup: "FACTORY",
    name: "",
    representative: "",
    phone: "",
    ...
    defaultDriverPhone: "",
    defaultDriverName: "",
    marketDeliveryTime: "Chuyến đêm 2h sáng"
  });

  const refreshList = () => {
    try {
      setBuyersList(me.getBuyers());
    } catch(e) {}
  };
  ...
  const filteredList = w.useMemo(() => {
    return buyersList.filter(b => {
      const matchGrp = (b.targetGroup || "FACTORY") === activeGroup;
      if (!matchGrp) return false;
      if (!searchTerm) return true;
      const term = searchTerm.toLowerCase();
      return (
        (b.name && b.name.toLowerCase().includes(term)) ||
        (b.representative && b.representative.toLowerCase().includes(term)) ||
        (b.phone && b.phone.includes(term)) ||
        (b.address && b.address.toLowerCase().includes(term))
      );
    });
  }, [buyersList, activeGroup, searchTerm]);

  const groupCounts = w.useMemo(() => {
    const counts = { FACTORY: 0, RETAIL: 0, MARKET: 0 };
    buyersList.forEach(b => {
      const g = b.targetGroup || "FACTORY";
      if (counts[g] !== undefined) counts[g]++;
    });
    return counts;
  }, [buyersList]);

  const openAddModal = (groupToSet) => {
    const grp = groupToSet || activeGroup;
    ...
    setFormData({
      ...
      longitude: "",
      note: "",
      shrimpPrices: grp === "FACTORY" ? [
        { id: "sp_init1", typeName: "Tôm càng sen size lớn (10-15 c/kg)", price: 220000 },
        { id: "sp_init2", typeName: "Tôm càng xô công nghiệp", price: 175000 },
        { id: "sp_init3", typeName: "Tôm lột cấp đông xuất khẩu", price: 140000 }
      ] : [],
      defaultVehiclePlate: "",
      defaultDriverPhone: "",
      defaultDriverName: "",
      marketDeliveryTime: grp === "MARKET" ? "Chuyến đêm 2h sáng" : ""
    });
    setShowModal(true);
  };
  ...
  const openEditModal = (buyer) => {
    ...
    setFormData({
      ...
      phone: buyer.phone || "",
      address: buyer.address || "",
      latitude: buyer.latitude ? String(buyer.latitude) : "",
      longitude: buyer.longitude ? String(buyer.longitude) : "",
      note: buyer.note || "",
      shrimpPrices: Array.isArray(buyer.shrimpPrices) && buyer.shrimpPrices.length > 0 
        ? [...buyer.shrimpPrices] 
        : [
            { id: "sp_1", typeName: "Tôm càng sen size lớn (10-15 c/kg)", price: 220000 },
            { id: "sp_2", typeName: "Tôm càng xô công nghiệp", price: 175000 }
  ...
  const handleGetGps = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setGpsLoading(false);
        alert("Không thể lấy toạ độ GPS: " + (err.message || "Vui lòng cấp quyền định vị trong trình duyệt"));
      },
      { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }
    );
  };

  const openDirections = (buyer) => {
    let url = "";
    if (buyer.latitude && buyer.longitude) {
      url = "https://www.google.com/maps/dir/?api=1&destination=" + buyer.latitude + "," + buyer.longitude;
    } else if (buyer.address) {
      url = "https://www.google.com/maps/dir/?api=1&destination=" + encodeURIComponent(buyer.address);
    } else {
      url = "https://www.google.com/maps/search/?api=1&query=" + encodeURIComponent(buyer.name);
    }
  ...
  const handleSaveBuyer = (e) => {
    ...
    if (!formData.name.trim()) {
      alert("Vui lòng nhập tên đối tác / khách hàng");
      return;
    }
    const cleanPrices = (formData.shrimpPrices || []).filter(sp => sp.typeName && sp.typeName.trim()).map(sp => ({
      id: sp.id || Date.now().toString() + Math.random().toString().slice(2, 6),
      typeName: sp.typeName.trim(),
      price: Number(sp.price || 0)
    }));

    const payload = {
      ...formData,
      id: editingBuyer ? editingBuyer.id : ("b_" + Date.now().toString()),
      ...
      shrimpPrices: cleanPrices,
      debtAmount: editingBuyer ? (editingBuyer.debtAmount || 0) : 0
    };
  ...
  const handleDeleteBuyer = (buyer) => {
    if (confirm("Bạn có chắc chắn muốn xóa khách hàng: " + buyer.name + "?")) {
      me.deleteBuyer(buyer.id);
      refreshList();
    }
  };

  const handleStartSaleForBuyer = (buyer) => {
    try {
      localStorage.setItem("preselected_buyer_for_sale", JSON.stringify(buyer));
    } catch(e) {}
    if (typeof onSelectForSale === "function") {
      onSelectForSale(buyer);
    } else {
      setRoute(re.SALES);
    }
  };

  const addShrimpPriceRow = (namePreset, pricePreset) => {
    setFormData(prev => ({
      ...prev,
      shrimpPrices: [
        ...(prev.shrimpPrices || []),
        {
          ...
          typeName: namePreset || "",
          price: pricePreset || 200000
        }
      ]
    }));
  };

  const removeShrimpPriceRow = (idx) => {
    setFormData(prev => ({
      ...prev,
      shrimpPrices: (prev.shrimpPrices || []).filter((_, i) => i !== idx)
    }));
  };

  const updateShrimpPriceRow = (idx, field, value) => {
    setFormData(prev => {
      const arr = [...(prev.shrimpPrices || [])];
      arr[idx] = { ...arr[idx], [field]: value };
      return { ...prev, shrimpPrices: arr };
    });
  };

  const curGrpDef = SALE_GROUPS[activeGroup] || SALE_GROUPS.FACTORY;
  ...
  return c.jsxs("div", {
    ...
    children: [
      c.jsxs("header", {
        ...
        children: [
              ...
              c.jsxs("div", {
                children: [
                  c.jsx("h2", {
                    ...
                    children: "Danh Bạ Khách Hàng"
                  }),
                  c.jsx("span", {
                    className: "text-[10px] font-bold opacity-80 uppercase tracking-widest block",
                    children: "🏢 Xí nghiệp • 🛒 Bán lẻ • 🏪 Chợ thủy hải sản"
                  })
                ]
              })
            ]
          ...
          c.jsxs("button", {
            ...
            className: "flex items-center gap-1.5 px-3.5 py-2 bg-amber-400 hover:bg-amber-300 text-gray-950 rounded-xl font-black text-xs uppercase tracking-wider shadow-md active:scale-95 transition-all cursor-pointer",
            children: [
              c.jsx(As, { size: 16 }),
              "Thêm Mới"
            ]
      ...
      c.jsxs("div", {
        className: "flex-1 overflow-y-auto p-4 space-y-4 max-w-4xl mx-auto w-full pb-24",
        children: [
          c.jsx("div", {
            className: "grid grid-cols-3 gap-2 p-1.5 bg-gray-200/80 dark:bg-gray-800/80 rounded-2xl",
            children: Object.values(SALE_GROUPS).map(grp => {
              const isActive = activeGroup === grp.id;
              const count = groupCounts[grp.id] || 0;
              return c.jsxs("button", {
                key: grp.id,
                onClick: () => setActiveGroup(grp.id),
                className: "py-2.5 px-2 rounded-xl text-xs font-black uppercase transition-all flex flex-col sm:flex-row items-center justify-center gap-1.5 cursor-pointer " +
                  (isActive ? grp.pillActive : grp.pillInactive),
                children: [
                  c.jsx("span", { className: "text-base leading-none", children: grp.icon }),
                  c.jsx("span", { className: "truncate", children: grp.shortLabel }),
                  c.jsxs("span", {
                    className: "px-1.5 py-0.5 rounded-full text-[10px] font-mono " + (isActive ? "bg-white/20 text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300"),
                    children: count
                  })
                ]
              });
          ...
          c.jsxs("div", {
            ...
            children: [
              c.jsx("input", {
                ...
                placeholder: "Tìm kiếm tên " + curGrpDef.label.toLowerCase() + ", người đại diện, SĐT, địa chỉ...",
                className: "flex-1 px-4 py-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl text-sm outline-none shadow-sm focus:ring-2 focus:ring-blue-500"
              }),
              searchTerm && c.jsx("button", {
                onClick: () => setSearchTerm(""),
                className: "px-3 py-2 bg-gray-200 dark:bg-gray-800 rounded-2xl text-xs font-bold cursor-pointer",
                children: "Xóa"
              })
            ]
          }),
          ...
          c.jsxs("div", {
            className: "p-3.5 rounded-2xl border flex items-center justify-between text-xs " + curGrpDef.badgeBg,
            children: [
              c.jsxs("div", {
                className: "flex items-center gap-2",
                children: [
                  c.jsx("span", { className: "text-xl", children: curGrpDef.icon }),
                  c.jsxs("div", {
                    children: [
                      c.jsx("span", { className: "font-black uppercase block", children: curGrpDef.label }),
                      c.jsx("span", { className: "opacity-80 text-[11px]", children: curGrpDef.sub })
                    ]
                  })
                ]
              }),
              c.jsxs("span", {
                className: "font-bold text-[11px] opacity-75 hidden sm:inline",
                children: ["Tổng: ", filteredList.length, " đối tác"]
              })
            ]
          }),

          filteredList.length === 0 ? c.jsxs("div", {
            className: "text-center py-12 px-4 bg-white dark:bg-gray-900 rounded-3xl border border-dashed border-gray-300 dark:border-gray-800 space-y-3",
            children: [
              c.jsx("div", { className: "text-4xl", children: curGrpDef.icon }),
              c.jsxs("p", { className: "text-gray-500 font-bold text-sm", children: ["Chưa có đối tác nào thuộc nhóm ", curGrpDef.label] }),
              c.jsxs("button", {
                onClick: () => openAddModal(activeGroup),
                className: "px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-black uppercase shadow-md active:scale-95 inline-flex items-center gap-1.5 cursor-pointer",
                children: [
                  c.jsx(As, { size: 14 }),
                  "Thêm " + curGrpDef.shortLabel + " đầu tiên"
                ]
              })
            ]
          }) : c.jsx("div", {
            className: "grid grid-cols-1 md:grid-cols-2 gap-3.5",
            children: filteredList.map(buyer => {
              const prices = buyer.shrimpPrices || [];
              const hasGps = buyer.latitude && buyer.longitude;
              return c.jsxs("div", {
                key: buyer.id,
                className: "bg-white dark:bg-gray-900 rounded-3xl p-4.5 border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-3.5 relative overflow-hidden",
                children: [
                  c.jsxs("div", {
                    className: "space-y-2",
                    children: [
                          ...
                          c.jsxs("div", {
                            ...
                            children: [
                              c.jsxs("div", {
                                className: "flex items-center gap-1.5",
                                children: [
                                  c.jsx("span", { className: "text-base", children: curGrpDef.icon }),
                                  c.jsx("h3", {
                                    className: "font-black text-base text-gray-900 dark:text-white leading-snug",
                                    children: buyer.name
                                  })
                                ]
                              }),
                              buyer.representative && c.jsxs("div", {
                                className: "text-xs text-gray-600 dark:text-gray-400 font-bold mt-1 flex items-center gap-1.5",
                                children: [
                                  c.jsx("span", { className: "opacity-60", children: "👤 Đại diện:" }),
                                  c.jsx("span", { className: "text-gray-800 dark:text-gray-200", children: buyer.representative })
                                ]
                              })
                            ]
                          }),
                          c.jsxs("div", {
                            ...
                            children: [
                              c.jsx("button", {
                                ...
                                className: "p-1.5 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-gray-800 rounded-lg transition-colors cursor-pointer",
                                children: c.jsx("span", { className: "text-xs font-bold", children: "✏️" })
                              }),
                              c.jsx("button", {
                                onClick: () => handleDeleteBuyer(buyer),
                                title: "Xóa đối tác",
                                className: "p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-gray-800 rounded-lg transition-colors cursor-pointer",
                                children: c.jsx(Iu, { size: 15 })
                              })
                            ]
                          })
                        ]
                      ...
                      buyer.phone && c.jsxs("div", {
                        className: "flex items-center gap-2 text-xs",
                        children: [
                          c.jsx("span", { className: "opacity-60", children: "📞 SĐT:" }),
                          c.jsx("a", {
                            href: "tel:" + buyer.phone,
                            className: "font-mono font-black text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-1",
                            children: [
                              buyer.phone,
                              c.jsx("span", { className: "text-[10px] px-1.5 py-0.2 bg-blue-100 dark:bg-blue-900/60 rounded text-blue-700 dark:text-blue-300", children: "Gọi ngay" })
                            ]
                          })
                        ]
                      }),
                      ...
                      c.jsxs("div", {
                        className: "text-xs text-gray-600 dark:text-gray-400 space-y-1.5 pt-1",
                        children: [
                          c.jsxs("div", {
                            className: "flex items-start gap-1.5",
                            children: [
                              c.jsx("span", { className: "opacity-60 shrink-0 mt-0.5", children: "📍 Địa chỉ:" }),
                              c.jsx("span", { className: "font-medium leading-relaxed", children: buyer.address || "Chưa cập nhật địa chỉ" })
                            ]
                          }),
                          c.jsxs("div", {
                            className: "flex flex-wrap items-center gap-2 pt-0.5",
                            children: [
                              // Ẩn toạ độ theo yêu cầu, chỉ hiển thị nút chỉ đường
                              hasGps && c.jsx("span", {
                                className: "inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold border border-emerald-200 dark:border-emerald-800",
                                children: "📍 Đã lưu vị trí GPS"
                              }),
                              c.jsxs("button", {
                                onClick: () => openDirections(buyer),
                                className: "inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/50 hover:bg-blue-100 text-blue-700 dark:text-blue-300 text-[11px] font-black border border-blue-200 dark:border-blue-800 active:scale-95 transition-all cursor-pointer",
                                children: [
                                  c.jsx("span", { children: "🗺️" }),
                      ...
                      activeGroup === "FACTORY" && c.jsxs("div", {
                        className: "mt-2 p-2.5 bg-blue-50/70 dark:bg-blue-950/30 rounded-2xl border border-blue-100 dark:border-blue-900/50 space-y-1.5",
                        children: [
                          c.jsxs("div", {
                            className: "flex items-center justify-between text-[11px] font-black text-blue-900 dark:text-blue-300",
                            children: [
                              c.jsxs("span", { children: ["🦐 Bảng giá tôm xuất bán (", prices.length, " loại):"] }),
                              c.jsx("button", {
                                onClick: () => openEditModal(buyer),
                          ...
                          prices.length === 0 ? c.jsx("p", { className: "text-[11px] text-gray-500 italic", children: "Chưa thiết lập bảng giá loại tôm cho xí nghiệp này." }) : c.jsx("div", {
                            className: "space-y-1 max-h-32 overflow-y-auto pr-1",
                            children: prices.map((sp, sIdx) => c.jsxs("div", {
                              key: sp.id || sIdx,
                              className: "flex justify-between items-center text-xs py-1 px-2 bg-white dark:bg-gray-800/80 rounded-lg shadow-2xs",
                              children: [
                                c.jsx("span", { className: "font-semibold text-gray-700 dark:text-gray-300 truncate max-w-[180px]", children: sp.typeName }),
                                c.jsxs("span", { className: "font-mono font-black text-blue-700 dark:text-blue-400 shrink-0", children: [Number(sp.price || 0).toLocaleString("vi-VN"), " đ/kg"] })
                              ]
                            }))
                          })
                        ]
                      ...
                      activeGroup === "RETAIL" && c.jsxs("div", {
                        className: "mt-2 p-2.5 bg-emerald-50/70 dark:bg-emerald-950/30 rounded-2xl border border-emerald-100 dark:border-emerald-900/50 text-[11px] text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5 font-medium",
                        children: [
                          c.jsx("span", { className: "text-sm", children: "📝" }),
                          "Bán lẻ: Loại tôm và đơn giá được nhập thủ công linh hoạt theo từng đơn bán."
                        ]
                      }),

                      activeGroup === "MARKET" && c.jsxs("div", {
                        className: "mt-2 p-2.5 bg-amber-50/80 dark:bg-amber-950/30 rounded-2xl border border-amber-100 dark:border-amber-900/50 text-xs space-y-1",
                        children: [
                          c.jsx("span", { className: "font-black text-amber-900 dark:text-amber-300 text-[11px] block uppercase", children: "🚛 Thông tin chuyến xe giao chợ:" }),
                          c.jsxs("div", {
                            className: "flex flex-wrap gap-x-4 gap-y-1 text-gray-700 dark:text-gray-300 text-[11px]",
                            children: [
                              c.jsxs("span", { children: ["Biển số: ", c.jsx("b", { className: "font-mono text-gray-900 dark:text-white", children: buyer.defaultVehiclePlate || "Chưa lưu" })] }),
                              c.jsxs("span", { children: ["Tài xế: ", c.jsx("b", { children: buyer.defaultDriverName || "Tài xế chợ" })] }),
                              buyer.defaultDriverPhone && c.jsxs("span", { children: ["SĐT: ", c.jsx("b", { className: "font-mono", children: buyer.defaultDriverPhone })] }),
                              buyer.marketDeliveryTime && c.jsxs("span", { className: "text-amber-700 dark:text-amber-400 font-bold", children: ["⏰ ", buyer.marketDeliveryTime] })
                            ]
                          })
                        ]
                      })
                  ...
                  c.jsxs("div", {
                    className: "pt-2 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between gap-2",
                    children: [
                      buyer.debtAmount > 0 ? c.jsxs("span", {
                        className: "text-[11px] font-bold text-red-600 dark:text-red-400 font-mono",
                        children: ["Nợ: ", Number(buyer.debtAmount).toLocaleString("vi-VN"), " đ"]
                      }) : c.jsx("span", {
                        className: "text-[11px] text-gray-400 font-medium",
                        children: "Công nợ: 0 đ"
                      }),
                      c.jsxs("button", {
                        onClick: () => handleStartSaleForBuyer(buyer),
                        className: "px-3.5 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs font-black uppercase tracking-wider shadow-md shadow-blue-500/20 active:scale-95 transition-all inline-flex items-center gap-1.5 cursor-pointer",
                        children: [
                          c.jsx("span", { children: "⚖️" }),
                          "Xuất Bán Ngay"
                        ]
      ...
      showModal && c.jsx("div", {
        className: "fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 z-50 animate-in fade-in duration-200 overflow-y-auto",
        children: c.jsxs("div", {
          className: "bg-white dark:bg-gray-900 rounded-3xl max-w-lg w-full p-5 shadow-2xl border border-gray-200 dark:border-gray-800 space-y-4 my-auto max-h-[92vh] overflow-y-auto",
          children: [
            c.jsxs("div", {
                ...
                c.jsxs("div", {
                  children: [
                    c.jsx("h3", {
                      ...
                      children: editingBuyer ? "Chỉnh Sửa Đối Tác" : "Thêm Khách Hàng / Đối Tác Mới"
                    }),
                    c.jsx("span", {
                      className: "text-xs text-gray-500 font-medium",
                      children: "Lưu thông tin địa chỉ, toạ độ chỉ đường và bảng giá bán"
                    })
                  ]
                }),
                c.jsx("button", {
                  onClick: () => setShowModal(false),
                  className: "p-2 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-full cursor-pointer",
                  children: c.jsx("span", { className: "text-xl font-bold leading-none", children: "×" })
                })
              ]
            }),
            ...
            c.jsxs("form", {
              ...
              children: [
                c.jsxs("div", {
                  className: "space-y-1.5",
                  children: [
                    c.jsx("label", { className: "text-xs font-bold text-gray-600 dark:text-gray-400 uppercase", children: "Phân nhóm khách hàng:" }),
                    c.jsx("div", {
                      className: "grid grid-cols-3 gap-2",
                      children: Object.values(SALE_GROUPS).map(grp => {
                        const isSel = formData.targetGroup === grp.id;
                        return c.jsxs("button", {
                          type: "button",
                          key: grp.id,
                          onClick: () => {
                            setFormData(prev => ({
                              ...prev,
                              targetGroup: grp.id,
                              shrimpPrices: grp.id === "FACTORY" && (!prev.shrimpPrices || prev.shrimpPrices.length === 0)
                                ? [
                                    { id: "sp_1", typeName: "Tôm càng sen size lớn (10-15 c/kg)", price: 220000 },
                                    { id: "sp_2", typeName: "Tôm càng xô công nghiệp", price: 175000 }
                                  ]
                          ...
                          className: "p-2 rounded-xl text-xs font-black uppercase border flex flex-col items-center gap-1 transition-all cursor-pointer " +
                            (isSel ? (grp.id === "FACTORY" ? "bg-blue-600 text-white border-blue-600 shadow-md" : (grp.id === "RETAIL" ? "bg-emerald-600 text-white border-emerald-600 shadow-md" : "bg-amber-600 text-white border-amber-600 shadow-md")) : "bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700"),
                          children: [
                            c.jsx("span", { className: "text-lg", children: grp.icon }),
                            c.jsx("span", { className: "text-[11px]", children: grp.shortLabel })
                          ]
                        });
                      })
                    })
                ...
                c.jsxs("div", {
                  className: "space-y-1",
                  children: [
                    c.jsxs("label", { className: "text-xs font-bold text-gray-700 dark:text-gray-300", children: [
                      formData.targetGroup === "FACTORY" ? "Tên Xí nghiệp / Nhà máy:" : (formData.targetGroup === "MARKET" ? "Tên Vựa tôm / Sạp cá chợ:" : "Tên Cửa hàng / Khách mua lẻ:"),
                      c.jsx("span", { className: "text-red-500 ml-1", children: "*" })
                    ] }),
                    c.jsx("input", {
                      type: "text",
                      required: true,
                      ...
                      placeholder: formData.targetGroup === "FACTORY" ? "VD: Công ty CP Thủy sản Camimex" : (formData.targetGroup === "MARKET" ? "VD: Vựa tôm Cô Tư (Chợ Bình Điền)" : "VD: Nhà hàng Biển Xanh / Anh Ba Cà Mau"),
                      className: "w-full p-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500"
                    })
                  ]
                }),
                ...
                c.jsxs("div", {
                  ...
                  children: [
                    c.jsxs("div", {
                      className: "space-y-1",
                      children: [
                        c.jsx("label", { className: "text-xs font-bold text-gray-700 dark:text-gray-300", children: "Tên người đại diện:" }),
                        c.jsx("input", {
                          type: "text",
                          value: formData.representative,
                          onChange: e => setFormData({ ...formData, representative: e.target.value }),
                          placeholder: "VD: Nguyễn Văn A (Quản lý)",
                          className: "w-full p-2 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500"
                        })
                      ]
                    }),
                    c.jsxs("div", {
                      className: "space-y-1",
                      children: [
                        c.jsx("label", { className: "text-xs font-bold text-gray-700 dark:text-gray-300", children: "Số điện thoại:" }),
                        c.jsx("input", {
                          type: "tel",
                          value: formData.phone,
                          onChange: e => setFormData({ ...formData, phone: e.target.value }),
                          placeholder: "VD: 0918.xxx.xxx",
                          className: "w-full p-2 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-sm font-mono outline-none focus:ring-2 focus:ring-blue-500"
                        })
                      ]
                    })
                  ]
                ...
                c.jsxs("div", {
                  className: "p-3.5 bg-blue-50/50 dark:bg-gray-800/60 rounded-2xl border border-blue-100 dark:border-gray-700 space-y-2.5",
                  children: [
                    c.jsxs("div", {
                      className: "flex justify-between items-center",
                      children: [
                        c.jsx("label", { className: "text-xs font-black uppercase text-blue-900 dark:text-blue-300 flex items-center gap-1", children: [
                          c.jsx("span", { children: "📍" }),
                          "Địa chỉ & Ghim toạ độ chỉ đường:"
                        ] }),
                        c.jsxs("button", {
                          type: "button",
                          onClick: handleGetGps,
                          disabled: gpsLoading,
                          className: "px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[11px] font-bold shadow-xs active:scale-95 transition-all inline-flex items-center gap-1 cursor-pointer",
                          children: [
                            gpsLoading ? c.jsx("span", { className: "animate-spin", children: "⏳" }) : c.jsx("span", { children: "🎯" }),
                            gpsLoading ? "Đang dò GPS..." : "Lấy GPS hiện tại"
                          ]
                        })
                      ]
                    }),
                    c.jsx("input", {
                      ...
                      className: "w-full p-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl text-sm outline-none"
                    }),
                    // Đã ẩn 2 ô kinh độ và vĩ độ theo yêu cầu - GPS được tự động lưu ngầm qua nút Lấy GPS
                    gpsSuccess && c.jsx("p", {
                      className: "text-[11px] text-emerald-600 dark:text-emerald-400 font-bold",
                      children: "✓ Đã bắt toạ độ GPS chính xác! Chỉ đường Maps sẽ dẫn thẳng tới điểm này."
                    })
                  ]
                }),

                formData.targetGroup === "FACTORY" && c.jsxs("div", {
                  className: "p-3.5 bg-indigo-50/50 dark:bg-indigo-950/30 rounded-2xl border border-indigo-200 dark:border-indigo-900/60 space-y-2.5",
                  children: [
                    c.jsxs("div", {
                      className: "flex justify-between items-center",
                      children: [
                        c.jsxs("div", {
                          children: [
                            c.jsx("label", { className: "text-xs font-black uppercase text-indigo-900 dark:text-indigo-300 block", children: "🦐 Bảng giá loại tôm của xí nghiệp:" }),
                            c.jsx("span", { className: "text-[10px] text-gray-500 dark:text-gray-400", children: "Khi cân bán, hệ thống sẽ tự nạp giá từng loại tôm này để chọn 1 chạm" })
                          ]
                        }),
                        c.jsxs("button", {
                          type: "button",
                          onClick: () => addShrimpPriceRow("", 200000),
                          className: "px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold inline-flex items-center gap-1 cursor-pointer",
                          children: [
                            c.jsx(As, { size: 12 }),
                            "Thêm loại"
                          ]
                    ...
                    c.jsxs("div", {
                      ...
                      children: [
                        c.jsx("span", { className: "text-[10px] text-gray-500 font-bold self-center", children: "Gợi ý nhanh:" }),
                        [
                          { name: "Tôm càng sen lớn (10-15 c/kg)", price: 220000 },
                          { name: "Tôm càng xô", price: 175000 },
                          { name: "Tôm lột cấp đông", price: 140000 },
                          { name: "Tôm thẻ size 30", price: 165000 }
                        ].map((preset, pIdx) => c.jsx("button", {
                          key: pIdx,
                          type: "button",
                          onClick: () => addShrimpPriceRow(preset.name, preset.price),
                          className: "px-2 py-0.5 bg-white dark:bg-gray-800 hover:bg-indigo-50 text-[10px] rounded-md border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-medium cursor-pointer",
                    ...
                    c.jsx("div", {
                      ...
                      children: (formData.shrimpPrices || []).map((sp, idx) => c.jsxs("div", {
                        key: sp.id || idx,
                        ...
                        children: [
                          c.jsx("input", {
                            type: "text",
                            value: sp.typeName,
                            onChange: e => updateShrimpPriceRow(idx, "typeName", e.target.value),
                            placeholder: "Tên loại tôm (VD: Sen lớn, Xô...)",
                            className: "flex-1 text-xs font-bold p-1.5 bg-transparent border-b border-gray-300 dark:border-gray-700 outline-none"
                          }),
                          c.jsxs("div", {
                            ...
                            children: [
                              c.jsx("input", {
                                ...
                                step: 1000,
                                value: sp.price,
                                onChange: e => updateShrimpPriceRow(idx, "price", e.target.value),
                                placeholder: "Giá",
                                className: "w-24 text-xs font-mono font-black text-right p-1.5 bg-transparent border-b border-gray-300 dark:border-gray-700 outline-none text-blue-600 dark:text-blue-400"
                              }),
                              c.jsx("span", { className: "text-[10px] text-gray-500 font-bold", children: "đ/kg" })
                            ]
                          }),
                          c.jsx("button", {
                            type: "button",
                            onClick: () => removeShrimpPriceRow(idx),
                            className: "p-1 text-gray-400 hover:text-red-600 rounded cursor-pointer",
                            children: c.jsx(Iu, { size: 14 })
                          })
                        ]
                      }))
                    })
                ...
                formData.targetGroup === "RETAIL" && c.jsxs("div", {
                  className: "p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-2xl border border-emerald-200 dark:border-emerald-800/60 text-xs text-emerald-800 dark:text-emerald-300 space-y-1",
                  children: [
                    c.jsx("span", { className: "font-black block", children: "💡 Quy cách Khách hàng Bán Lẻ:" }),
                    c.jsx("p", { children: "Đối với khách hàng bán lẻ, loại tôm và giá bán sẽ được nhập thủ công linh hoạt theo từng đơn xuất tôm (hoặc chọn nhanh theo nhu cầu thực tế)." })
                  ]
                }),

                formData.targetGroup === "MARKET" && c.jsxs("div", {
                  className: "p-3.5 bg-amber-50/70 dark:bg-amber-950/30 rounded-2xl border border-amber-200 dark:border-amber-900/60 space-y-2.5",
                  children: [
                    c.jsx("label", { className: "text-xs font-black uppercase text-amber-900 dark:text-amber-300 block", children: "🚛 Thông tin giao hàng cho vựa chợ:" }),
                    c.jsxs("div", {
                      className: "grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs",
                      children: [
                        c.jsxs("div", {
                          children: [
                            c.jsx("span", { className: "text-[11px] font-bold text-gray-700 dark:text-gray-300", children: "Biển số xe đi giao chợ:" }),
                            c.jsx("input", {
                              type: "text",
                              value: formData.defaultVehiclePlate,
                              onChange: e => setFormData({ ...formData, defaultVehiclePlate: e.target.value }),
                              placeholder: "VD: 69C-088.99",
                              className: "w-full p-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl font-mono uppercase font-bold"
                            })
                          ]
                        }),
                        c.jsxs("div", {
                          children: [
                            c.jsx("span", { className: "text-[11px] font-bold text-gray-700 dark:text-gray-300", children: "Số điện thoại tài xế:" }),
                            c.jsx("input", {
                              type: "tel",
                              value: formData.defaultDriverPhone,
                              onChange: e => setFormData({ ...formData, defaultDriverPhone: e.target.value }),
                              placeholder: "VD: 0919.444.555",
                              className: "w-full p-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl font-mono"
                            })
                          ]
                        }),
                        c.jsxs("div", {
                          children: [
                            c.jsx("span", { className: "text-[11px] font-bold text-gray-700 dark:text-gray-300", children: "Tên tài xế giao hàng:" }),
                            c.jsx("input", {
                              type: "text",
                              value: formData.defaultDriverName,
                              onChange: e => setFormData({ ...formData, defaultDriverName: e.target.value }),
                              placeholder: "VD: Tài xế Ba Chở Hàng",
                              className: "w-full p-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl"
                            })
                          ]
                        }),
                        c.jsxs("div", {
                          children: [
                            c.jsx("span", { className: "text-[11px] font-bold text-gray-700 dark:text-gray-300", children: "Khung giờ giao / chuyến xe:" }),
                            c.jsx("input", {
                              type: "text",
                              value: formData.marketDeliveryTime,
                              onChange: e => setFormData({ ...formData, marketDeliveryTime: e.target.value }),
                              placeholder: "VD: Chuyến đêm 2h sáng",
                              className: "w-full p-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl"
                            })
                          ]
                        })
                ...
                c.jsxs("div", {
                  className: "space-y-1",
                  children: [
                    c.jsx("label", { className: "text-xs font-bold text-gray-700 dark:text-gray-300", children: "Ghi chú thêm:" }),
                    c.jsx("textarea", {
                      rows: 2,
                      value: formData.note,
                      onChange: e => setFormData({ ...formData, note: e.target.value }),
                      placeholder: "Ghi chú thêm về yêu cầu kiểm hàng, giao nhận...",
                      className: "w-full p-2 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-xs outline-none"
                    })
                  ]
                }),

                c.jsxs("div", {
                  className: "pt-3 flex gap-3 border-t border-gray-100 dark:border-gray-800",
                  children: [
                    c.jsx("button", {
                      type: "button",
                      onClick: () => setShowModal(false),
                      className: "flex-1 py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-bold rounded-xl text-xs uppercase active:scale-95 transition-all cursor-pointer",
                      children: "Hủy bỏ"
                    }),
                    c.jsx("button", {
                      type: "submit",
                      className: "flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl text-xs uppercase tracking-wider shadow-md shadow-blue-500/30 active:scale-95 transition-all cursor-pointer",
                      children: editingBuyer ? "Lưu Thay Đổi" : "Tạo Đối Tác Mới"
                    })
                  ]
                })
              ]
...
// ==================== SCREEN 2: Xm_SalesScreen ====================
const Xm_SalesScreen = ({ setRoute, onViewDetail, role }) => {
  // Formatters
  const fmtKg = (n) => {
    if (!n || n <= 0) return "0";
    const r = Math.round(n * 100) / 100;
  ...
  // Target group: FACTORY | RETAIL | MARKET
  const [targetGroup, setTargetGroup] = w.useState(() => {
    try {
      return localStorage.getItem("last_sale_target_group") || "FACTORY";
    } catch(e) {
      return "FACTORY";
    }
  });

  // Mode: true = Cấu hình thông số (Parameters), false = Bàn cân 5x5 đang cân (Active Weighing Grid)
  const [O, D] = w.useState(true);
  const [promptReq, setPromptReq] = w.useState(null);

  // Lists from local storage
  const [salesList, setSalesList] = w.useState([]);
  const [buyersList, setBuyersList] = w.useState([]);
  const refreshData = () => {
    try {
      setSalesList(me.getSales());
      setBuyersList(me.getBuyers());
    } catch(e) {}
  };
  w.useEffect(() => {
  ...
  // Modals & dropdowns
  const [showSettlementModal, setShowSettlementModal] = w.useState(false);
  const [showHistoryModal, setShowHistoryModal] = w.useState(false);
  const [showBuyerDropdown, setShowBuyerDropdown] = w.useState(false);
  const [buyerSearch, setBuyerSearch] = w.useState("");
  const [showShrimpDropdown, setShowShrimpDropdown] = w.useState(false);

  // Selected buyer object for factory custom price list
  const [selectedBuyerObj, setSelectedBuyerObj] = w.useState(null);

  // Payment status: true = Đã thu đủ 100%, false = Ghi sổ nợ 100%
  const [Q, se] = w.useState(true);

  // Active parameter state (KHÔNG TRỪ BÌ - KHÔNG CỌC)
  const [A, v] = w.useState(() => {
    try {
      const stored = localStorage.getItem("temp_sale_weighing_info_v3");
      if (stored) {
        const parsed = JSON.parse(stored);
    ...
    return {
      targetGroup: "FACTORY",
      buyerName: "Công ty CP Thủy sản Camimex",
      buyerPhone: "0290.3838383",
      buyerAddress: "KCN Phường 8, TP. Cà Mau, Tỉnh Cà Mau",
      buyerRepresentative: "Nguyễn Văn Cảnh (GĐ Thu mua)",
      buyerLatitude: 9.1768,
      buyerLongitude: 105.1524,
      contractCode: "",
      riceType: "Tôm càng sen size lớn (10-15 c/kg)",
      price: 220000,
      vehiclePlate: "",
      driverPhone: "",
      driverName: "",
      marketDeliveryTime: "",
      tareSpec: "Bán ra KHÔNG TRỪ BÌ (100% Cân thực tế = Tính tiền)"
    };
  });
  ...
  w.useEffect(() => {
    try {
      localStorage.setItem("temp_sale_weighing_info_v3", JSON.stringify({ ...A, targetGroup }));
    } catch(e) {}
  }, [A, targetGroup]);

  // Handle buyer select
  const handleSelectBuyer = (buyer) => {
    const grpKey = buyer.targetGroup || "FACTORY";
    setTargetGroup(grpKey);
    setSelectedBuyerObj(buyer);
    try {
      localStorage.setItem("last_sale_target_group", grpKey);
    } catch(e) {}

    let shrimpName = A.riceType;
    let shrimpPrice = A.price;
    if (grpKey === "FACTORY" && Array.isArray(buyer.shrimpPrices) && buyer.shrimpPrices.length > 0) {
      shrimpName = buyer.shrimpPrices[0].typeName;
      shrimpPrice = buyer.shrimpPrices[0].price;
    }
    ...
    v(prev => ({
      ...
      buyerLatitude: buyer.latitude || null,
      buyerLongitude: buyer.longitude || null,
      riceType: shrimpName,
      price: shrimpPrice,
      vehiclePlate: buyer.defaultVehiclePlate || prev.vehiclePlate || "",
      driverPhone: buyer.defaultDriverPhone || prev.driverPhone || "",
      driverName: buyer.defaultDriverName || prev.driverName || "",
      marketDeliveryTime: buyer.marketDeliveryTime || prev.marketDeliveryTime || ""
    }));
    setShowBuyerDropdown(false);
  };

  // Check preselected buyer from Customers screen
  w.useEffect(() => {
    try {
      const preselectedStr = localStorage.getItem("preselected_buyer_for_sale");
      if (preselectedStr) {
        const b = JSON.parse(preselectedStr);
        if (b && b.name) {
          localStorage.removeItem("preselected_buyer_for_sale");
          handleSelectBuyer(b);
        }
      }
    } catch(e) {}
  ...
  // Multi-type completed batches in current sale ticket
  const [completedBatches, setCompletedBatches] = w.useState([]);

  // Active 5x5 weighing tables for current shrimp type
  const [_, z] = w.useState([Array(5).fill(0).map(() => Array(5).fill(0))]);

  // Calculations for current active table - KHÔNG TRỪ BÌ (Gross = Net)
  const Oe = w.useMemo(() => _.flat(2).filter($ => $ > 0), [_]);
  const De = w.useMemo(() => Math.round(Oe.reduce(($, Te) => $ + Te, 0) * 100) / 100, [Oe]); // Gross kg
  const J = w.useMemo(() => Oe.length, [Oe]); // Bales / baskets count
  const tareWeight = 0; // Market tare is resolved later
  const currentNet = Math.round((De - tareWeight) * 100) / 100;
  const memoizedCurrentMoney = w.useMemo(() => Math.round(currentNet * Number(A.price || 0)), [currentNet, A.price]);
  const currentMoney = targetGroup === "MARKET" ? 0 : memoizedCurrentMoney;

  // Aggregate sums across completed batches + active batch
  const totalNetAllTypes = w.useMemo(() => {
    const prevNet = completedBatches.reduce((acc, b) => acc + (b.net || 0), 0);
    return Math.round((prevNet + currentNet) * 100) / 100;
  }, [completedBatches, currentNet]);
  ...
  const totalGrossAllTypes = w.useMemo(() => {
    const prevGross = completedBatches.reduce((acc, b) => acc + (b.gross || 0), 0);
    return Math.round((prevGross + De) * 100) / 100;
  }, [completedBatches, De]);
  ...
  // Switch target group
  const handleSelectTargetGroup = (groupKey) => {
    setTargetGroup(groupKey);
    try {
      localStorage.setItem("last_sale_target_group", groupKey);
    } catch(e) {}
    const buyersInGroup = buyersList.filter(b => (b.targetGroup || "FACTORY") === groupKey);
    if (buyersInGroup.length > 0) {
      handleSelectBuyer(buyersInGroup[0]);
    } else {
      setSelectedBuyerObj(null);
      const def = SALE_GROUPS[groupKey];
      const firstShrimp = def && def.defaultShrimpTypes && def.defaultShrimpTypes[0];
      v(prev => ({
        ...prev,
        targetGroup: groupKey,
        buyerName: groupKey === "FACTORY" ? "Xí nghiệp Thủy sản" : (groupKey === "MARKET" ? "Vựa tôm chợ đầu mối" : "Khách lẻ tại vựa"),
        buyerPhone: "",
        buyerAddress: "",
        buyerRepresentative: "",
        buyerLatitude: null,
        buyerLongitude: null,
        riceType: firstShrimp ? firstShrimp.typeName : prev.riceType,
        price: firstShrimp ? firstShrimp.price : prev.price,
        vehiclePlate: "",
        driverPhone: "",
        driverName: "",
        marketDeliveryTime: ""
  ...
  // Available shrimp types list
  const availableShrimpTypes = w.useMemo(() => {
    if (targetGroup === "FACTORY" && selectedBuyerObj && Array.isArray(selectedBuyerObj.shrimpPrices) && selectedBuyerObj.shrimpPrices.length > 0) {
      return selectedBuyerObj.shrimpPrices.map(sp => ({
        name: sp.typeName,
        price: sp.price,
        desc: "Đơn giá theo hợp đồng xí nghiệp"
      }));
    }
    const def = SALE_GROUPS[targetGroup];
    if (def && def.defaultShrimpTypes) {
      return def.defaultShrimpTypes.map(st => ({
        name: st.typeName,
        price: st.price,
        desc: st.desc || ""
      }));
    ...
    return [
      { name: "Tôm càng sen size lớn (10-15 c/kg)", price: 220000, desc: "Hàng xuất khẩu loại 1" },
      { name: "Tôm càng sen size trung (16-25 c/kg)", price: 180000, desc: "Hàng chợ / nhà hàng" },
      { name: "Tôm càng xào (size 30-40 c/kg)", price: 135000, desc: "Hàng tiêu thụ nội địa" },
      { name: "Tôm ngộp / ướp đá", price: 110000, desc: "Đông lạnh bảo quản đá" }
    ];
  }, [targetGroup, selectedBuyerObj]);

  // Chuyển loại tôm cân tiếp (Multi-shrimp batching)
  const handleSwitchShrimpType = () => {
    if (currentNet > 0 || J > 0) {
      const newBatch = {
        typeName: A.riceType,
  ...
  // Top-right button: "Cân" (Switch to weigh mode) or "Lưu" (Open Settlement)
  const Ue = () => {
    if (O) {
      D(false);
      setTimeout(() => {
  ...
  const handleConfirmSaveSession = () => {
    ...
    const grp = SALE_GROUPS[targetGroup] || SALE_GROUPS.FACTORY;
    const codePrefix = grp.id === "FACTORY" ? "XN" : (grp.id === "MARKET" ? "CHO" : "BL");
    const paidAmount = Q ? totalMoneyFinal : 0;
    const remainingDebt = Q ? 0 : totalMoneyFinal;
    ...
    const newSale = {
      ...
      targetGroupLabel: grp.label,
      buyerName: A.buyerName || (targetGroup === "FACTORY" ? "Xí nghiệp tôm" : (targetGroup === "MARKET" ? "Vựa tôm chợ" : "Khách bán lẻ")),
      buyerPhone: A.buyerPhone || "",
      buyerAddress: A.buyerAddress || "",
      buyerRepresentative: A.buyerRepresentative || "",
      buyerLatitude: A.buyerLatitude || null,
      buyerLongitude: A.buyerLongitude || null,
      ...
      totalGross: totalGrossAllTypes,
      totalTare: completedBatches.reduce((acc, b) => acc + (b.tare || 0), 0) + tareWeight,
      totalNet: totalNetAllTypes,
      totalBales: totalBalesAllTypes,
      totalMoney: totalMoneyFinal,
      weights: _,
      batches: allBatches,
      notes: "Phiếu cân xuất bán không trừ bì (100% Cân = Tính tiền)"
    };
    ...
    alert("✅ Đã lưu thành công phiếu xuất bán: " + newSale.code + " (" + newSale.buyerName + ")");
  };
  ...
  const handleShareZalo = () => {
    ...
    const grp = SALE_GROUPS[targetGroup] || SALE_GROUPS.FACTORY;
    let msg = "🦐 [PHIẾU XUẤT BÁN TÔM - " + grp.label.toUpperCase() + "]\n";
    msg += "Khách hàng: " + A.buyerName + (A.buyerPhone ? " (" + A.buyerPhone + ")" : "") + "\n";
    if (A.buyerRepresentative) msg += "Đại diện: " + A.buyerRepresentative + "\n";
    if (targetGroup === "MARKET" && A.vehiclePlate) msg += "Biển số xe: " + A.vehiclePlate + (A.driverPhone ? " (SĐT: " + A.driverPhone + ")" : "") + "\n";
    msg += "Ngày xuất: " + new Date().toLocaleDateString("vi-VN") + "\n";
    msg += "Quy cách: KHÔNG TRỪ BÌ (100% Cân thực tế)\n";
    msg += "-----------------------------\n";
    allBatches.forEach((b, idx) => {
      msg += (idx + 1) + ". " + b.typeName + ": " + fmtKg(b.net) + " kg x " + Number(b.unitPrice).toLocaleString() + " đ = " + Number(b.money).toLocaleString() + " đ\n";
    });
    msg += "-----------------------------\n";
    msg += "Tổng sản lượng xuất: " + fmtKg(totalNetAllTypes) + " kg (" + totalBalesAllTypes + " rổ)\n";
    msg += "TỔNG TIỀN HÀNG: " + _e(totalMoneyFinal) + " VNĐ\n";
    msg += "(" + numberToWordsVN_v3(totalMoneyFinal) + ")\n";
    msg += "Trạng thái: " + (Q ? "ĐÃ THU ĐỦ 100%" : "GHI SỔ NỢ 100%") + "\n";
    msg += "Kính chúc Quý đối tác kinh doanh phát tài!";
  ...
  return c.jsxs("div", {
    className: "flex flex-col h-screen bg-[#f1f3f4] dark:bg-[#121212] font-roboto select-none overflow-hidden transition-colors",
    children: [
      // Top Header: EXACT OM PREVIOUS STYLE
      c.jsxs("div", {
        className: "bg-[#1a237e] dark:bg-[#0d134d] px-4 py-3 flex items-center justify-between shadow-xl z-50",
        children: [
          // Back button
          c.jsx("button", {
            onClick: () => {
              setRoute(re.DASHBOARD);
            },
            className: "p-2 text-white bg-white/10 rounded-2xl active:scale-90",
            children: c.jsx(On, { size: 24 })
          }),
          // Center Title & Buyer Badge
          c.jsxs("div", {
            className: "flex flex-col items-center flex-1 mx-3 text-center",
            children: [
              c.jsx("span", {
                className: "font-black text-white text-lg uppercase italic truncate w-full tracking-tighter",
                children: A.buyerName || "Khách Mua Tôm"
              }),
              c.jsxs("div", {
                className: "text-[9px] font-black text-blue-200 uppercase italic bg-white/10 px-2 py-0.5 rounded-full mt-1 flex items-center gap-1",
                children: [
                  targetGroup === "FACTORY" && c.jsxs(c.Fragment, { children: ["🏢 ", A.buyerRepresentative ? "Xí nghiệp: " + A.buyerRepresentative : "Xí Nghiệp Tôm Xuất Khẩu"] }),
                  targetGroup === "MARKET" && c.jsxs(c.Fragment, { children: [c.jsx(tc, { size: 10 }), " ", A.vehiclePlate ? "Xe: " + A.vehiclePlate : "🏪 Chợ Thủy Hải Sản"] }),
                  targetGroup === "RETAIL" && c.jsxs(c.Fragment, { children: ["🛒 Khách Bán Lẻ Tại Vựa"] })
                ]
              })
            ]
          }),
          // Right Button: Cân / Lưu (EXACT OM STYLE)
          c.jsxs("button", {
            onClick: Ue,
            className: `h-12 px-5 rounded-2xl flex items-center gap-2 font-black shadow-lg transition-all active:scale-95 border-b-4 ${
              O
      ...
      // Sub-bar under Header: Target Group Segment & Shortcut Buttons
      c.jsxs("div", {
        className: "bg-[#141b63] dark:bg-[#080d38] px-4 py-2 flex items-center justify-between gap-2 border-t border-white/10 shadow-sm shrink-0",
        children: [
          // 3 Target Groups (Segmented Pill Switcher)
          c.jsxs("div", {
            className: "flex items-center gap-1 bg-black/20 p-1 rounded-2xl",
            children: [
              c.jsxs("button", {
                type: "button",
                onClick: () => handleSelectTargetGroup("FACTORY"),
                className: `px-3 py-1.5 rounded-xl text-xs font-black uppercase italic transition-all flex items-center gap-1.5 ${
                  targetGroup === "FACTORY"
                    ? "bg-yellow-400 text-blue-950 shadow-md scale-[1.02]"
                    : "text-blue-100 hover:bg-white/10"
                }`,
                children: [c.jsx("span", { children: "🏢" }), "Xí Nghiệp"]
              }),
              c.jsxs("button", {
                type: "button",
                onClick: () => handleSelectTargetGroup("RETAIL"),
                className: `px-3 py-1.5 rounded-xl text-xs font-black uppercase italic transition-all flex items-center gap-1.5 ${
                  targetGroup === "RETAIL"
                    ? "bg-yellow-400 text-blue-950 shadow-md scale-[1.02]"
                    : "text-blue-100 hover:bg-white/10"
                }`,
                children: [c.jsx("span", { children: "🛒" }), "Bán Lẻ"]
              }),
              c.jsxs("button", {
                type: "button",
                onClick: () => handleSelectTargetGroup("MARKET"),
                className: `px-3 py-1.5 rounded-xl text-xs font-black uppercase italic transition-all flex items-center gap-1.5 ${
                  targetGroup === "MARKET"
                    ? "bg-yellow-400 text-blue-950 shadow-md scale-[1.02]"
                    : "text-blue-100 hover:bg-white/10"
                }`,
                children: [c.jsx("span", { children: "🏪" }), "Chợ"]
              })
            ]
          }),
          // History & Customers Quick Links
          c.jsxs("div", {
            className: "flex items-center gap-1.5",
            children: [
              c.jsxs("button", {
                ...
                className: "px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-bold uppercase italic flex items-center gap-1 active:scale-95 transition-all",
                children: [c.jsx(Fd, { size: 14 }), "Lịch Sử"]
              }),
              c.jsxs("button", {
                type: "button",
                onClick: () => setRoute(re.CUSTOMERS),
                className: "px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-bold uppercase italic flex items-center gap-1 active:scale-95 transition-all",
                children: [c.jsx(Os, { size: 14 }), "Khách Hàng"]
              })
            ]
          })
        ]
      ...
      // Main Scroll Area: EXACT OM STYLE
      c.jsxs("div", {
        className: "flex-1 overflow-y-auto bg-white dark:bg-[#181818] scroll-smooth pb-20 transition-colors",
        children: [
          // Parameters Panel: EXACT OM STYLE
          c.jsxs("div", {
            className: `p-4 space-y-4 transition-all duration-500 ${!O && p.lockParamsDuringWeighing ? "opacity-30 pointer-events-none" : ""}`,
            children: [
              // 2 NÚT BẤM CÓ DANH MỤC THẢ XUỐNG: KHÁCH HÀNG / ĐỐI TÁC & LOẠI TÔM
              c.jsxs("div", {
                className: "grid grid-cols-2 gap-4 relative z-20",
                children: [
                  // Nút 1: Chọn Khách Hàng / Đối Tác
                  c.jsxs("div", {
                    className: "space-y-1 relative",
                    children: [
                      c.jsxs("div", {
                        ...
                        children: [
                          c.jsx("label", {
                            className: "text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase italic",
                            children: targetGroup === "FACTORY" ? "Xí Nghiệp Thu Mua" : (targetGroup === "MARKET" ? "Vựa Chợ Đầu Mối" : "Khách Hàng Bán Lẻ")
                          }),
                          c.jsx("button", {
                            type: "button",
                            onClick: () => {
                              setPromptReq({ title: "Nhập tên khách hàng mới hoặc đối tác:", defaultVal: A.buyerName || "", onConfirm: (newName) => {
                                if (newName && newName.trim()) v({ ...A, buyerName: newName.trim() });
                              }});
                            },
                            className: "text-[9px] font-black text-blue-600 dark:text-blue-400 uppercase italic underline",
                            children: "Đổi tên"
                        ...
                        children: [
                          c.jsxs("div", {
                            ...
                            children: [
                              c.jsx("div", {
                                className: "text-base font-black text-gray-800 dark:text-gray-100 uppercase truncate",
                                children: A.buyerName || "Chọn Khách Hàng..."
                              }),
                              c.jsx("div", {
                                className: "text-[10px] font-bold text-gray-400 font-mono truncate",
                                children: A.buyerPhone || A.buyerRepresentative || "Khách mua tự do"
                              })
                            ]
                          }),
                          c.jsx("span", {
                            className: `text-xs font-black text-gray-500 transition-transform duration-200 ${showBuyerDropdown ? "rotate-180" : ""}`,
                            children: "▼"
                          })
                        ]
                      }),
                      // Dropdown Menu List
                      showBuyerDropdown && c.jsxs("div", {
                        className: "absolute left-0 right-0 top-full mt-1.5 z-[90] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 max-h-80 overflow-y-auto p-2 space-y-1.5",
                        children: [
                          // Search Box in Dropdown
                          c.jsxs("div", {
                          ...
                          // Option 1: Quick Custom Buyer
                          c.jsxs("button", {
                            type: "button",
                            onClick: () => {
                              setPromptReq({ title: "Nhập tên khách hàng tự do:", defaultVal: targetGroup === "RETAIL" ? "Khách Lẻ Mua Tại Vựa" : "Khách Mua Tự Do", onConfirm: (customName) => {
                                if (customName) v(prev => ({ ...prev, buyerName: customName, buyerPhone: "", buyerRepresentative: "" }));
                              }});
                              setShowBuyerDropdown(false);
                            },
                            className: "w-full p-2.5 bg-yellow-50 dark:bg-yellow-950/30 hover:bg-yellow-100 rounded-xl text-left flex items-center gap-2 border border-yellow-200 dark:border-yellow-800/40 text-yellow-900 dark:text-yellow-200",
                            children: [
                              ...
                              c.jsxs("div", {
                                children: [
                                  c.jsx("div", { className: "font-black text-xs uppercase", children: "+ Nhập Khách Mới / Tự Do" }),
                                  c.jsx("div", { className: "text-[9px] text-yellow-700 dark:text-yellow-400", children: "Cân nhanh không cần lưu danh bạ" })
                                ]
                              })
                            ]
                          ...
                          buyersList.filter(b => {
                            ...
                            return (b.targetGroup || "FACTORY") === targetGroup;
                          }).map(b => c.jsxs("button", {
                            key: b.id || b.name,
                            type: "button",
                            ...
                            children: [
                              c.jsxs("div", {
                                className: "flex justify-between items-start",
                                children: [
                                  c.jsx("div", { className: "font-black text-xs uppercase text-gray-800 dark:text-gray-100", children: b.name }),
                                  c.jsx("span", {
                                    className: "text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-100 text-blue-800 dark:bg-blue-950/60 dark:text-blue-300",
                                    children: b.targetGroup === "FACTORY" ? "Xí Nghiệp" : (b.targetGroup === "MARKET" ? "Chợ" : "Bán Lẻ")
                                  })
                                ]
                              }),
                              c.jsxs("div", {
                                className: "text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-2 truncate",
                                children: [
                                  b.phone && c.jsxs("span", { children: ["📞 ", b.phone] }),
                                  b.representative && c.jsxs("span", { className: "text-blue-600 dark:text-blue-400 font-bold truncate", children: ["👤 ", b.representative] })
                                ]
                              })
                            ]
                          }))
                  ...
                  c.jsxs("div", {
                    ...
                    children: [
                      c.jsxs("div", {
                        ...
                        children: [
                          ...
                          c.jsx("button", {
                            type: "button",
                            onClick: () => {
                              setPromptReq({ title: "Nhập tên loại tôm mới:", defaultVal: A.riceType || "", onConfirm: (customType) => {
                                if (customType && customType.trim()) {
                                  setTimeout(() => {
                                    if (targetGroup === "MARKET") {
                                      v(prev => ({ ...prev, riceType: customType.trim(), price: 0 }));
                                    } else {
                                      setPromptReq({ title: "Nhập đơn giá xuất bán (đ/kg):", defaultVal: String(A.price || "180000"), onConfirm: (customPrice) => {
                                        v(prev => ({ ...prev, riceType: customType.trim(), price: Number(customPrice) || prev.price }));
                                      }});
                                    }
                                  }, 100);
                                }
                      ...
                      showShrimpDropdown && c.jsxs("div", {
                        className: "absolute left-0 right-0 top-full mt-1.5 z-[90] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 max-h-80 overflow-y-auto p-2 space-y-1 animate-in fade-in",
                        children: [
                          availableShrimpTypes.map(item => c.jsxs("button", {
                            key: item.name,
                            type: "button",
                            onClick: () => {
                              v(prev => ({
                          ...
                          c.jsxs("button", {
                            type: "button",
                            onClick: () => {
                              setPromptReq({ title: "Nhập tên loại tôm khác:", defaultVal: "", onConfirm: (customName) => {
                                if (customName && customName.trim()) {
                                  setTimeout(() => {
                                    if (targetGroup === "MARKET") {
                                      v(prev => ({ ...prev, riceType: customName.trim(), price: 0 }));
                                    } else {
                                      setPromptReq({ title: "Nhập đơn giá bán (đ/kg):", defaultVal: "180000", onConfirm: (customPrice) => {
                                        v(prev => ({ ...prev, riceType: customName.trim(), price: Number(customPrice) || prev.price }));
                                      }});
                                    }
                                  }, 100);
                                }
              ...
              // Big Bright Yellow Net Weight Box: EXACT OM PREVIOUS STYLE
              c.jsxs("div", {
                className: "bg-[#fff176] dark:bg-yellow-700/90 p-6 rounded-[30px] shadow-xl border-b-[6px] border-yellow-600 dark:border-yellow-900 flex justify-between items-center active:translate-y-1 transition-all",
                children: [
                  c.jsxs("div", {
                    children: [
                      c.jsx("span", {
                        className: "text-[11px] font-black text-yellow-900 dark:text-yellow-100 uppercase italic block leading-none mb-1",
                        children: completedBatches.length > 0 ? "Tổng Tôm Xuất (Tất Cả Loại)" : "Tôm Xuất Bán (Không Trừ Bì)"
                      }),
                      c.jsxs("span", {
                        className: "text-5xl font-black text-black dark:text-white italic tracking-tighter tabular-nums leading-none",
                        children: [
              ...
              // Two Cards: Giá Bán (kg) & Quy Cách Trừ Bì: EXACT OM STYLE
              c.jsxs("div", {
                className: "space-y-3",
                children: [
                  c.jsxs("div", {
                    ...
                    children: [
                      targetGroup !== "MARKET" ? c.jsxs("div", {
                        className: "bg-white dark:bg-gray-800 p-4 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-sm transition-colors",
                        children: [
                          c.jsx("label", {
                            className: "text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase italic mb-2 block",
                            children: "Giá Bán (đ/kg)"
                          }),
                          c.jsx("input", {
                            type: "number",
                            value: A.price,
                            onChange: $ => v({ ...A, price: $.target.value }),
                            className: "w-full text-right text-3xl font-black text-[#1a237e] dark:text-blue-300 outline-none bg-transparent"
                          })
                        ]
                      }) : c.jsxs("div", {
                  ...
                  // Extra transport fields for Market (Chợ)
                  targetGroup === "MARKET" && c.jsxs("div", {
                    className: "grid grid-cols-2 gap-4",
                    children: [
                      c.jsxs("div", {
                        className: "bg-white dark:bg-gray-800 p-3 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xs",
                        children: [
                          c.jsx("label", { className: "text-[9px] font-black text-gray-400 uppercase italic block mb-1", children: "Biển Số Xe / Tài Xế" }),
                          c.jsx("input", {
                            type: "text",
                            placeholder: "VD: 69C-123.45",
                            value: A.vehiclePlate,
                            onChange: e => v({ ...A, vehiclePlate: e.target.value }),
                            className: "w-full text-xs font-bold text-gray-800 dark:text-gray-200 bg-transparent outline-none"
                          })
                        ]
                      }),
                      c.jsxs("div", {
                        className: "bg-white dark:bg-gray-800 p-3 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xs",
                        children: [
                          c.jsx("label", { className: "text-[9px] font-black text-gray-400 uppercase italic block mb-1", children: "SĐT / Giờ Xuất Xe" }),
                          c.jsx("input", {
                            type: "text",
                            placeholder: "VD: 0912... - 22:00",
                            value: A.driverPhone || A.marketDeliveryTime,
                            onChange: e => v({ ...A, driverPhone: e.target.value }),
                            className: "w-full text-xs font-bold text-gray-800 dark:text-gray-200 bg-transparent outline-none"
                          })
                        ]
                      })
                  ...
                  // Black Payment Card: EXACT OM PREVIOUS STYLE
                  c.jsxs("div", {
                    className: "bg-[#1e1e1e] dark:bg-black text-white p-6 rounded-[35px] shadow-2xl flex flex-col gap-1 border-b-8 border-black active:translate-y-1 transition-all",
                    children: [
                      c.jsxs("div", {
                      ...
                      c.jsx("p", {
                        className: "text-[11px] text-gray-400 italic font-semibold mt-1",
                        children: numberToWordsVN_v3(totalMoneyFinal)
                      })
                    ]
                  })
                ]
          ...
          // Weighing Tables Area: EXACT OM PREVIOUS STYLE
          c.jsxs("div", {
            ref: f,
            className: `px-3 transition-all duration-700 ${O ? "opacity-20 pointer-events-none translate-y-10" : "translate-y-0"}`,
            children: [
                ...
                children: [
                  c.jsxs("div", {
                    ...
                    children: [
                      ...
                      c.jsxs("span", {
                        className: "text-xs font-black uppercase text-gray-700 dark:text-gray-300 italic",
                        children: ["Đang cân bán: ", A.riceType, " (", Number(A.price).toLocaleString(), " đ/kg) - KHÔNG TRỪ BÌ"]
                      })
                    ]
                  }),
                  // Multi-type action: Chuyển loại tôm
              ...
              // Big Dashed "Sửa Thông Số" Button: EXACT OM PREVIOUS STYLE
              !O && c.jsxs("div", {
                className: "pb-40 pt-10 flex flex-col items-center gap-6",
                children: [
                  c.jsxs("button", {
                  ...
                  c.jsxs("button", {
                    ...
                    className: "w-full py-5 bg-gradient-to-r from-green-600 to-emerald-700 text-white font-black uppercase text-base italic rounded-[28px] shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all",
                    children: [
                      c.jsx(ec, { size: 24 }),
                      "Hoàn Tất & Lưu Phiếu Xuất"
                    ]
                  })
                ]
              })
      ...
      // Settlement Modal: EXACT OM PREVIOUS STYLE
      promptReq && c.jsxs("div", {
        className: "fixed inset-0 bg-black/60 z-[200] flex items-center justify-center p-4 backdrop-blur-sm",
        children: c.jsxs("div", {
          className: "bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-sm shadow-2xl animate-in zoom-in duration-200",
          children: [
            c.jsx("h3", { className: "font-black text-sm mb-4 text-gray-900 dark:text-white uppercase", children: promptReq.title }),
            c.jsx("input", {
              autoFocus: true,
              defaultValue: promptReq.defaultVal,
              id: "customPromptInput",
              className: "w-full p-3 border-2 border-blue-500 rounded-xl dark:bg-gray-900 dark:text-white font-bold text-lg mb-5 outline-none"
            }),
            c.jsxs("div", {
              className: "flex gap-3 justify-end",
              children: [
                c.jsx("button", { onClick: () => setPromptReq(null), className: "px-5 py-2.5 bg-gray-200 dark:bg-gray-700 font-bold rounded-xl", children: "Hủy" }),
                c.jsx("button", {
                  onClick: () => {
                    const val = document.getElementById("customPromptInput").value;
                    promptReq.onConfirm(val);
                    setPromptReq(null);
                  },
                  className: "px-5 py-2.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg",
                  children: "Xác nhận"
                })
              ]
            })
              ...
              children: [
                c.jsxs("div", {
                  children: [
                    c.jsx("h3", {
                      className: "text-xl font-black uppercase italic tracking-tight text-gray-900 dark:text-white",
                      children: "Quyết Toán Phiếu Xuất Bán Tôm"
                    }),
                    c.jsx("p", {
                      className: "text-xs text-gray-500 font-bold",
                      children: `${A.buyerName} - ${new Date().toLocaleDateString("vi-VN")}`
                    })
                  ]
                }),
                c.jsx("button", {
                ...
                c.jsxs("div", {
                  ...
                  children: [
                    c.jsxs("div", {
                      className: "flex justify-between",
                      children: [
                        c.jsx("span", { className: "text-gray-500 font-bold", children: "Tổng sản lượng tôm xuất:" }),
                        c.jsxs("span", { className: "font-black text-gray-900 dark:text-gray-100", children: [fmtKg(totalNetAllTypes), " kg (", totalBalesAllTypes, " rổ)"] })
                      ]
                    }),
                    c.jsxs("div", {
                      ...
                      children: [
                        c.jsx("span", { className: "font-bold", children: "Quy cách trừ bì:" }),
                        c.jsx("span", { className: "font-black", children: "0 kg (Không trừ bì)" })
                      ]
                    }),
                    c.jsxs("div", {
                      className: "flex justify-between items-baseline pt-2 border-t border-gray-200 dark:border-gray-700",
                      children: [
                        c.jsx("span", { className: "font-black uppercase text-sm text-gray-800 dark:text-gray-200", children: "Tổng tiền thu:" }),
                        c.jsxs("span", { className: "text-2xl font-black text-red-600", children: [_e(totalMoneyFinal), " VNĐ"] })
                      ]
                    }),
                    c.jsx("p", {
                      className: "text-xs italic text-gray-500 text-right font-bold pt-1",
                  ...
                  children: [
                    c.jsxs("div", {
                      ...
                      children: [
                        ...
                        c.jsx("span", {
                          className: "font-black uppercase text-xs",
                          children: Q ? "Đã thu đủ 100% tiền hàng" : "Ghi sổ nợ (Chưa thu tiền)"
                        })
                      ]
                    }),
                    c.jsx("span", {
      ...
      // History Modal: Lịch Sử Xuất Bán Tôm
      showHistoryModal && c.jsx("div", {
        className: "fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm",
        children: c.jsxs("div", {
...
const Xm_SaleDetailScreen = ({ setRoute, sale: initialSale, onBack }) => {
...
class Xm_ErrorBoundary extends w.Component{constructor(p){super(p);this.state={hasError:!1}}static getDerivedStateFromError(){return{hasError:!0}}componentDidCatch(e,i){console.error("App Error:",e,i)}render(){if(this.state.hasError)return c.jsxs("div",{className:"min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-6 text-center font-sans",children:[c.jsx("div",{className:"text-4xl mb-3",children:"🦐"}),c.jsx("h2",{className:"text-lg font-black text-white mb-2",children:"Cổng Hợp Đồng Mua Bán Tôm Càng Xanh"}),c.jsx("p",{className:"text-xs text-slate-400 mb-4 max-w-sm",children:"Đang làm mới phiên kết nối an toàn. Quý khách vui lòng bấm nút bên dưới để tải lại:"}),c.jsx("button",{type:"button",onClick:()=>window.location.reload(),className:"px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-lg cursor-pointer",children:"↺ Tải lại trang ngay"})]});return this.props.children}};
const to=document.getElementById("root");if(!to)throw new Error("Could not find root element to mount to");const wm=Z1.createRoot(to);wm.render(c.jsx(w1.StrictMode,{children:c.jsx(Xm_ErrorBoundary,{children:c.jsx(Bm,{})})}));
